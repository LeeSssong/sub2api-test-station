...
- 首次应用后又完成 14 个异常账号定向补跑。最终公开池为 Pro 6、专属 Pro 同池 6、Plus 7、特惠 5；`49,50,164,165,172,173` 已按证据归组，`80,105,106,163,168,169,170,171` 继续跳过或已删除。
- 最低利润已重新验证：公开 Pro 0.10、专属 Pro 0.05、Plus 0.05、特惠 0.02；公开池无未知成本、无不可调度成员、无利润违规账号，Pro/专属 Pro 完全镜像。
- 本轮并发新建的 `174–177` 只有 0–1 条探测，其中 177 放 Plus 仅有 0.04 利润，已全部隔离到测试组等待至少 20 个有效样本。
- 最终活动 blue API、worker、relay-ops 均 healthy、重启数 0，Caddy 内部 `/health` 为 200；评分/调度系统建议仍未实施。
...
- relay-ops：当前代码契约只保留 `/pricing`、健康检查、采集与出站通知；`/ops` 和 relay admin API 未挂载，Caddy 将所有旧 `/ops` 路径 `302` 到 Sub2API `/admin/ops`。日报与 15 分钟告警/持续提醒/恢复使用同一只读数据边界。飞书卡片只有指向 `/admin/accounts/monitor` 的导航按钮，不能确认、接手或修改状态；App Bot 出站投递只需要 App ID、App Secret 和目标会话配置，不再需要 verification token、Encrypt Key、命令路由文件或入站 callback。**2026-07-30 原生运维重定向和现有飞书纯提醒边界已完成生产部署并验证生效**；独立的原生 P0/P1 告警事件桥接仍处于设计/实施阶段；历史 relay-ops 表、迁移与旧 model-release 证据保留，但不定义当前上游或活动监控任务。
