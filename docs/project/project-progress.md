...
**本轮登记（2026-08-06）：** 账号监控评分权重入口、账号成本/预计额度与上游余额刷新修复，状态：进行中（设计已确认，正在编写正式规格；将恢复分组评分权重入口，以“采购成本 CNY / 预计可用额度 USD”统一采购账号成本倍率，为 OpenAI API Key 上游展示 USD 余额，并将 New API 付费倍率测量改为 6 小时自动刷新、单卡手动强制刷新。未完成实现、推送、受控部署和线上验证前不得标记完成）。
...
0.0. **账号监控成本、余额与评分权重修复**：恢复分组评分权重编辑入口；OpenAI API Key 账号使用上游倍率，OpenAI 非 API Key 账号使用采购成本和预计可用额度计算等效倍率；余额刷新跟随账号健康探测，New API 付费倍率测量每 6 小时自动刷新并支持单卡强制刷新。**状态：进行中（设计规格阶段，尚未实施、推送、部署或线上验证）**。
