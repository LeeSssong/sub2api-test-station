...
- 队列状态：T01、T02、T03、T04 已部署并完成各自既定线上验证，但用户复验确认仍有大量流水没有上游扣费信息。代码核对证明 T03 只实现管理员打开详情时的同步上游查询和空白值解析，没有独立持久化证据。T03-R1 用户可见独立顶层任务已启动；用户在 brainstorming 中否决直接扩展 `usage_logs`，将范围收敛为“独立逐笔证据 + 异常核对 + 账号财务首页 + OAuth 每日成本 + 查询时实时汇总”的管理员 MVP。当前新书面规格等待最终批准；旧候选实现冻结，不得恢复。T03-R1 部署和线上验收前不得启动 T05 或后续任务包。
...
- 当前状态：进行中（新书面规格等待用户最终批准）。独立任务基线为 `main@747c7fb14d1ded243794a77984778babece7c799`，分支 `codex/t03-r1-upstream-cost-persistence`。旧规格、旧计划和直接扩展 `usage_logs` 的候选提交 `1c3e87684`、`ce5691527` 已冻结并被新设计取代，不得实施、合并、推送或部署。
...
- T03-R1 用户可见独立顶层任务已完成生产证据探索、方案比较和分段设计批准；下一门禁只能是用户审阅并批准新书面规格，随后重新调用 `writing-plans`。旧计划不得继续执行。
