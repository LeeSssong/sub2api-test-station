...
**本轮 T01 大上下文入站上传稳定性（2026-08-11）：** 状态：READY_FOR_ROOT_REVIEW。候选工作区 `.worktrees/t01-upload-stability`，分支 `codex/t01-upload-stability`，基线 `main@54f35713323116ce44646041347bde9ce548295e`。范围仅限 Caddy 入站长窗口、现有请求体大小保护兼容、配置合同测试及受控慢速/不完整上传验证；不修改错误转译、重试、调度、CDN、生产或其他任务包。已登记规格 `docs/superpowers/specs/2026-08-11-t01-large-context-upload-stability-design.md` 与计划 `docs/superpowers/plans/2026-08-11-t01-large-context-upload-stability.md`。实现：Caddy `read_body 15m` 与公共 fallback `response_header_timeout 15m`，原生 128/128/16 MiB body limits 保持不变；新增配置合同和受控上传测试。验证：301 秒持续上传成功、不完整上传释放；Caddy validate/adapt、Caddy/NGINX 合同、后端 ingress/body-limit 定向测试、`go vet`、构建、`git diff --check` 通过。完整 `tests/infra/validate-baseline.sh` 在既有无关 Dockerfile `ARG FRONTEND_NODE_MAX_OLD_SPACE_SIZE=1536` 合同不匹配处失败，未修改该无关问题；无迁移、无生产动作，尚未合并、推送、部署或线上验证。
