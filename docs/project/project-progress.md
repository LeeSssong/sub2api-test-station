...
**T12 经营页本站探测花费与排序/美元字段优化登记（2026-08-16）：** 状态：进行中（`DESIGNING`，仅登记/排队）。用户要求保持未消费金额 USD，补充请求/Token/账号计费/用户扣费/利润/利润率六项排序，新增独立“本站探测花费”字段/卡片/账号列；探测记录与用户消费隔离，不影响账号成本、用户成本、利润或利润率；外部金额两位小数、内部原始精度保留；不做历史迁移/回填，启用后重新记录。官方 v0.1.177 当前因 `downtime_required=true` 处于 `BLOCKED` 并独占发布优先级，本任务不得进入实现或发布车道。用户可见 GPT-5.6 Sol/medium 顶层任务 `01a0094a-190c-76a3-ab56-eed3ecc8d824` 已创建，独立 worktree `/Users/gongtengxinwen/.codex/worktrees/16af/sub2api搭建` 当前停在规格审批前；其 docs-only 候选 `6db469c7278d7518641d75b71714efe6d2c64a7f` 已完成 brainstorming/现状盘点/正式规格/自审/交接，未调用 writing-plans、未实现、未合并、未推送、未部署、未访问生产。已发现旧 `codex/account-probe-cost-design@893933924` 仅有 docs-only 规格草案，现冻结为只读设计证据，不复用其分支或 worktree。
