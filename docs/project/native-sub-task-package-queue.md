...
- **T47-R1 operational timeline color correction**: status DEPLOYING. Candidate `codex/t47-r1-operational-green@8d38819d0` has been fast-forwarded into root `main`; both candidate and merged-root focused Vitest 20/20, `pnpm typecheck`, `pnpm build`, and `git diff --check` passed. The correction removes T47's frontend-only latency `>= 2000ms` degraded override so every runnable/`operational` bucket is green with `UP`; unavailable remains red and no-probe remains gray. No API, data source, migration, config, or production-data changes. Root `main` is ready for push and the existing preloaded blue-green release chain; online verification remains pending.
