...
**T19 Monitor V2 缓存命中率有效样本口径修正（2026-08-17）：** 用户明确批准按既有方案修正并加入全局任务队列；候选已在独立 worktree `/Users/gongtengxinwen/Documents/sub2api搭建/.worktrees/t19-monitor-v2-cache-eligibility`、分支 `codex/t19-monitor-v2-cache-eligibility` 刷新到根 `main@8729884a113cf844a2850ba87463c2f7f711577c`；最新候选 tip `1b8832461`、tree `1484c609a9a4f4281eae6dcf0ce71b1f16d0c6a6`，刷新合并提交 `c4fc01c53802300bec61c5c8e5d55c58cff82a2a`，功能提交 `0f9ef38f2a0621d9afe5b5c965da025161dba399`，交接为 `docs/handoffs/2026-08-17-t19-monitor-v2-cache-eligibility-handoff.md`。范围仅限 `upstream/sub2api/backend/internal/repository/monitor_v2_repo.go` 的 Monitor V2 缓存统计 SQL 与直接相关 sqlmock 测试：分子/分母限定为 `actual_cost > 0`、成功流水及文本 Token Prompt Cache 语义（`billing_mode='token'`，或历史 `billing_mode` 为空且图片/视频字段全零），排除图片/视频/按请求计费及零成本失败占位；不改 API、前端、账务/价格/倍率或缓存策略，无迁移、无生产数据写入，预期 `downtime_required=false`。刷新后 TDD RED/GREEN、仓储/服务聚焦测试、后端 compile-only/build、gofmt、diff-check 均通过。**状态：进行中（READY_FOR_ROOT_REVIEW；用户最新指令为暂不发布，候选未合并、未推送、未部署；排在 T15 发布车道完成后、T18 候选之后）。**

**T16 经营页真实结果与视觉层级重设计（2026-08-17）：** 用户最新指令“没有其他的待实施任务了吗？如果有就接着实施”恢复本项。独立 worktree `/Users/gongtengxinwen/.codex/worktrees/026c/sub2api搭建`、分支 `codex/t16-profitability-visual-hierarchy` 基线为根 `main@483dde398`，候选 tip `818b119f7179ba4aec0274f518d78bd13ed68afa`，实现提交 `a9068dbb683c583ad3cefe01943899638b7abe1e`，tree `4f14ef0de02029ca4fd4cecbcb29efcdf9f0e8fa`。已完成正式规格/计划、TDD 实现、后端直接相关测试与 compile-only/build、前端 19 tests、typecheck、build、diff-check，交接见 `docs/handoffs/2026-08-17-t16-profitability-visual-hierarchy-handoff.md`。范围继续复用 Sub 原生 `usage_logs`，按只读 `users.role='admin'` 分类内部运营消耗，明确历史角色变更风险，不用 `user_cost=0` 猜身份，不做迁移/回填/生产写入，不改变原生成本公式或发布控制面。**状态：进行中（READY_FOR_ROOT_REVIEW；按用户指令暂不发布，未合并、未推送候选、未预检、未部署、未线上验收）。**
