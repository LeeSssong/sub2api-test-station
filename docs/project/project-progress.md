...
**T12 经营页本站探测花费与排序/美元字段优化（2026-08-17）：** 依据修订规格、自审、实施计划和离席代审授权，从最新根 `main@5fea0f665280b988aef927534a75be23934bae32` 恢复干净候选；当前 worktree `/Users/gongtengxinwen/Documents/sub2api搭建/.worktrees/t12-native-probe-cost-cards`、分支 `codex/t12-native-probe-cost-cards`，基线已确认无漂移。执行范围为逐提交移植 Task 1-3，并按新页面合同用 TDD 重写 Task 4；旧 `a54222c535` worktree 与唯一未提交 RED 测试继续只读保全。**状态：进行中（IMPLEMENTING；候选尚未合并、推送、部署或线上验证）。**
