...
func TestPrepareUsageLogInsert_SessionIDArgWiring(t *testing.T) {
	require.Len(t, usageLogInsertArgTypes, 71, "arg-type table must include upstream cost persistence fields")
	...
	sessionArg := prepared.args[len(prepared.args)-12]
	...
	require.Equal(t, "text", usageLogInsertArgTypes[len(usageLogInsertArgTypes)-12],
...
func TestPrepareUsageLogInsert_SessionIDNullWhenAbsent(t *testing.T) {
	...
	nsEmpty := preparedEmpty.args[len(preparedEmpty.args)-12].(sql.NullString)
...
func TestPrepareUsageLogInsert_AttemptAuditFields(t *testing.T) {
	...
	require.Equal(t, "logical-1", prepared.args[len(prepared.args)-11].(sql.NullString).String)
	require.Equal(t, "logical-1:2", prepared.args[len(prepared.args)-10].(sql.NullString).String)
	require.Equal(t, "partial", prepared.args[len(prepared.args)-9])
	require.Equal(t, true, prepared.args[len(prepared.args)-8])
	require.Equal(t, true, prepared.args[len(prepared.args)-7])
...
func TestPrepareUsageLogInsert_NormalizesUnsupportedUsageCompleteness(t *testing.T) {
	...
	require.Equal(t, "unknown", prepared.args[len(prepared.args)-9])
}
