<template>
  <section class="mt-5 min-w-0 border-t border-slate-700/80 pt-5" aria-labelledby="codexradar-community-title">
      ...
      <div class="min-w-0">
        <div id="codexradar-community-title" class="flex flex-wrap gap-2" role="tablist" aria-label="CodexRadar 社区测试类型">
          <button
            ...
            :class="activeKey === tab.key ? 'border-blue-400/70 bg-blue-400/15 text-blue-100' : 'border-slate-600 bg-slate-900 text-slate-300 hover:border-slate-500 hover:text-white'"
            @click="activeKey = tab.key"
          >{{ tab.icon }} {{ tab.title }}</button>
        </div>
        <p class="mt-2 text-xs font-medium text-slate-400">社区众测数据 · 每多一份贡献，结果就更准确。</p>
        <p v-if="activeKey === 'comprehensive'" class="mt-1 text-[11px] leading-4 text-slate-500">综合 IQ 为软件工程能力 IQ 与视觉空间推理 IQ 的等权几何平均，仅纳入两维均有有效成绩的档位。</p>
      </div>
      <div v-if="activeTab" class="flex shrink-0 items-center gap-2 text-xs font-medium text-slate-400">
        <span v-if="community?.stale" class="rounded-full border border-amber-400/40 bg-amber-400/10 px-2 py-0.5 text-[11px] text-amber-300">最近成功数据</span>
