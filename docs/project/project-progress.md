...
**T15 账号监控原生探测模型与异步模型检测（2026-08-17）：** 独立 worktree `/Users/gongtengxinwen/Documents/sub2api搭建/.worktrees/t15-native-probe-model-detection`、分支 `codex/t15-native-probe-model-detection` 的候选 `fcefd06f86bc2aa43de6ce33737ac3f6fda285e7`、tree `8cf5d21f0b8536497517124c3c8daa4758522fa3` 已合入根 `main`；合并后 migration/service/repository/routes focused、受影响后端 compile-only、前端两文件 93 tests、`vue-tsc --noEmit`、`vite build`、gofmt 与 diff-check 均通过。预加载宿主预检因新增 migration `225_account_model_detection.sql` 返回 `downtime_required=true`、`reason_code=migration_set_changed`、预计不可用 300 秒，并在任何生产变更前停止；门禁证据为 `/Users/gongtengxinwen/.codex/release-evidence/sub2api/2026-08-17-main-06040ebd5-t15-downtime-gate-v1.json`。用户已于 2026-08-17 明确授权停机部署。根核对确认生产迁移集 `aaebed88f7fb712e1f518e73cc89bd44eb214f365f3b49f003598c93883a4604` 到候选迁移集 `bb6ebff31f0ffe9be5ad204ba79ef896d98522ccdd7b3933843c94d6c9ad5951` 尚未进入宿主精确 allowlist；新增 T15 合同测试先真实 RED 为 `migration_set_changed`，现仅补充该 `MAINTENANCE_11` 哈希对，错误 old/new、未知目标和未授权路径继续 fail-closed。实现保持 detector 许可证门禁，生产不配置 `SUB2API_MODEL_DETECTOR_URL`/`SUB2API_MODEL_DETECTOR_TOKEN`。**状态：进行中（DEPLOYING；停机授权已取得，维护门禁补丁验证中，生产未变）。**

**T18 渠道状态官方聚合/自建监控可切换（2026-08-17）：** 用户可见顶层任务已在独立 worktree `/Users/gongtengxinwen/Documents/sub2api搭建/.worktrees/channel-status-official-toggle`、分支 `codex/channel-status-official-toggle` 刷新到根 `main@8729884a113cf844a2850ba87463c2f7f711577c`；最新候选 tip `c088b00cc`、tree `d0300320714776a5f532233c6a601dcc4a4cf4cf`，刷新合并提交 `607909f12f3d1523cc5a6d09bf2087cff7973540`，worktree 干净。功能提交 `ee52dd5ea`；仅改 `MonitorV2RouteView` 入口与专项测试，复用 `channel_monitor_mode=v1|v2`，无后端/迁移/workflow 变化。刷新后 `MonitorV2RouteView` 3/3、`pnpm typecheck`、`pnpm build` 与 `git diff --check main...HEAD` 通过；上线参数为 `channel_monitor_enabled=true`、`channel_monitor_mode=v2`，回滚为 `channel_monitor_mode=v1`。**状态：进行中（READY_FOR_ROOT_REVIEW；用户最新指令为暂不发布，排队等待 T15 停机门禁释放，未合并、未推送、未部署、未线上验收）。**
