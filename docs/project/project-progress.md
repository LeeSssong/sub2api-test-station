...
**S1-R2 确定性故障原生隔离编排候选（2026-08-17）：** 用户可见顶层任务 `01a00da8-ed25-7b72-b9d9-cdcee5fa75c1` 在独立 worktree `/Users/gongtengxinwen/.codex/worktrees/6195/sub2api搭建`、分支 `codex/s1-r2-native-deterministic-failure-isolation` 已完成根 `main` 刷新、generic 402 根审 follow-up、TDD 实现、直接相关验证和 handoff；当前候选 SHA 以 handoff fresh `git rev-parse HEAD` 为准，worktree 干净。实现把余额不足统一投影为原生 90 分钟账号冷却（配置 60–120，越界回退 90），把明确模型不支持投影为账号 + canonical model 的原生 `model_rate_limits/probe_required`，保持 API Key 明确凭据失效使用原生 error/不可调度，并把未收到成功终态的 SSE 接入既有账号模型 transient cooldown/failover，同时保留输出后禁止重放；泛化 403、网络失败、空/截断模型目录不硬隔离。generic 402 现在仅在稳定机器码或明确余额/额度耗尽消息下进入确定性分类，明确余额 402 仍保留；直接相关 service、unit OAuth/model recovery、config、受影响包 compile-only、server build、gofmt 与 diff-check 均通过；无迁移、未使用 225/226、无 GitHub Actions 变化。交接为 `docs/handoffs/2026-08-17-s1-r2-native-deterministic-failure-isolation-handoff.md`。用户部署冻结继续有效，本候选受保护且不得合并、push、预检、部署或访问生产；`downtime_required=unverified`，S2/S3 不得启动。**状态：准备完成（READY_FOR_ROOT_REVIEW；未合并、未推送、未部署、未线上验证）。**
