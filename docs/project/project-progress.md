...
**本轮推进（2026-08-07）：** 管理页官方升级适配蓝绿生产拓扑继续保持“进行中”。已复现并修正发布控制器测试中的过期迁移哈希断言，且为本地 `sub2api-release-metadata.rb discover` 补充受测试保护的 `--force-rebuild`，用于在官方版本与 provenance 相同的情况下生成新的同版本审计候选；尚未提交、推送、重建候选、部署或线上验证。
