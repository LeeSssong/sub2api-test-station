...
**本轮生产逐笔成本二次收口（2026-08-10）：** 状态：进行中。生产 `main@9e24bfde27a9e1a4751d3a30158e24067b900670`、版本 `0.1.173` 的蓝绿发布和版本身份验证已成功，但逐笔成本验收继续暴露两条独立事实：一是 New API 的逐笔日志使用其即时中转生成并通过 `X-Oneapi-Request-Id` 返回的 `request_id`，本站当前只保存可能属于下一级供应商的 `x-request-id`，导致精确 ID 匹配失败；二是账号 `#42 Plus-WAWAZZ` 虽返回 `sub2api.key_billing` 倍率声明，但其 `/v1/usage/records` 与 New API 日志/状态接口均为 HTTP 404，本站 `account_cost` 只是配置/规则计算快照，禁止冒充上游实际扣费。候选 `.worktrees/fix-production-defects` 已快进到当前 `main`；实施前基线还发现带 `unit` 标签的 service 测试无法编译，原因是 `0.1.173` 合并保留了 resolved account-cost 回归测试，却丢失 `postUsageBillingParams.AccountCost/AccountCostSet` 及其 account quota 消费链。本轮按 `docs/superpowers/plans/2026-08-10-production-cost-followup.md` 在原失败候选中先恢复该计费合同，再以 TDD 修复 New API 即时账单请求 ID 持久化，经独立任务复审、整枝复审、合并 `main`、并版门禁、推送、预加载蓝绿发布和新鲜自然流水线上验证后才可完成；两个用户明确保护 worktree 继续不修改、不合并、不清理，不使用 GitHub Actions。
