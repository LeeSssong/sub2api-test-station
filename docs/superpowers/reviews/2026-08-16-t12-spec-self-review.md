...
- **历史保留已闭环：** `account_id` 外键固定为 `ON DELETE RESTRICT`，明确禁止 `CASCADE`；存在 probe 历史时账号物理删除失败，append-only 记录不会丢失或成为孤儿。
- **财务安全已闭环：** probe 成本永不进入账号计费、用户扣费、利润、利润率或余额；缺失 Token/价格只显示不完整，不估算。
- **查询失败语义已闭环：** `probe_cost_status=unavailable` 仅表示查询成功但无记录；聚合失败由顶层 `probe_data_error=true` 和稳定 `probe_error_code` 表达，probe 数值/status 为 `null`，UI 显示故障/重试而非 `$0.00`，用户六项继续返回。
- **USD 与兼容已闭环：** 现有未消费金额字段、接口名称和 DTO 完全保持；T12 只维持/校正经营页 USD 两位显示，不新增字段、alias、deprecation、余额迁移或其他页面改造。
- **页面合同已闭环：** 页面按全站、分组、账号三层组织；账号层一账号一卡，同卡区分用户流水账号成本与本站探测花费；桌面最多两列、390px 单列且不依赖横向滚动。利润率固定两位小数，余额不显示 CNY 语义。
- **范围无漂移：** 仅覆盖探测写入链路、account-financial 读模型/API 和经营页 UI；未包含调度、用户入口、历史数据、外部控制面、余额 DTO 重构或部署动作。

结论：`PASS（最小修订后重新请求根总控书面审阅）`。本轮仍未调用 `writing-plans`，未写实现/迁移/测试，未访问生产。
