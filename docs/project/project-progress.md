...
**T12 经营页本站探测花费与排序/美元字段优化（2026-08-17）：** 候选 `codex/t12-native-probe-cost-cards@c7587599a7947e6103a915fdfc9f030d343bb198` 已完成功能与直接相关测试，曾无冲突合并至根 `main@7ded6b219d0e87334c9db63c607ab27044c063ff`，但在发布链启动前因新 P0 会话事故紧急停止；生产未构建、未迁移、未切换。根总控已用 `408e7a1a3` 与 `9d9e2b758` 两个可追溯 revert 将尚未发布的 T12 从 `main` 移出；候选 worktree、分支与测试证据原样保全，禁止继续写入、合并、推送、部署或清理。**状态：FROZEN（待 P0 生产收口后由唯一发布总控明确解冻）。**
