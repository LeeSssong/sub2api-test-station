...
日期：2026-09-01
任务：T87 逻辑请求错误生命周期投影
基线：`main@d61fa0c698ae779a3b689e057896326b9209470b`
候选 worktree：`codex/t87-logical-request-error-lifecycle`
状态：设计中，等待书面规格审阅批准

## 1. 摘要

当前系统已经能够在部分安全场景下对同一用户请求执行同账号重试或跨账号切号，但生产错误记录多数仍按一次 attempt 或一次 `ops_error_logs` 行展示。这样会产生三类误判：
...
本任务只增加对既有请求事实的**逻辑请求生命周期读时投影**。成功必须代表用户最终收到完整协议终态；只有中间错误最终恢复成功时才隐藏中间错误。最终错误、流内终止、重试耗尽和不可安全重放停止必须对用户可见并计入失败。不得新增第二套错误事实源、账务事实源或运行时平行控制面。
