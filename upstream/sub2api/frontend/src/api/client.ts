...
import { authStorageGet, authStorageRemove } from '@/utils/authStorage'
...
apiClient.interceptors.request.use(
  (config: InternalAxiosRequestConfig) => {
    ...
    const token = authStorageGet('auth_token')
  ...
  async (error: AxiosError<ApiResponse<unknown>>) => {
    ...
    if (error.response) {
      ...
      if (status === 401 && !originalRequest._retry) {
        const refreshToken = authStorageGet('refresh_token')
        const isAuthEndpoint =
          url.includes('/auth/login') || url.includes('/auth/register') || url.includes('/auth/refresh')

        // If we have a refresh token and this is not an auth endpoint, try to refresh
        if (refreshToken && !isAuthEndpoint) {
          const refreshSessionUser = authStorageGet('auth_user')
          ...
          try {
            ...
            const sessionChanged =
              authStorageGet('refresh_token') !== refreshToken ||
              authStorageGet('auth_user') !== refreshSessionUser
            if (sessionChanged) {
              return Promise.reject({
                status: 401,
        ...
        const hasToken = !!authStorageGet('auth_token')
