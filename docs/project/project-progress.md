...
| 待部署/待验证/阻塞 | 14 | 进行中；必要时注明外部阻塞/历史保留 |
| **合并后的独立事项合计** | **46** | **当前总账快照** |
...
6. **Monitor V2 卡片信息精简**：已确认移除各指标样本数量与模型列表，并停止接口返回模型字段；实现、推送、部署与生产验证尚未完成。**状态：进行中**。
...
14. **Monitor V2 卡片信息精简**：删除四项指标的样本数量、模型折叠区和接口中的 `models` 字段，保留性能指标、有效调用、P95 与趋势。**状态：进行中，待实现、推送、部署和生产验证**。
