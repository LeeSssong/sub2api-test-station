...
**T17 用量详情“上游扣费/利润”统一 Sub 原生有效账号成本口径热修（2026-08-17）：** S3 已完成生产收口；T17 独立 worktree `/Users/gongtengxinwen/Documents/sub2api搭建/.worktrees/t17-effective-account-cost-hotfix`、分支 `codex/t17-effective-account-cost-hotfix` 已从根 `main@120562f95` 刷新，刷新合并提交 `e887bfe44`；正式规格提交 `6a164393d`、实施计划提交 `14528991b` 已经代审批准。已确认使用记录列表与账号利润/经营页的主成本口径均为 `COALESCE(account_cost, COALESCE(account_stats_cost, total_cost) * COALESCE(account_rate_multiplier, 1))`，而详情弹窗错误以 `usage_upstream_cost_evidence.normalized_cost_cny` 决定主金额，导致严格 evidence 为 `unavailable` 时即使 `account_cost` 已有值仍显示 `-`。生产 `usage_log_id=125444/125509/125512` 以及账号 214 当日 518 笔流水已提供口径不一致证据；两个详情 API 均 HTTP 200，不是接口失败。候选已将详情主成本统一为 effective account cost，利润统一为 `actual_cost - effective_account_cost`，历史 `account_cost` 为空时使用原生 fallback；严格 evidence 只保留核验状态/原因。不改价格、倍率、成本写入或经营页公式，无迁移、回填、生产数据修改、账务重算或 evidence 表删除。直接相关前端 38/38 测试、typecheck/build、后端管理员/DTO focused tests、server compile-only/build 和范围检查均通过；交接见 `docs/handoffs/2026-08-17-t17-effective-account-cost-hotfix-handoff.md`，验证见 `docs/superpowers/reports/2026-08-17-t17-effective-account-cost-hotfix-verification.md`。根当前另有 docs-only 提交 `85454d883`，合并前必须刷新候选。**状态：准备根审（READY_FOR_ROOT_REVIEW；排在 S3 生产收口之后）。**
