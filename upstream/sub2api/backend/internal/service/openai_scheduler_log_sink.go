	...
	for {
		...
		case event := <-s.queue:
			...
			decision, err := json.Marshal(map[string]any{
				...
				"selected_priority": event.SelectedPriority, "selected_priority_signal": event.SelectedPrioritySignal,
				"cold_start_priority_signal": event.ColdStartPrioritySignal,
				"daily_priority_signal":      event.DailyPrioritySignal,
