...
func (_q *RedeemCodeQuery) QueryQuotaGrants() *UserQuotaGrantQuery {
...
func (_q *RedeemCodeQuery) WithQuotaGrants(opts ...func(*UserQuotaGrantQuery)) *RedeemCodeQuery {
	query := (&UserQuotaGrantClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *RedeemCodeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*RedeemCode, error) {
	...
	if query := _q.withQuotaGrants; query != nil {
		if err := _q.loadQuotaGrants(ctx, query, nodes,
			func(n *RedeemCode) { n.Edges.QuotaGrants = []*UserQuotaGrant{} },
			func(n *RedeemCode, e *UserQuotaGrant) { n.Edges.QuotaGrants = append(n.Edges.QuotaGrants, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *RedeemCodeQuery) loadQuotaGrants(ctx context.Context, query *UserQuotaGrantQuery, nodes []*RedeemCode, init func(*RedeemCode), assign func(*RedeemCode, *UserQuotaGrant)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(userquotagrant.FieldRedeemCodeID)
	}
	query.Where(predicate.UserQuotaGrant(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(redeemcode.QuotaGrantsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
