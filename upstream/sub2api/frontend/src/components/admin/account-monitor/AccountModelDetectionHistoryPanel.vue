<template>
  <div v-if="show" class="fixed inset-0 z-[80] flex bg-slate-950/70 backdrop-blur-[2px]" data-test="detection-history-panel" @click.self="emit('close')">
    <section class="ml-auto flex h-full w-[min(34vw,520px)] min-w-[420px] flex-col border-l border-slate-700/80 bg-slate-950 text-slate-100 shadow-[-12px_0_32px_rgba(2,6,23,0.28)] max-md:w-full max-md:min-w-0 max-md:border-l-0" role="dialog" aria-modal="true" aria-labelledby="detection-history-title">
      <header class="flex items-start justify-between gap-3 border-b border-slate-800 px-4 py-3">
        <div class="min-w-0"><h2 id="detection-history-title" class="text-sm font-semibold text-white">{{ t('admin.accounts.modelDetection.historyTitle') }}</h2><p class="mt-1 truncate text-[11px] text-slate-400">{{ account?.name }} #{{ account?.account_id }}</p></div>
      ...
      <div class="space-y-1.5 border-b border-slate-800 px-4 py-2">
        <label class="sr-only" for="detection-history-status-filter">{{ t('admin.accounts.modelDetection.statusFilter') }}</label><select id="detection-history-status-filter" v-model="statusFilter" class="h-7 w-full rounded-md border border-slate-700 bg-slate-800 px-2.5 text-[10px] text-slate-100 outline-none transition focus:border-cyan-400 focus:ring-1 focus:ring-cyan-400/30" data-test="detection-history-status-filter"><option value="">{{ t('common.all') }}</option><option value="normal">{{ t('admin.accounts.modelDetection.status.normal') }}</option><option value="abnormal">{{ t('admin.accounts.modelDetection.status.abnormal') }}</option><option value="insufficient">{{ t('admin.accounts.modelDetection.status.insufficient') }}</option></select>
        <label class="sr-only" for="detection-history-profile-filter">{{ t('admin.accounts.modelDetection.profileFilter') }}</label><select id="detection-history-profile-filter" v-model="profileFilter" class="h-7 w-full rounded-md border border-slate-700 bg-slate-800 px-2.5 text-[10px] text-slate-100 outline-none transition focus:border-cyan-400 focus:ring-1 focus:ring-cyan-400/30" data-test="detection-history-profile-filter"><option value="">{{ t('common.all') }}</option><option value="low">{{ t('admin.accounts.modelDetection.profileNames.low') }}</option><option value="medium">{{ t('admin.accounts.modelDetection.profileNames.medium') }}</option><option value="high">{{ t('admin.accounts.modelDetection.profileNames.high') }}</option><option value="unknown">{{ t('admin.accounts.modelDetection.profileNames.unknown') }}</option></select>
      </div>

      <div v-if="loading && !items.length" class="px-4 py-6 text-center text-xs text-slate-400" role="status">{{ t('admin.accounts.modelDetection.loading') }}</div>
      <div v-else-if="loadError" class="px-4 py-6 text-center text-xs text-rose-300" data-test="detection-history-error" role="alert"><p>{{ t('admin.accounts.modelDetection.historyLoadError') }}</p><button class="btn btn-secondary mt-3" type="button" @click="load()">{{ t('common.refresh') }}</button></div>
      <div v-else-if="!items.length" class="px-4 py-6 text-center text-xs text-slate-400" data-test="detection-history-empty">{{ t('admin.accounts.modelDetection.historyEmpty') }}</div>
      <div v-else class="min-h-0 flex-1 overflow-y-auto px-4 py-3">
        <div class="hidden overflow-hidden border border-slate-800 md:block" data-test="detection-history-table">
          <table class="w-full table-fixed text-left text-[10px]"><thead class="bg-slate-900/90 text-slate-400"><tr><th class="w-[96px] px-2 py-2 font-medium">{{ t('admin.accounts.modelDetection.time') }}</th><th class="w-[88px] px-2 py-2 font-medium">{{ t('admin.accounts.modelDetection.profile') }}</th><th class="px-2 py-2 font-medium">{{ t('admin.accounts.modelDetection.reason') }}</th><th class="w-[74px] px-2 py-2 font-medium">{{ t('admin.accounts.modelDetection.samples') }}</th><th class="w-[58px] px-2 py-2 font-medium">{{ t('admin.accounts.modelDetection.statusLabel') }}</th></tr></thead><tbody><template v-for="item in items" :key="item.run_id"><tr class="cursor-pointer border-t border-slate-800 transition-colors hover:bg-slate-900/70" @click="toggle(item.run_id)"><td class="whitespace-nowrap px-2 py-2 text-slate-300">{{ formatTime(item.finished_at || item.queued_at) }}</td><td class="px-2 py-2"><span class="rounded-full border px-1.5 py-0.5 font-mono text-[10px]" :class="profileClass(item)">{{ profileLabel(item) }}</span></td><td class="break-words px-2 py-2 text-slate-300">{{ reasonLabel(item) }}</td><td class="px-2 py-2 font-mono text-slate-200">{{ samplesLabel(item) }}</td><td class="px-2 py-2"><span class="font-semibold" :class="statusClass(item.status)" data-test="detection-history-row">{{ statusLabel(item.status) }}</span></td></tr><tr v-if="selected === item.run_id" class="border-t border-slate-800 bg-slate-900/50"><td colspan="5" class="px-3 py-3"><Detail :item="item" /></td></tr></template></tbody></table>
        </div>
