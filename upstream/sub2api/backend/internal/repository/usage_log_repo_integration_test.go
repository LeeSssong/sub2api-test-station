...
func newUpstreamCostPersistenceUsageLog(user *service.User, apiKey *service.APIKey, account *service.Account) *service.UsageLog {
	status := service.UsageUpstreamCostStatusConfirmed
	reason := "sub_bill_available"
	cost := 0.004
	profit := 0.00288
...
func requireUpstreamCostPersistence(t *testing.T, got *service.UsageLog) {
	t.Helper()
	...
	require.Equal(t, "sub_bill_available", *got.UpstreamCostReason)
	require.NotNil(t, got.Profit)
	require.InEpsilon(t, 0.00288, *got.Profit, 1e-9)
	require.NotNil(t, got.UpstreamCostRecordedAt)
	require.True(t, got.UpstreamCostRecordedAt.Equal(time.Date(2026, 8, 12, 13, 0, 0, 0, time.UTC)))
}

func TestUsageLogRepositoryUpstreamCostPersistenceNormalBatch(t *testing.T) {
	ctx := context.Background()
	client := testEntClient(t)
	repo := newUsageLogRepositoryWithSQL(client, integrationDB)
	repo.outbox = nil // exercise the normal asynchronous batch path.

	user := mustCreateUser(t, client, &service.User{Email: "usage-upstream-cost-batch-" + uuid.NewString() + "@example.com"})
	apiKey := mustCreateApiKey(t, client, &service.APIKey{UserID: user.ID, Key: "sk-usage-upstream-cost-batch-" + uuid.NewString(), Name: "k"})
	account := mustCreateAccount(t, client, &service.Account{Name: "acc-usage-upstream-cost-batch-" + uuid.NewString()})
	log := newUpstreamCostPersistenceUsageLog(user, apiKey, account)
	...
	got, err := repo.GetByID(ctx, log.ID)
	require.NoError(t, err)
	requireUpstreamCostPersistence(t, got)
}

func TestUsageLogRepositoryUpstreamCostPersistenceBestEffortBatch(t *testing.T) {
	ctx := context.Background()
	client := testEntClient(t)
	repo := newUsageLogRepositoryWithSQL(client, integrationDB)
	repo.outbox = nil // exercise the best-effort asynchronous batch path.

	user := mustCreateUser(t, client, &service.User{Email: "usage-upstream-cost-best-effort-" + uuid.NewString() + "@example.com"})
	apiKey := mustCreateApiKey(t, client, &service.APIKey{UserID: user.ID, Key: "sk-usage-upstream-cost-best-effort-" + uuid.NewString(), Name: "k"})
	account := mustCreateAccount(t, client, &service.Account{Name: "acc-usage-upstream-cost-best-effort-" + uuid.NewString()})
	log := newUpstreamCostPersistenceUsageLog(user, apiKey, account)
	...
	require.Eventually(t, func() bool {
		err := integrationDB.QueryRowContext(ctx, "SELECT id FROM usage_logs WHERE request_id = $1 AND api_key_id = $2", log.RequestID, log.APIKeyID).Scan(&id)
		return err == nil && id > 0
...
func TestUsageLogRepositoryUpstreamCostPersistenceAtomicOutbox(t *testing.T) {
	ctx := context.Background()
	client := testEntClient(t)
	repo := newUsageLogRepositoryWithSQL(client, integrationDB)

	user := mustCreateUser(t, client, &service.User{Email: "usage-upstream-cost-atomic-" + uuid.NewString() + "@example.com"})
	apiKey := mustCreateApiKey(t, client, &service.APIKey{UserID: user.ID, Key: "sk-usage-upstream-cost-atomic-" + uuid.NewString(), Name: "k"})
	account := mustCreateAccount(t, client, &service.Account{Name: "acc-usage-upstream-cost-atomic-" + uuid.NewString()})
	log := newUpstreamCostPersistenceUsageLog(user, apiKey, account)
...
WHERE event_type = 'request.completed' AND payload->>'request_id' = $1
`, log.RequestID).Scan(&outboxCount))
