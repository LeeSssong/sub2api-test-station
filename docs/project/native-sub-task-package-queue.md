...
- **T35 采购保存 PostgreSQL 参数类型热修**：状态 `DESIGNING`。独立用户可见任务 `01a01b11-3613-7ce0-811e-93d986e5cf16` 使用 `gpt-5.6-terra/high`，worktree `/Users/gongtengxinwen/.codex/worktrees/eeb6/sub2api搭建`，基线 `main@101357776e1af9dbf83df282afd96cdb284ffcf4`，分支 `codex/t35-procurement-audit-type-hotfix`。正式规格 `5dca4b13104bad06658b39acd7d5caf79d5b38be` 已由根总控审查并批准：三条 `jsonb_build_object` 审计 SQL 的动态参数分别固定为账号 `bigint`、金额/额度 `double precision`、原因 `text`；真实 PostgreSQL 18 PREPARE 探针已复现未定型参数错误并验证 casts 成功。实施必须以真实 PostgreSQL 直接调用生产 service 的录入、清空、结算 RED/GREEN 为主证据，同时保留源码精确合同和 sqlmock 回滚控制流；采购版本台账、事务、幂等、审计、成本公式和前端合同不变。范围不含迁移、历史回填、生产数据改写、账务公式或其他页面；预期 `downtime_required=false`，最终以根发布预检为准。
