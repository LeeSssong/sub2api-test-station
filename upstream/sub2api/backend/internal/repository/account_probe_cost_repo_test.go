...
func TestAccountProbeCostRepositoryAppendDuplicateIdenticalIsIdempotent(t *testing.T) {
	...
	}).AddRow(log.ProbeRunID, log.AccountID, *log.GroupID, string(log.ProbeKind), log.Model, log.InputTokens, log.OutputTokens, log.CacheCreationTokens, log.CacheReadTokens, *log.AccountCost, string(log.UsageCompleteness), string(log.ProbeOutcome), nil, log.CreatedAt))
...
func TestAccountProbeCostRepositoryAppendDuplicateConflictIsTyped(t *testing.T) {
	...
	mock.ExpectQuery("SELECT probe_run_id").WithArgs(log.ProbeRunID).WillReturnRows(sqlmock.NewRows([]string{
		"probe_run_id", "account_id", "group_id", "probe_kind", "model", "input_tokens", "output_tokens", "cache_creation_tokens", "cache_read_tokens", "account_cost", "usage_completeness", "probe_outcome", "error_code", "created_at",
	}).AddRow(log.ProbeRunID, 99, *log.GroupID, string(log.ProbeKind), log.Model, log.InputTokens, log.OutputTokens, log.CacheCreationTokens, log.CacheReadTokens, *log.AccountCost, string(log.UsageCompleteness), string(log.ProbeOutcome), nil, log.CreatedAt))
...
func TestAccountProbeCostRepositoryReadWindowAggregatesHalfOpenWindowAndNullableCost(t *testing.T) {
	...
	mock.ExpectQuery("SELECT group_id, account_id").WithArgs(from, to).WillReturnRows(sqlmock.NewRows([]string{
...
func TestAccountProbeCostRepositoryReadWindowPropagatesQueryError(t *testing.T) {
	...
	from, to := time.Now().Add(-time.Hour), time.Now()
	mock.ExpectQuery("SELECT group_id, account_id").WithArgs(from, to).WillReturnError(queryErr)
