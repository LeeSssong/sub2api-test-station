...
func TestResolveAccountStatsCostResolution_FixedImageTiersIgnoreAccountMultiplier(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		log := &UsageLog{BillingMode: func() *string { v := string(BillingModeImage); return &v }(), ImageSize: &tc.tier, ImageCount: tc.count, ServiceTier: &serviceTier}
		applyAccountStatsCost(context.Background(), log, cs, nil, 1, 10, "gpt-image-2", "gpt-image-2", UsageTokens{}, 9, time.Time{}, 1.75)
...
func TestApplyAccountStatsCost_UsesUsageLogServiceTier(t *testing.T) {
	...
	applyAccountStatsCost(
		...
		UsageTokens{InputTokens: 100, OutputTokens: 50}, 999, time.Time{}, 1,
	)
