...
**T105 OpenAI OAuth 429 账号级原生限流恢复（2026-08-31）：** 状态 `INTEGRATING`。用户已明确授权插队并优先部署本窗口任务。范围仅账号级、不做模型级；保留当前有限同账号重试，重试窗口耗尽或进入跨账号 failover 时复用 Sub 原生 `SetRateLimited`/账号限流状态；上游提供可靠 reset 时尊重该时间，无可靠 reset 时固定 5 分钟。请求级 `failedAccountIDs` 只用于当前请求防重复，不形成持久化的第二套控制；管理员现有 clear-rate-limit/recover-state 继续作为手动恢复入口。候选工作区 `/Users/gongtengxinwen/Documents/sub2api搭建/.worktrees/t105-openai-429-account-cooldown`，分支 `codex/t105-openai-429-account-cooldown`，候选提交 `23f39837de06d4f5fbc3c59d15e8247701e24315`（实现 `7afeccae221e47bc9e6ebc537e02ba7cffd5136b`，tree `1ff0f56bf78fdf68e6477a8b287b2f0fa0bf0321`）。候选验证完成；现进入根整合，待合并后最小回归、推送、发布预检和线上验证。
