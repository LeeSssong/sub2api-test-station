...
**官方 Sub2API v0.1.177 最高优先级更新（2026-08-16）：** 状态：进行中（`IMPLEMENTING`，停机维护已获授权）。用户已于 2026-08-16 明确“允许停机部署”；发布链此前返回 `downtime_required=true`、`reason_code=migration_set_changed`、预计不可用约 300 秒，候选未启动、未切换槽位。活动迁移集为 `d3fe99bba69b0cf0cca8a7f5ec45499921f3496f58dd74c3a671d90a653589b5`，官方候选迁移集为 `ef1213846cba597cbc5cd64238558a3c392585df3568acb321f3227776e88bc5`；现创建独立 T09-R1 GPT-5.6 Sol/medium 顶层维护任务，只增加该精确迁移转换的宿主放行和最小合同复审，再由唯一发布总控合并、推送并执行既有维护发布链。原候选与停机证据 `/Users/gongtengxinwen/.codex/release-evidence/sub2api/2026-08-16-main-716f6876f-official-v0177-downtime-gate.json` 保留。T12、T13 与 T14 继续暂停在设计/排队阶段。
