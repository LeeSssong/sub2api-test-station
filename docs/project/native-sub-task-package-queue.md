...
- **T47-R2 Monitor V2 紧凑布局与信息层级优化（2026-08-21）**：状态 `DEPLOYING`。用户已确认实施“紧凑服务行”方案：放大标题与核心指标，统一服务信息/时间线网格和垂直基线，移除 Tooltip 常驻占位行并改为悬浮定位，时间标签贴近柱状图，收紧卡片间距；移动端继续上下堆叠且仅时间线内部横向滚动。范围仅 `MonitorV2GroupCard.vue`、`MonitorV2Timeline.vue` 及直接相关前端测试；保持 operational/unavailable/no-data 状态契约、API、数据源、迁移、配置和调度语义不变。候选已合入并推送根 `main@dcedd7eac`；直接相关测试 10/10、typecheck、build、diff-check 均通过，evidence `/Users/gongtengxinwen/.codex/release-evidence/sub2api/2026-08-21-main-525b35d3a-t47-r2.json`。预加载蓝绿发布在 47.5MB 制品传输阶段超过 600 秒门限退出，生产未切换，当前活动源仍为 `11d871832`；失败候选与证据保留，待同一候选继续发布。
