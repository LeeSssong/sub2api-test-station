...
**OAuth 图片编辑上传 MIME 兼容热修登记（2026-08-16）：** 状态：进行中（`REVIEWING`，不占用 `INTEGRATING`、`DEPLOYING` 或 `VERIFYING` 发布车道）。T11-R1 已完成生产验收与收口，用户可见顶层任务 `01a00892-2294-7083-aa01-7aa6f94d1dc4` 已使用 GPT-5.6 Sol/应用默认 medium 从最新 `main@44095897d1bba3302c877431ba9bb5b6e356ab46` 创建独立 worktree `/Users/gongtengxinwen/.codex/worktrees/d883/sub2api搭建`。规格 `3a1d97d2a` 采用 OAuth Responses 私有 helper，计划 `37cdc9bce` 收敛为单一 TDD 任务，均已由发布总控批准；fresh implementer 已形成代码提交 `4adfe246b` 与证据提交 `ba1350198`，工作树干净，当前进行任务级独立复审与 fresh whole-branch 终审。已确认 OAuth 账号 `/v1/images/edits` 会把 multipart 图片转换为 Data URL 并保留 `application/octet-stream`，上游因此返回 400 `unsupported MIME type`；API-key 账号链路正常，事故不是图片尺寸问题。已知 `user_id=34`、API key `50`、生图 `group_id=19` 的请求命中；生产临时规避已于 10:02 将 OAuth 账号 `222/223` 移出 `group_id=19`。用户邮箱仅保留在授权交接上下文，不写入仓库总账。最小范围仅在 `openai_images_responses.go` 对空 MIME 或 `application/octet-stream` 进行字节识别，只允许识别为 `image/*`，无法识别则拒绝。不改错误码/文案、中文提示、`ErrorPassthroughRule`、客户端表现或其他上传链路；仅做相关单测、后端必要构建、diff/范围、发布预检和上线后 `/images/edits` 定向验收，禁止 GitHub Actions 或直接覆盖生产。功能窗口只交付 `READY_FOR_ROOT_REVIEW` 候选，根发布总控负责后续合并、推送、部署与线上验证。
