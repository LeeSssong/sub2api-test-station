...
**S1-R2 确定性故障原生隔离编排候选（2026-08-17）：** 用户可见顶层任务 `01a00da8-ed25-7b72-b9d9-cdcee5fa75c1` 在独立 worktree `/Users/gongtengxinwen/.codex/worktrees/6195/sub2api搭建`、分支 `codex/s1-r2-native-deterministic-failure-isolation` 从本地 `main@a00fdb186b9598c0ab0ca747d9dff1a5cea04ae2` 完成原生能力盘点、完整 brainstorming、方案比较、正式规格/自审/批准、writing-plans、TDD 实现、直接相关验证和 handoff；最终根审候选为 `9f4b7d9a4aace8377b8b51fdfa796689ed3ec313`，worktree 干净。实现把余额不足统一投影为原生 90 分钟账号冷却（配置 60–120，越界回退 90），把明确模型不支持投影为账号 + canonical model 的原生 `model_rate_limits/probe_required`，保持 API Key 明确凭据失效使用原生 error/不可调度，并把未收到成功终态的 SSE 接入既有账号模型 transient cooldown/failover，同时保留输出后禁止重放；泛化 403、网络失败、空/截断模型目录不硬隔离。直接相关 service、unit OAuth/model recovery、config、受影响包 compile-only、server build、gofmt 与全候选 `git diff --check main...HEAD` 均通过；无迁移、未使用 225/226、无 GitHub Actions 变化。交接为 `docs/handoffs/2026-08-17-s1-r2-native-deterministic-failure-isolation-handoff.md`。用户部署冻结继续有效，本候选受保护且不得合并、push、预检、部署或访问生产；`downtime_required=unverified`，S2/S3 不得启动。**状态：准备完成（READY_FOR_ROOT_REVIEW；未合并、未推送、未部署、未线上验证）。**

**T16 经营页真实结果与视觉层级重设计启动（2026-08-17）：** 用户已明确批准视觉方向，本项作为 T12 已生产收口后的新独立任务包，不回写或重开 T12。用户可见顶层任务 `01a00dd1-d476-79b3-bdef-5e4d06103f50` 已在 `/Users/gongtengxinwen/.codex/worktrees/026c/sub2api搭建` 建立分支 `codex/t16-profitability-visual-hierarchy`，精确基线为本地 `main@3d0f5b374aa9f4fe6f57cb2810f4984660997904`。产品合同为：默认打开“全部真实结果”；账号明细只保留运营消耗、业务消耗、业务营收、总消耗、净利润五项；管理员免费使用的站内扣费虽为 0，真实上游成本仍计入内部运营消耗和总消耗，净利润可为负但必须表述为内部补贴/内部运营而非客户亏损。事实源继续唯一复用 Sub 原生 `usage_logs`，不改变 `cost`/`user_cost` 基础公式或建立第二账务源；禁止以 `user_cost=0` 猜管理员身份，正式规格必须核查 `user_id/role` 的历史角色变化风险，若需不可变 actor type 则单独论证最小数据契约且不回填猜测。视觉语义为业务营收蓝、真实上游消耗琥珀、内部运营紫、净利润绿、真实亏损/内部补贴警示红；摘要突出业务营收/总消耗/净利润/对外毛利率并单列已包含于总消耗的内部运营消耗，明细使用紧凑表格，390px 摘要两列且整页无横向溢出。下一步先完成原生能力与身份口径盘点、完整 brainstorming、正式规格/自审/批准，再 planning、实现和直接相关验证；部署冻结继续有效，最多停在 `READY_FOR_ROOT_REVIEW`。**状态：进行中（DESIGNING；未合并、未推送、未部署）。**
