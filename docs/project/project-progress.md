...
**本轮 T04 账号监控移除外部控制面状态（2026-08-12）：** 状态：进行中（brainstorming 澄清阶段，尚未写规格书或实施）。T03 已完成推送、生产验收、可恢复归档和候选清理；T04 已从当时最新干净 `main@eaa73610238f1effc253dc84a98eae4965390525` 创建用户可见的独立顶层 Codex 任务 `019ff559-b4d6-7411-81c7-d0e631584d70`，独立 worktree 为 `/Users/gongtengxinwen/.codex/worktrees/72c7/sub2api搭建`，目标分支为 `codex/t04-account-monitor-native-only`。该顶层任务已完整阅读事实源并完成首轮只读探索：账号监控当前先请求原生 `/admin/accounts/monitor`，再请求外部 `/xingqiao/externalization/pages/monitor`，外部决策允许时还请求 `/xingqiao/accounts/monitor`，外部结果可替换原生数据；失败时保留原生卡片但显示“控制面暂时不可用 / 完整性 unknown”并延长加载链。当前已按完整 brainstorming 门禁一次只向用户提出一个问题：T04 是否要求账号监控在所有场景对 `/xingqiao/**` 零请求、始终以 `/admin/accounts/monitor` 为唯一页面数据源，同时保留被 T05/T06 共享的控制面客户端和状态组件供后续任务处理。未经用户回答、分段设计批准和书面规格书明确批准，不得调用 writing-plans、实施、派生实现代理、合并、推送或部署。范围不包含用量页、利润页、全局评分或“暂不建议入组”。
