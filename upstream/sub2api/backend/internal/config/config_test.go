...
func TestLoadHTTPIngressSafetyDefaults(t *testing.T) {
	...
	require.False(t, cfg.Security.SessionBindingAllowed)
...
func TestLoadSessionBindingAllowedFromEnvironment(t *testing.T) {
	resetViperWithJWTSecret(t)
	t.Setenv("SECURITY_SESSION_BINDING_ALLOWED", "true")

	cfg, err := Load()

	require.NoError(t, err)
	require.True(t, cfg.Security.SessionBindingAllowed)
}
