...
func registerOperationsRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	operations := admin.Group("/operations")
	{
		...
		operations.GET("/self-purchased-profitability", h.Admin.Dashboard.GetSelfPurchasedProfitability)
...
func registerOpsRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	ops := admin.Group("/ops")
	{
		...
		ops.GET("/stream-diagnostics", h.Admin.Ops.GetStreamDiagnostic)
...
func registerUserManagementRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	users := admin.Group("/users")
	{
		...
		users.GET("/:id/quota-summary", h.Admin.User.GetQuotaSummary)
		users.POST("/:id/quota-ledger", h.Admin.User.CreateQuotaLedgerEntry)
		users.GET("/:id/quota-ledger", h.Admin.User.GetQuotaLedger)
...
func registerAccountRoutes(admin *gin.RouterGroup, h *handler.Handlers, stepUpAuth middleware.StepUpAuthMiddleware) {
	accounts := admin.Group("/accounts")
	{
		...
		accounts.POST("/:id/external-command/v1", h.Admin.Account.UpdateExternalCommandV1)
...
func registerAccountMonitorRoutes(admin *gin.RouterGroup, h *handler.Handlers, stepUpAuth middleware.StepUpAuthMiddleware) {
	...
	monitors := admin.Group("/account-monitors")
	{
		...
		monitors.GET("/:account_id/models", h.Admin.AccountMonitor.AccountModelDetectionModels)
		monitors.PUT("/:account_id/models", h.Admin.AccountMonitor.SaveAccountModelDetectionModels)
		monitors.POST("/:account_id/detection", h.Admin.AccountMonitor.EnqueueAccountModelDetection)
		monitors.GET("/:account_id/detection", h.Admin.AccountMonitor.AccountModelDetectionHistory)
