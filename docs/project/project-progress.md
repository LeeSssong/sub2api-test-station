...
**全局无停机发布授权门禁（2026-08-17）：** 用户明确指令：发布预检返回 `downtime_required=false` 时不需要再次取得人工授权，唯一发布总控应直接继续既有本地/宿主蓝绿发布与线上验证；只有返回 `downtime_required=true` 时，才必须在任何停服、迁移、重启或切换前暂停并请求明确授权。此前“先不部署”的全局冻结现已解除；T15、T16 和历史冻结/受保护 worktree 的独立保护状态不因此解除。S2 因根治理提交领先候选进入 `REFRESH_REQUIRED`，刷新并通过直接相关验证后继续唯一发布车道。**状态：进行中。**
...
**S2 共享健康、故障域与抗故障重试（2026-08-17）：** 独立 worktree `/Users/gongtengxinwen/Documents/sub2api搭建/.worktrees/s2-shared-health-failure-domain`、分支 `codex/s2-shared-health-failure-domain` 已从根审计退回状态完成 TDD 修复并刷新 `main@b35d3f100`；最终候选为 `5464faa0c`，已验证运行时 tip 为 `e3d905412`。闭环内容包括普通成功向 Redis 写入独立幂等 success event 并清零 shared streak/cooldown、Redis 不可用且无 30 秒内可信快照时最多保留一次剩余跨账号切换、按 provider channel/quota pool 优先未失败域且保持最佳 bucket 内顺序、5xx/连接退避稳定 0–20% jitter、五类稳定预算耗尽 reason，以及 Chat Completions shared failure event/attempt metadata。repository/service/handler/config 聚焦测试、server compile-only/build、gofmt、diff-check 均通过；真实 Redis integration-tag 用例仍被既有无关 `stringPtr` 重名编译冲突阻断，未越界修复。候选 worktree 干净；因根 `main` 新增治理提交，当前先进入刷新门禁，整合最新根治理基线并重跑直接相关验证后恢复候选状态；尚未合并根 `main`、未推送 S2 功能代码、未执行发布预检、未部署，S3 继续等待 S2 生产验收。**状态：进行中（REFRESH_REQUIRED）。**

**S3 自适应选择、粘性逃逸与调度体验观测（2026-08-17）：** 历史拆分规格已登记为后续 backlog；依赖 S2 完成生产验收，不创建 worktree、不实施、不部署。**状态：排队（BACKLOG）。**
