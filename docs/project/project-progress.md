...
**本轮 T03 上游扣费与利润始终有值（2026-08-12）：** 状态：进行中。候选 `codex/t03-upstream-cost-profit-values@fe9d04c9ccd8e40faf1e58ff137004f57964392b` 基于 `main@0b6ef4ef0faec1b84de6f7133b5c99c4ae6e405d` 已完成规格、TDD 实现、定向回归、`go vet`、server build、前端 confirmed-zero 契约验证与独立复审，审查结论 P0-P2 无发现；工作区干净，状态为 `READY_FOR_ROOT_REVIEW`。实现仅在 Sub/New 原生账单记录被精确请求 ID 命中后，将 `null`、缺失或空字符串实际扣费归一为 `0`，利润继续按本站实际扣费减上游实际扣费计算；非空非法值、未命中、查询失败及 New `quota_per_unit` 缺失/非法仍保持不可用，普通用户接口与 DTO 不变。无迁移、无配置、无前端实现、无 relay-ops 或 GitHub Actions 变化，预期 `downtime_required=false`；尚未合并、推送、部署或线上验证，不得标记完成。验证报告见 `docs/superpowers/reports/2026-08-12-t03-upstream-cost-profit-values.md`。
