...
**本轮官方 `v0.1.175` 快速升级实施（2026-08-13）：** 状态：进行中。用户已批准 `docs/superpowers/specs/2026-08-12-official-v0175-fast-merge-design.md`，并授权立即按“提取定制行为 -> 升级官方源码 -> 语义回接”实施。执行范围严格限定为已发现的 9 个冲突文件、版本/来源身份、定向测试、迁移分类和蓝绿发布门禁；不实施 T03-R1、不做架构外置、不运行无关专项。将从最新干净 `main` 创建 `.worktrees/official-v0175-fast-merge` / `codex/official-v0175-fast-merge`，T03-R1 worktree 保持原样，待本次生产升级验证后接入新基线。
