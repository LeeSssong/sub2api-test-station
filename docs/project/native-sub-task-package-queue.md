...
- 当前状态：`DESIGNING`。用户已明确批准最终业务设计（2026-08-18）；排在 T22 之后登记，不打断 T21 的生产验证、不解冻 T16，也不进入合并/部署车道，直到前序任务按单车道完成或由根总控明确处置。
- 依赖与窗口：T21 当前 `VERIFYING`，T22 `BACKLOG`；T23 仅占用第二个设计/实现窗口。必须从届时最新干净 `main` 创建独立用户可见顶层任务、独立 worktree、正式规格和计划；不得从现有自购账号历史 worktree 或其他候选派生。
