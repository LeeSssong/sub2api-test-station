...
- **T32 账号评分回归修复**：状态 `DESIGNING`。评分、当前状态与排名只允许使用 Sub 原生主动探测证据；暂停账号仍可参与评分和排名；只有主动探测返回 4xx/5xx 且调度关闭时停止探测并退出排名；调度关闭但主动探测成功时继续探测、评分和排名。仅改账号监控原生投影、卡片/页面和直接测试，不改公式、权重、计费、调度或事实源。独立用户可见任务、独立 worktree、单独发布。
- **T33 经营页账号卡片与搜索**：状态 `DESIGNING`。USD/CNY 经营视图均改为每账号独立卡片并支持搜索；本站展示按 CNY 经营口径，底层 USD 额度按 1 USD = 1 CNY 展示关系处理，不新增汇率或第二账务源。仅改经营页前端、已有原生字段投影和直接测试，不改账务公式、采购保存或生产数据。独立用户可见任务、独立 worktree、单独发布。
- **T34 渠道状态原生探测重构**：状态 `BACKLOG`，依赖 T32/T33 发布后的最新 `main`。Monitor V2 完全复用 Sub 原生账号主动探测；可用性、首字速度、平均耗时和时间线均来自主动探测，不使用真实请求；分组内至少一个可调度账号探测成功即为时间桶可用。分组倍率紧邻分组名称，删除“旗舰”，不再额外放大页面。独立用户可见任务、独立 worktree、单独发布。
