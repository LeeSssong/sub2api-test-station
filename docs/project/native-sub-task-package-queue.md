...
**T132 账号监控检测判定与证据可见性修复（2026-09-05）：** 状态 `BLOCKED`。候选提交 `0b53b67b8`、`f6ad3d34f`、`9ea86eb0d` 已无冲突合入根 `main@822e85297`，追加修复 `933986be4` 已推送。已恢复 v4.1.1 官方预设及正式模型判定；账号卡片拆分 Juice/模型两行结果并使用固定性能柱宽；历史详情展示 Juice 申报冲突证据，并为升级检测保存上一轮具体可疑证据白名单快照。追加审查修复将证据不足作为合法 `insufficient` 终态而非请求失败，并将主动兜底探测最大周期钳制为 5 分钟；失败探测仍不进入成功率分子、分母或延迟指标。旧记录不回填，无凭据、提示词或完整响应落库。直接相关 adapter 9/9、sidecar/runner、前端 69/69、typecheck 和 server build 已通过；宽泛 handler 运行命中两个既有监控投影旧断言失败，未纳入本次直接门禁。生产预检在任何切换前返回 `downtime_required=true`、`reason_code=migration_set_changed`、预计不可用 300 秒，现有活动槽保持不变；等待用户明确停机授权。独立验收站按授权未同步。
