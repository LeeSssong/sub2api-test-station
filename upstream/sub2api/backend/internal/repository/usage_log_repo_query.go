...
func (r *usageLogRepository) UpdateActualResponseModelByRequestID(ctx context.Context, requestID, model string) error {
	...
	if len([]rune(model)) > 100 {
		model = string([]rune(model)[:100])
	}
	if requestID == "" || model == "" {
		return nil
	}
	_, err := r.sql.ExecContext(ctx, "UPDATE usage_logs SET actual_response_model = $1 WHERE request_id = $2", model, requestID)
	return err
}
...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		actualResponseModel       sql.NullString
	...
	if actualResponseModel.Valid {
		value := actualResponseModel.String
		log.ActualResponseModel = &value
	}
