...
**T03-R1 Task 8 集成验证（2026-08-13）：** 状态：进行中（候选 `32f8ec12572ad9d2e50eab054788d4ec0bf05454`，tree `ab19d7807763e1fcd03d0d0cd3f8f49d335f9fbb`；尚未合并、推送、部署或线上验证）。已执行 Task 8 负面门禁、冻结旧候选 `ce5691527` 的只读 RED 证据、后端/前端集成矩阵、迁移/hash/config/diff 检查，并生成三份 Task 8 证据草稿。迁移 222 SHA-256 为 `47f786d6b2b020d0211a17d4ccd2bc6bb3774a315f483fdc0ac45657c9ee738e`，相对 merge base `19492c57da24270eb2b3e9b5d9727c2865aebb9e` 仅新增该 SQL 和测试文件，无既有迁移修改、无配置变化。通过：后端 migration/repository/service/handler 定向测试、`go vet`、`make build`、前端 6 文件 53 项 Vitest、typecheck、production build、`git diff --check`。已知阻塞：`internal/handler/admin/usage_handler.go` 仍存在 `SubUpstreamCostService.GetByUsageID` nil-service fallback，精确 brief read-time HTTP guard 因此未闭合；不在 Task 8 文档范围内修复，交由独立全分支 reviewer/根任务处置。`downtime_required=unverified until root preflight`；`stash@{0}` 保留。详见三份 Task 8 报告。
