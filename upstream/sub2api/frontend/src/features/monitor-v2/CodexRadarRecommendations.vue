<template>
  <section data-test="codexradar-panel" class="mt-7 min-w-0 overflow-hidden rounded-2xl border border-slate-700 bg-slate-950 p-5 text-slate-100 shadow-lg sm:p-6 xl:p-7" aria-labelledby="codexradar-title">
    <header class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <div class="flex items-center gap-2">
          <span class="inline-flex h-9 w-9 items-center justify-center rounded-full border border-slate-600 bg-slate-900 text-base">◎</span>
          <h2 id="codexradar-title" class="text-xl font-black tracking-tight sm:text-2xl">站长推荐</h2>
        ...
        <p class="mt-1.5 text-sm text-slate-400">根据 CodexRadar 公开实测数据，按任务场景自动推荐。</p>
      </div>
    ...
    <div v-else-if="insights" class="mt-5 grid min-w-0 grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
      ...
      >
        <div class="border-b border-current/25 px-4 py-3.5">
          ...
          <p class="mt-1.5 line-clamp-3 break-words text-xs leading-5 text-slate-400" :title="recommendation.rule">{{ recommendation.rule }}</p>
        </div>
        <div class="divide-y divide-slate-700/80">
          <div v-for="item in recommendation.items" :key="`${item.model}-${item.effort}`" class="grid grid-cols-[minmax(0,1fr)_auto] gap-3 px-4 py-3.5">
            <div class="min-w-0">
              <p class="truncate text-sm font-bold text-slate-100">{{ modelLabel(item.model) }} {{ item.effort }}</p>
              <p class="mt-1 text-xs text-slate-400">IQ {{ Math.round(item.iq) }} · {{ Math.round(item.average_duration_minutes) }} 分钟</p>
            </div>
            <p class="self-center text-base font-black text-emerald-400">${{ item.average_cost_usd.toFixed(2) }}</p>
          </div>
        </div>
      </article>
    </div>
