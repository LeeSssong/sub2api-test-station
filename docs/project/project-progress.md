...
**本轮任务登记（2026-08-10）：** 排查并修复管理员将 `GPT-Plus` 分组 RPM 更新为 `0（不限制）` 时出现“更新分组失败”，并分析中转站对上游 overloaded、平台/本站并发限制、泛化 upstream stream failure 三类错误的可控措施；状态：进行中。目标仅将生产 `groups.id=6` 的 `rpm_limit` 改为 `0`，保留倍率、账号绑定、调度强度、用户级限额、路由和计费不变；先通过管理请求、应用日志、数据库和缓存边界确认失败根因，若需代码修复则按测试先行、并入 `main`、专项回归、推送、蓝绿部署和线上验证链执行。当前工作区为根目录 `main`；已盘点全部非 `main` worktree，`.worktrees/account-monitor-group-recommendation` 为尚未部署完成的活动候选，`.worktrees/fix-official-update-stuck` 与 `.worktrees/gpt-group-baseline-apply` 保留其现有活动/未提交内容，本轮均不修改、不清理。
