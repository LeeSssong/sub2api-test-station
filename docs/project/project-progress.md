...
**T05 用量页移除外部控制面状态重新启动（2026-08-14）：** 状态：进行中（`DESIGNING`）。账号监控卡片已完成合并、推送、无停机蓝绿部署、登录态线上验收和候选归档，串行门禁已解除。唯一发布总控以最新干净 `main@6b606c731faed504f9368730a910489d08ee8ab3` 为基线创建新的用户可见顶层任务和独立 worktree；旧 detached `a71c675b1` 仅作启动审计，不继续使用。T05 范围严格限定为原生管理员用量页删除外部控制面 banner、完整性/unknown 状态及相关控制面调用，同时保留原生流水、错误请求和管理员详情；不修改成本数值规则、利润页、账号监控、后端账务模型或生产。当前只允许完整 brainstorming、方案比较、分段设计批准、正式规格书及用户明确批准；未经批准不得 writing-plans、实施、合并或部署。
