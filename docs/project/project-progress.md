...
**T12 经营页本站探测花费与排序/美元字段优化（2026-08-17）：** 候选 `codex/t12-native-probe-cost-cards@c7587599a7947e6103a915fdfc9f030d343bb198` 已完成功能与直接相关测试，曾无冲突合并至根 `main@7ded6b219d0e87334c9db63c607ab27044c063ff`，但在发布链启动前因 P0 会话事故停止，生产未构建、未迁移、未切换。根总控用 `408e7a1a3` 与 `9d9e2b758` 可追溯 revert 将 T12 从 `main` 移出并保全候选；P0 现已 `DONE`，唯一发布总控明确解除 T12 冻结。当前 worktree `/Users/gongtengxinwen/Documents/sub2api搭建/.worktrees/t12-native-probe-cost-cards` 干净；因原候选已是 `main` 历史祖先，本轮将在该 worktree 快进最新 `main` 后反向撤销两笔 T12 撤回提交，重新生成候选差异并保留全部 P0 修复，再执行直接相关验证。旧 docs-only T12、v0.1.175 和 S1 分支继续冻结，不合并、不清理。**状态：进行中（REFRESH_REQUIRED）。**
