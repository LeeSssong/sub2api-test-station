...
**本轮 T04 账号监控移除外部控制面状态（2026-08-12）：** 状态：进行中（实现候选已完成，待独立任务复审、全分支终审、根任务授权合并和发布；不得标记已完成）。独立工作区为 `/Users/gongtengxinwen/.codex/worktrees/72c7/sub2api搭建`，分支 `codex/t04-account-monitor-native-only`，基线 `eaa73610238f1effc253dc84a98eae4965390525`。账号监控现已在首次加载、时间窗切换、刷新和代表性单账号探测后重载中只调用 `/admin/accounts/monitor`，且页面不再请求 `/xingqiao/**`；页面控制面投影、状态及状态条已删除，未引入替代常驻状态，也未改动卡片、布局或写操作。原生首次加载失败显示独立的可重试空态；保留已有快照时继续显示范围错误和旧卡片。专项 `pnpm exec vitest run src/views/admin/__tests__/AccountMonitorView.spec.ts` 通过（28/28）；`pnpm typecheck`、`pnpm build` 与 `git diff --check` 通过，页面的控制面符号与 `/xingqiao/` 扫描为零匹配。共享控制面文件未改动；无迁移、无配置变更、无需停机。正式设计见 `docs/superpowers/specs/2026-08-12-t04-account-monitor-native-only-design.md`，实施计划见 `docs/superpowers/plans/2026-08-12-t04-account-monitor-native-only.md`；候选提交、复审结论和发布证据将继续登记。最终交付仍需根任务发送包含目标 `main` SHA 的 `AUTHORIZE_MERGE_TO_MAIN` 后完成合并、推送、既有本地/宿主链部署和线上验证；不得使用 GitHub Actions。
