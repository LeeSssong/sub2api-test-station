...
**本轮生产逐笔成本二次收口（2026-08-10）：** 状态：进行中（已合并并通过发布前门禁，待推送、部署和线上自然流水验证）。生产 `main@9e24bfde27a9e1a4751d3a30158e24067b900670`、版本 `0.1.173` 的蓝绿发布和版本身份验证已成功，但逐笔成本验收继续暴露两条独立事实：一是 New API 的逐笔日志使用其即时中转生成并通过 `X-Oneapi-Request-Id` 返回的 `request_id`，本站当前只保存可能属于下一级供应商的 `x-request-id`，导致精确 ID 匹配失败；二是账号 `#42 Plus-WAWAZZ` 虽返回 `sub2api.key_billing` 倍率声明，但其 `/v1/usage/records` 与 New API 日志/状态接口均为 HTTP 404，本站 `account_cost` 只是配置/规则计算快照，禁止冒充上游实际扣费。修复已合并至 `main@cfd3ea772e87c0372ab3df4b08fd6bb952df96d9`：恢复 resolved account-cost 的三条 account-quota 消费链及两个 usage 传播入口，并让成功的 OpenAI Responses/Chat/Messages/Passthrough/Images/Embeddings/Alpha Search usage 路径按 `X-Oneapi-Request-Id` → `x-request-id` → `xai-request-id` 保存立即上游请求 ID；精确匹配策略未放宽。聚焦回归、New API 成本匹配、后端 `go vet`/build、蓝绿发布控制器、发布元数据和发布流程测试均已通过。下一步直接推送、使用预加载宿主蓝绿链部署，并等待一条自然 New API 流水完成线上精确成本/利润验证；账号 #42 继续诚实显示不可用。两个用户明确保护 worktree 继续不修改、不合并、不清理，不使用 GitHub Actions。
