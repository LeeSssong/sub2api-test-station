...
- 全局审计（2026-08-18）：根 `main`/`origin/main` 当前为 `21522069cd70b2bcf1686f228098b205c7966c27`，队列登记任务均为 `DONE`，没有任务占用 `INTEGRATING`、`DEPLOYING` 或 `VERIFYING` 车道。`codex/t21-model-detector-sidecar-closure`、`codex/t17-usage-detail-effective-cost`、`codex/official-v0175-fast-merge` 是已部署成果的旧分叉；`codex/account-probe-cost-design` 是历史设计证据；`codex/upstream-resilience-hardening` 为 dirty 的冻结/保护 worktree。上述五个 worktree 均保留只读证据，不合并、不恢复、不清理。其余历史发布 worktree 干净且无相对 `main` 的领先提交；根目录既有未跟踪资料保留。若无新任务登记，当前队列进入发布收口待命。
