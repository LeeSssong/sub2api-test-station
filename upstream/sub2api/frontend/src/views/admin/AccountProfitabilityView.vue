<template>
  <AppLayout>
    <main class="mx-auto max-w-[1400px] space-y-5 p-5" data-test="account-profitability-page">
      ...
      <section class="overflow-x-auto rounded-xl border bg-white"><table class="min-w-full text-sm"><tbody><tr v-for="row in report.accounts" :key="row.id"><td>{{ row.name }}</td><td><template v-if="activeRange === 'today'"><input :data-test="`account-edit-revenue-${row.id}`" @change="saveOverride(row.id, 'revenue_cny', $event)" /><input :data-test="`account-edit-cost-${row.id}`" @change="saveOverride(row.id, 'cost_cny', $event)" /><input v-if="row.type === 'oauth'" :data-test="`account-edit-oauth-cost-${row.id}`" @change="saveOAuthCost(row.id, $event)" /></template></td><td><button v-if="row.exception_count" :data-test="`account-exceptions-${row.id}`" @click="jump(row.id)">{{ row.exception_count }}</button></td></tr></tbody></table></section>
    </main>
  </AppLayout>
</template>
