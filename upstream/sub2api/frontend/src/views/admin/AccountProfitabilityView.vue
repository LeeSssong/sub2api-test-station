  ...
  <AppLayout>
    <main class="mx-auto max-w-[1400px] space-y-5 p-5" data-test="account-profitability-page">
      ...
      <section v-if="selectedGroup" class="space-y-3" :data-test="`group-summary-${selectedGroup.id}`">
        ...
        <p v-else-if="!selectedGroup.complete" class="text-sm text-amber-700 dark:text-amber-300" data-test="incomplete-group-notice">{{ t('admin.accountProfitability.scope.incomplete') }}</p>
      </section>
...
async function load() { loading.value = true; try { report.value = await adminAPI.accountFinancial.getReport({ range: activeRange.value }); if (activeScope.value.kind === 'group' && !selectedGroup.value) activeScope.value = { kind: 'all' } } finally { loading.value = false } }
