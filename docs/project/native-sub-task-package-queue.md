...
**T105 OpenAI OAuth 429 账号级原生限流恢复（2026-08-31）：** 状态 `DEPLOYING`。用户已明确授权插队并优先部署本窗口任务，且已授权约 300 秒停机维护发布。范围仅账号级、不做模型级；保留当前有限同账号重试，重试窗口耗尽或进入跨账号 failover 时复用 Sub 原生 `SetRateLimited`/账号限流状态；上游提供可靠 reset 时尊重该时间，无可靠 reset 时固定 5 分钟。请求级 `failedAccountIDs` 只用于当前请求防重复，不形成持久化的第二套控制；管理员现有 clear-rate-limit/recover-state 继续作为手动恢复入口。根 `main` 已包含一次有界分组恢复轮次：分组因瞬时 429 候选耗尽时，刷新权威分组账号投影，清除 T105 短 cooldown 与请求级排除，保留原生 7 天 quota 与长期账号状态。当前以根 `main` 维护发布，停机起点迁移哈希为 `88a0ff14d25215f07d05f307a521f03114005ace43078b80f3bd2702e0f08d03`。
