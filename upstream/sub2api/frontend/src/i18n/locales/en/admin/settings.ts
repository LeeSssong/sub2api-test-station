    ...
    settings: {
      ...
      features: {
        channelMonitor: {
          ...
          modeNativeProbe: 'Native active probes',
          modeHybrid: 'Hybrid performance',
          ...
          modeNativeProbeHint: 'Native page: uses the native active-probe monitor without changing probe semantics.',
          modeHybridHint: 'Hybrid mode: combines active probes and successful real requests into five-minute availability and P95 performance metrics.',
      ...
      openaiExperimentalScheduler: {
        ...
        topKLabel: 'TopK',
        topKHint: 'Number of highest-scoring candidates kept per request. Smaller values reduce probing cost but can starve lower-scoring accounts; fairness modes may add an overdue account. Range: positive integer (>= 1); blank inherits config/default.',
        priorityWeight: 'Priority',
        priorityWeightHint: 'Weight for account priority. Higher values prefer lower numeric priority values; 0 disables this factor. Range: finite non-negative number (>= 0; decimals allowed); blank inherits config/default.',
        loadWeight: 'Load',
        loadWeightHint: 'Weight for current concurrency load. Higher values avoid busy accounts and reduce single-account concentration. Range: finite non-negative number (>= 0; decimals allowed); blank inherits config/default.',
        queueWeight: 'Queue',
        queueWeightHint: 'Weight for waiting queue length. Higher values prefer accounts with shorter queues. Range: finite non-negative number (>= 0; decimals allowed); blank inherits config/default.',
        errorRateWeight: 'Error rate',
        errorRateWeightHint: 'Weight for recent error rate. Higher values avoid accounts with more failures. Range: finite non-negative number (>= 0; decimals allowed); blank inherits config/default.',
        ttftWeight: 'TTFT',
        ttftWeightHint: 'Weight for recent time-to-first-token. Higher values prefer faster accounts. Range: finite non-negative number (>= 0; decimals allowed); blank inherits config/default.',
        resetWeight: 'Reset window',
        resetWeightHint: 'Weight for accounts near a session-window reset. Higher values consume expiring capacity first; 0 disables it. Range: finite non-negative number (>= 0; decimals allowed); blank inherits config/default.',
        quotaHeadroomWeight: 'Quota headroom',
        quotaHeadroomWeightHint: 'Weight for remaining quota. Higher values prefer accounts with more headroom; 0 disables it. Range: finite non-negative number (>= 0; decimals allowed); blank inherits config/default.',
        upstreamCostWeight: 'Billing rate',
        upstreamCostWeightHint: 'Weight for upstream billing rate. Higher values prefer lower-rate accounts; 0 ignores rate. Range: finite non-negative number (>= 0; decimals allowed); blank inherits config/default.',
        previousResponseWeight: 'previous_response sticky',
        previousResponseWeightHint: 'Weight for previous_response_id affinity. Higher values reuse the original account and reduce context moves. Range: finite non-negative number (>= 0; decimals allowed); blank inherits config/default.',
        sessionStickyWeight: 'session_hash sticky',
        sessionStickyWeightHint: 'Weight for session_hash affinity. Higher values keep a session on the same account. Range: finite non-negative number (>= 0; decimals allowed); blank inherits config/default.',
        fairnessTitle: 'Fair scheduling',
        fairnessDescription: 'Controls candidate coverage and exploration of accounts that have not been used recently, reducing concentration on a few upstream accounts. Sticky requests keep their existing binding semantics.',
        poolMode: 'Candidate pool mode',
        poolModeHint: 'top_k uses only Top-K; all_eligible considers every eligible account; hybrid keeps Top-K and adds the oldest unused account when needed.',
        poolModeTopK: 'Top-K only',
        poolModeAll: 'All eligible accounts',
        poolModeHybrid: 'Hybrid fairness (recommended)',
        explorationRatio: 'Exploration ratio',
        explorationRatioHint: 'Only applies to hybrid. When no account crosses the starvation threshold, this percentage samples the oldest-unused account. 0=off, 100=every request.',
        starvationThreshold: 'Starvation threshold (seconds)',
        starvationThresholdHint: 'An account unused for this long becomes an exploration priority. 0 disables the threshold and leaves only the exploration ratio. Default 21600=6 hours.',
        ...
        presetPro: 'Pro',
        weightRange: 'Range: 0-10',
        topKRange: 'Range: 1-32',
        topKShortHint: 'Number of candidate accounts scored per request.',
        poolModeRange: 'Range: three fixed options',
        poolModeShortHint: 'Select which account pool participates in scoring.',
        explorationRange: 'Range: 0-100%',
        explorationShortHint: 'Controls exploration of infrequently used accounts.',
        starvationRange: 'Range: 0 or 300-86400 seconds',
        starvationShortHint: 'Protects accounts that have not been selected for a long time.',
        fairnessRange: 'Range: 0-10',
        fairnessShortHint: 'Controls how strongly fairness affects ranking.',
        priorityShortHint: 'Influence of the account base priority.',
        loadShortHint: 'Penalty strength for current load.',
        queueShortHint: 'Penalty strength for queued work.',
        errorRateShortHint: 'Penalty strength for historical errors.',
        ttftShortHint: 'Preference for time to first token.',
        resetShortHint: 'Preference for accounts near quota reset.',
        quotaHeadroomShortHint: 'Preference for available quota headroom.',
        upstreamCostShortHint: 'Influence of upstream cost on ranking.',
        previousResponseShortHint: 'Preference for continuing on the previous response account.',
        sessionStickyShortHint: 'Preference for keeping a session on the same account.',
        ...
        stepGroup: 'Step 1: Choose a group',
        stepMode: 'Step 2: Choose a policy mode',
        stepValues: 'Step 3: Configure parameters or choose a preset',
        customMode: 'Custom parameters',
        presetMode: 'Preset mode',
        customPresetName: 'Preset name',
        customPresetNamePlaceholder: 'For example: Peak-hour steady',
        saveAsPreset: 'Save as preset',
        presetNameRequired: 'Enter a preset name from 1 to 40 characters.',
        presetNameDuplicate: 'That preset name already exists.',
        presetReadOnlyHint: 'Preset values come from the server snapshot and are read-only.',
        customPresetTitle: 'Admin presets',
        renamePreset: 'Rename',
        deletePreset: 'Delete',
        deletePresetConfirm: 'Delete preset “{name}”?',
        presetReferenced: 'This preset is used by a group and cannot be deleted.',
        builtinPreset: 'Built-in',
        customPreset: 'Admin',
        selectGroupBeforePolicy: 'Choose a group before configuring its policy.',
        serviceGuard: 'Service continuity is always protected; health, capacity, cooldown, and concurrency checks remain active.',
        prioritySummary: 'Current priorities:',
        resetRecommended: 'Restore recommended settings',
        metricProfit: 'Profit',
        metricTTFT: 'First-token speed',
        metricLatency: 'Total latency',
        balanceTitle: 'Account balance',
        balanceLow: 'Spread high load only',
        balanceStandard: 'Standard coverage',
        balanceHigh: 'Prioritize coverage',
        peakTitle: 'Peak protection',
        peakStrict: 'Strict protection',
        peakStandard: 'Standard protection',
        peakOpen: 'Keep coverage',
        sessionTitle: 'Session continuity',
        sessionKeep: 'Keep when possible',
        sessionStandard: 'Standard',
        sessionSwitch: 'Switch faster',
        previewNormalTitle: 'Normal traffic',
        previewNormalBody: 'Choose a healthy account with available capacity using the current priorities.',
        previewPeakTitle: 'Peak traffic',
        previewPeakStrict: 'Avoid tight accounts first and pause pure coverage exploration.',
        previewPeakOpen: 'Keep necessary account coverage within safety limits.',
        previewSessionTitle: 'Session switch',
        previewSessionKeep: 'Reuse the current account when possible to reduce context movement.',
        previewSessionSwitch: 'Switch faster when load or health requires it.',
        previewSessionStandard: 'Balance health, capacity, and continuity together.'
      },
      usageRecords: {
        title: 'Usage Records',
      ...
      monitorPageRefresh: {
        title: 'Monitor page auto-refresh',
        description: 'Only administrators can set the refresh interval; users cannot adjust it.',
        off: 'Off',
