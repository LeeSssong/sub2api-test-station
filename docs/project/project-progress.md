...
**T08“暂不建议入组”轻提示（2026-08-15）：** 状态：进行中（`REFRESH_REQUIRED`）。已确认 T07 已完成生产发布并线上验收；已盘点非 `main` worktree：`codex/t07-global-score-weights` 为已部署候选的历史证据，`official-v0175-fast-merge` 与 `upstream-resilience-hardening` 均为未完成或含未提交内容的旧候选，不具备“已推送 + 已部署 + 已验证”完成证据，按冻结/保护规则保留，不合并、不修改、不清理。T08 用户可见顶层任务 `01a00306-a473-73f3-9240-addaf11b119d` 已完成完整 brainstorming、规格书与计划代审批准、TDD、逐任务独立复审和最终全分支终审；候选 `codex/t08-do-not-recommend-light-hint@0a514cb0fbc48be02dd4eae4fe58a4b632b2cd17` 基于 `main@5fa37dbfd`，54 项定向测试、typecheck、build 与 diff-check 通过，worktree 干净，`downtime_required=false`。由于根 `main` 已推进到 `3e44f599d3f626b7bef1d251cfd75eeb93ff038c`，旧候选不得直接进入合并车道；当前等待同一顶层任务刷新最新 `main`、重跑专项验证并完成刷新后的最终独立复审。未合并、未推送、未部署。
