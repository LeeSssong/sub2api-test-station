...
**T06 / T06-R1 利润页原生化验收与显示修复（2026-08-14）：** 状态：T06 `VERIFYING`（线上验收失败），T06-R1 `REFRESH_REQUIRED`。T06 候选已通过顶层任务规格/计划、实施、独立复审、全分支终审和根授权合并；发布基线为已推送的 `main@04349dda8f461f51aabb5f9fc0a3e2f6af251517`，tree `1c30f936e4a4d8c6338d18ace36d71860b58fc1c`，迁移哈希 `6a0e141eb4788460a99fc3e108ce5b46c866fd2c45b9a7265ea66b0ef8faaf71`。已完成 `downtime_required=false` 蓝绿生产发布；宿主记录 `/var/lib/sub2api/release-records/20260814T154749Z-production-3329818.json` 为 `succeeded/promoted`、`rolled_back=false`，活动槽 `blue`，API/worker 运行镜像 `ghcr.io/leesssong/xingqiao-sub2api:release-04349dda8f461f51aabb5f9fc0a3e2f6af251517-89acc95334957dc0e7aab3c346e1db7d2be111cdfe65ac44dbc55ecc47950d3e` 且健康，PostgreSQL、Redis、Caddy 和保留的 green 槽均健康；公网 `/healthz`、`/readyz`、`/health` 均返回 200。管理员登录态 `/admin/operations/account-profitability` 专项验收确认三项缺陷：页面卡片/表格固定白底但在深色主题继承浅色文字，导致内容近乎不可见；`24h`/`31d` 显示原始 i18n key；表头硬编码英文。根因已由 DOM 计算样式和源码交叉确认。T06 不得标记完成。用户可见 T06-R1 顶层任务 `01a00117-6fae-75f0-bbc1-6f340342acdc` 已从 `main@651bc2fab27544a8cc131137ab351bf8f2f90f89` 创建独立 worktree `/Users/gongtengxinwen/.codex/worktrees/e5e5/sub2api搭建`，使用 GPT-5.5、medium，仅修复主题可读性与中文本地化并重新完成发布、登录态验收；T07 继续禁止启动。 首轮候选 `d79e9195a024957b05a1939b2685fb7da329fc39` 已完成 8/8 页级测试、typecheck、production build、light/dark smoke、独立任务复审和最终全分支复审；根审查发现其落后当前 `main@d27cd9e5cdc0de18fa983bc04e2c9f7ccf0886c5` 两个治理文档提交，已进入刷新门禁，禁止直接合并。根目录新增未跟踪目录 `tools/gpt56_api_detector-git/` 归属未确认，已明确保留且不得在 T06-R1 整合中删除、覆盖或提交。
