...
**P0 Cloudflare 边缘 IP 误触发会话绑定事故（2026-08-17）：** 生产在 Cloudflare 代理开启后仍启用可选 `session_binding_enabled=true`，但 `api_key_acl_trust_forwarded_ip=false` 且 `forwarded_client_ip_headers=[]`；后端因此把 Cloudflare 边缘节点 IP 写入 IP/UA 会话指纹。生产时序证据显示登录 `200` 后紧接的 `/auth/me` 因另一边缘 IP 触发 `auth.session_binding.mismatch` 并撤销整个 refresh token family，后续 `/auth/refresh` 必然 `401` 并跳回登录页。根总控已停止 T12 发布且确认本轮未触发任何生产变更；随后通过既有管理设置 API 将 `session_binding_enabled` 恢复为 Sub 原生默认 `false`，写入与复读均返回 HTTP 200/`false`。独立候选已撤回旧 P0 的 Redis 轮换锁/密文 replay marker，使 auth/cache 核心文件恢复官方 v0.1.177。首次发布前只读预检又证明生产已设置 `SERVER_TRUSTED_PROXIES`，因此单独依赖该配置的门禁不足；修订候选 `codex/p0-native-session-stability@f381c8802` 改为必须同时具备默认关闭且只能由宿主设置的 `security.session_binding_allowed` 明确许可和 trusted-proxy 策略，生产不设置该许可，管理 API 无法单独重新开启绑定。修订已合入根 `main@77965c391`，候选与两次合并树直接相关测试、unit-tag 包编译、官方核心文件零差异、无 migration 和 diff-check 均通过；第一次证据不用于发布，T12 候选继续冻结保全。**状态：进行中（INTEGRATING；待最终 push、蓝绿发布与真实重新登录验收）。**
