...
**T137 账号卡片账号级统一有效观测修复（2026-09-08）：** 状态 `DEPLOYING`。规格与计划已获用户确认并直接实施。候选 `codex/t137-account-card-account-observations@dc3fb13de` 已合入根 `main@5cd68ce08`，账号卡片只消费账号级数据；有效观测为去重后的真实逻辑请求终态与去重后的主动探测终态直接相加，同桶不互斥；真实成功不再依赖 `actual_cost > 0`。直接相关后端仓储测试、前端账号卡片 26/26、差异检查通过；无 migration、配置或生产业务数据变化。用户已授权“快速部署主站，不同步测试站”，待推送根 `main` 后执行主站发布，测试站保持未同步并记录版本差异。
