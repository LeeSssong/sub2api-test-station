...
**T03-R1 维护发布迁移门禁补全（2026-08-14）：** 状态：进行中（实现与独立复审已通过并合并 `main@9b807e6fb`，等待推送、新证据、维护发布和线上验收）。用户在已获知宿主预检 `downtime_required=true`、`reason_code=migration_set_changed`、预计不可用最多 300 秒及保留独立表的应用回滚方式后，明确授权“允许停机部署”。独立工作区 `/Users/gongtengxinwen/Documents/sub2api搭建/.worktrees/t03-r1-maintenance-transition`、分支 `codex/t03-r1-maintenance-transition` 从 `main@fbd85f608fb6519a52e87b6a8d71011efcfa90a2` 创建；TDD 仅增加生产活动迁移集合 `f1b1f3537d518c30dc2fe99d75e9f2d7a5a27452f59ce4a50a1e81277c8cfbcc` 到 T03-R1 集合 `6a0e141eb4788460a99fc3e108ce5b46c866fd2c45b9a7265ea66b0ef8faaf71` 的第七组完整哈希 allowlist、migration 222 受控维护测试与运行手册，未知或篡改目标仍在任何 mutation 前 fail-closed。首次独立审查仅指出运行手册将 300 秒 API/worker 硬上限与 `+600 seconds` 恢复控制 deadline 混淆；提交 `c01b5674c` 修正后，复审 Spec/Quality 均 `APPROVE`、open findings 0。合并后 T03-R1 后端专项、vet、server build、前端 55/55、typecheck/build、控制器完整测试、精确迁移转换及关键维护回滚/截止场景均通过；尚未推送或触碰生产。下一步从当前干净 `main` 生成绑定新 HEAD/tree 的 `0600` 证据并执行已授权维护发布；禁止通配、跳过哈希、修改业务迁移或启动 T05。
