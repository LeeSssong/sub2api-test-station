...
**OAuth 图片编辑上传 MIME 兼容热修登记（2026-08-16）：** 状态：进行中（`BACKLOG`，排队等待，不占用发布车道）。已确认 OAuth 账号 `/v1/images/edits` 会把 multipart 图片转换为 Data URL 并保留 `application/octet-stream`，上游因此返回 400 `unsupported MIME type`；API-key 账号链路正常，事故不是图片尺寸问题。已知 `user_id=34`、API key `50`、生图 `group_id=19` 的请求命中；生产临时规避已于 10:02 将 OAuth 账号 `222/223` 移出 `group_id=19`。用户邮箱仅保留在授权交接上下文，不写入仓库总账。本任务必须等待 T11-R1 完成推送、部署和线上验证后，才从最新干净 `main` 创建用户可见 GPT-5.6 Sol/medium 顶层任务与独立 worktree；最小范围仅在 `openai_images_responses.go` 对空 MIME 或 `application/octet-stream` 进行字节识别，只允许识别为 `image/*`，无法识别则拒绝。不改错误码/文案、中文提示、`ErrorPassthroughRule`、客户端表现或其他上传链路；仅做相关单测、后端必要构建、diff/范围、发布预检和上线后 `/images/edits` 定向验收，禁止 GitHub Actions 或直接覆盖生产。

**T11-R1 Sub 原生计费聚合经营页纠偏登记与移交（2026-08-16）：** 状态：进行中（`INTEGRATING`，`P0/URGENT`）。最终候选 `codex/t11-r1-native-accounting-profitability@6e38e2f9d607361145dd183384824c32cc8c3a9c` 完成 Tasks 1–4、逐任务复审、前端/浏览器修复复审和 fresh whole-branch review，随后由根总控按 `AUTHORIZE_MERGE_TO_MAIN` 合入 `main@1043ac3a9f68002e48ebe4e7c9800a70575a7967`。merged-main 后端 focused 4 组、repository/service/admin/server neighboring、Colima 真 PostgreSQL integration、前端 API/View/browser 19/19、lint、typecheck、build、diff/protected/forbidden/config/dependency/migration/workflow 和发布脚本语法均通过；真实 390×844 runner 返回 `pass=true`、`viewportExact=true`、`externalTargets=[]`、金额卡片 `overflow=false`、`outside=false`、`adjacentOverlap=false`，用户 Chrome `127.0.0.1:9222` 哨兵未受影响。whole-branch reviewer 结论为 Ready to merge，Critical/Important 为 0；root 仅保留 handoff 指定的受保护内容。`downtime_required=false`；31 天生产尺度 API latency/query-plan 与登录态生产页仍是部署后的 `VERIFYING` 门禁，关闭前不得标记 DONE。当前等待最终账本提交、clean release worktree final-tree evidence、推送与蓝绿部署。经营语义保持：`requests`、`tokens`、账号计费 `cost = SUM(COALESCE(account_cost, COALESCE(account_stats_cost, total_cost) * COALESCE(account_rate_multiplier, 1)))`、用户扣费 `user_cost = SUM(actual_cost)`，利润 `user_cost - cost`，美元单位；用户未消费余额保留为独立只读 CNY 卡。T03/T03-R1 历史 evidence/review/OAuth/override 能力保留但不再作为经营页取数源；仅移除已批准的页面控件/入口。T09、S1-R2 与 OAuth 图片编辑上传 MIME 兼容热修继续暂停。
