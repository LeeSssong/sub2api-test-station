...
func (_q *PromoCodeUsageQuery) QueryQuotaGrants() *UserQuotaGrantQuery {
...
func (_q *PromoCodeUsageQuery) WithQuotaGrants(opts ...func(*UserQuotaGrantQuery)) *PromoCodeUsageQuery {
	query := (&UserQuotaGrantClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PromoCodeUsageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*PromoCodeUsage, error) {
	...
	if query := _q.withQuotaGrants; query != nil {
		if err := _q.loadQuotaGrants(ctx, query, nodes,
			func(n *PromoCodeUsage) { n.Edges.QuotaGrants = []*UserQuotaGrant{} },
			func(n *PromoCodeUsage, e *UserQuotaGrant) { n.Edges.QuotaGrants = append(n.Edges.QuotaGrants, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *PromoCodeUsageQuery) loadQuotaGrants(ctx context.Context, query *UserQuotaGrantQuery, nodes []*PromoCodeUsage, init func(*PromoCodeUsage), assign func(*PromoCodeUsage, *UserQuotaGrant)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(userquotagrant.FieldPromoCodeUsageID)
	}
	query.Where(predicate.UserQuotaGrant(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(promocodeusage.QuotaGrantsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.PromoCodeUsageID
		if fk == nil {
			return fmt.Errorf(`foreign-key "promo_code_usage_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "promo_code_usage_id" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
