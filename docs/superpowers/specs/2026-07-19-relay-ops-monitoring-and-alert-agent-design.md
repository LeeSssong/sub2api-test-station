...
- 已完成：生产来源录入、公开价格采集、候选六小时调度、事件状态机、持久化飞书投递、只读 Agent 契约、`/pricing`、管理员 `/ops` 以及服务器 `read_only` 部署。
- 已接入：账单会话登记 API 和管理员表单；服务器秘密文件仅允许挂载在 `/run/secrets/upstream-sessions/`，每小时读取用量页，401/登录页识别、24 小时告警去重和恢复通知均已实现。
- 当前门禁：真实 Neko/Wawazz 会话文件、Feishu Webhook 和 Agent API Key 尚未安装。未安装时质量/价格监控继续工作，真实账单只显示未配置，不得伪造倍率证据。
- 新增零成本验收：`POST /relay-ops/api/acceptance/synthetic` 由管理员触发固定脱敏事件，复用同一事件状态机、Agent 分析和持久化 Feishu 投递；重复事件去重，无上游请求、账单读取、余额/路由/Key 写入。`/ops` 已提供单一命令按钮和实时结果区；动态管理员视图禁用缓存，移动/桌面布局已验收。生产已实际返回“上游未访问 / Agent 确定性回退 / 飞书未配置”；审计记录为 confirmed、样本 2、Agent 分析与投递均 0。v7 继承集成配置状态证据，并将公开价格证据版本化为 `pricing-evidence-v2`；HTML 倍率必须有明确语义上下文，终端尺寸、CSS、脚本或编码片段不能进入成本证据。剩余门禁是安装真实 Feishu/Agent 凭据后验证外部送达。
