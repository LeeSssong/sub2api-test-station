...
**T20/T21/T22 新任务登记（2026-08-18）：** 用户已明确批准三项后续工作并要求按优先顺序继续：T20“用量详情过时提示清理 + 盈利页零流水账号补齐”先行，T21“生产模型检测 sidecar 接入与离线状态纠正”排队，T22“官方 Channel Monitor V2 简洁运营视图”随后排队。根总控已扫描全部注册非 `main` worktree：未发现可直接并入的新候选；保护/历史 worktree 保留原状。T20 已完成规格/计划、实现和直接相关门禁，当前状态为 `READY_FOR_ROOT_REVIEW`：隔离 checkout `/Users/gongtengxinwen/Documents/sub2api搭建/.worktrees/t20-usage-detail-zero-flow`、分支 `codex/t20-usage-detail-zero-flow`、候选 `3b120046e328535ce587db60a5ef750586d652d0`、bundle `/private/tmp/t20-usage-detail-zero-flow-ready.bundle`。T21/T22 状态为 `BACKLOG`，不得插队或并行进入整合/发布车道。三项均未修改生产、未推送、未部署；当前沙箱禁止根 `.git` index/object/ref 锁，根登记提交与候选合并待发布环境恢复后执行；发布继续遵守本地/宿主蓝绿链、非 GitHub Actions 和 `downtime_required` 门禁。
