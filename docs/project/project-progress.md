...
**T24 特惠分组本地调度耗尽 503 纠正整合（2026-08-18）：** 独立顶层任务已从 `main@3b6fda9ba` 完成规格、计划、TDD 与直接相关验证，候选 `codex/t24-local-scheduling-exhaustion@6871ddaac` 工作区干净。根总控审查确认改动仅将 Responses/Chat Completions 流式与非流式的本地调度耗尽统一为 HTTP 503、`local_capacity_exhausted` 和中文解释，管理员诊断为 `routing/platform/未选择上游账号`，真实上游 503 透传、失败切换和诊断保持不变。候选已快进合入根 `main@6871ddaac`，无迁移、配置或生产数据变更；当前状态 `INTEGRATING`，正在合并后直接相关门禁，尚未推送、部署或线上验收。
