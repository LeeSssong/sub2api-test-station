...
- 候选已在本地刷新合入当前根 `main@43ffa2353ed96da668d0846753f472fea922d07d`，并已按用户授权快进合入根 `main`；根已推送为 `main@1827057cb9dc1f9266da9b4f659af6045fa88e2e`、tree `b79c3458a29ec46db536cbc6e34b368ebc877418`。
- 当前候选未提交文件（无暂存）：本恢复检查点与交接增量；代码与回归测试已提交在 `d0a4c9a1b`，不得覆盖或丢弃。根 `main` 当前无未提交文件。
...
- 当前根 `main@1827057cb` 与 `origin/main` commit/tree 一致；发布控制器已完成来源校验、构建和宿主预检，但因迁移集合变化 fail-closed，未进入生产变更。
...
- 未验证项：`migration_set_changed` 后的维护发布、验收站/主站线上功能和真实飞书投递；必须先取得并记录单独“允许停机部署”授权及精确 active migration hash。
- 需核对项目总账中“历史已清理/已发布”描述与当前宿主事实的时间差；不以旧总账或聊天摘要重放任何动作。

## 恢复后的唯一安全动作

唯一下一动作：等待用户明确“允许停机部署”并由根总控核对当前 active migration hash；授权后只能从已推送、干净的根 `main@1827057cb` 按维护发布链继续，任何 hash 不匹配或状态漂移立即停止。
