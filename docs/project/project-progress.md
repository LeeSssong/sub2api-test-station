...
7. **账号上游倍率自动同步**：上游 billing probe 已解析账号有效倍率，但当前只写入观测快照，未回写 `accounts.rate_multiplier`；本次增加托管/手工覆盖策略、审计、缓存同步和账号生命周期/定时触发。Task 1 的托管/手工覆盖策略与纯决策逻辑、Task 2 的仓储事务/审计/缓存同步、Task 3 的生命周期强制刷新和周期探测接线（含生命周期活跃状态竞态修复）均已完成本地实现、回归验证和任务级独立复审，正待最终全分支复审；尚未推送、部署或线上验证。**状态：进行中；待全分支复审、推送、部署和线上验证**。
