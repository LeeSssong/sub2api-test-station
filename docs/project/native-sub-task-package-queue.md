...
- 队列状态：官方 Sub2API `v0.1.177` 更新是当前最高优先级插队任务；5 个冲突已人工解决，候选已合入并推送为 `main@458c27c1cc4511a029884fbe3e34d907cd4ab47b`，现处于 `DEPLOYING`，只通过既有发布链生效，不做额外测试或验收。T12 与 T13 暂停在设计阶段，S1-R2 排在其后。
- 唯一发布总控：根目录 `/Users/gongtengxinwen/Documents/sub2api搭建` 的 `main`。只有发布总控可以修改全局队列/总账、根 `main`、发布证据和生产状态记录。
- 当前发布状态：官方 `v0.1.177` 已完成候选交接、根合并和 push，正在由唯一发布总控执行本地/宿主发布链；T12、T13 不得进入实现、合并或发布。
...
- 当前状态：`REFRESH_REQUIRED`。用户指定可见任务 `01a008e1-55d1-74f1-a659-fac363dcfd28` 继续作为执行窗口，并由可见任务 `01a006a8-ef15-7960-b72a-de2fddab0339` 统筹；必须先保全旧方向的提交和未提交内容，再按新全局约束刷新执行。
...
- 完成条件：发布链成功切换；不另做功能验收。发布链若因停机/迁移/运行就绪保护拒绝切换，保留候选和证据，不绕过保护。
...
- 当前状态：`DESIGNING`（仅登记/排队）。用户可见 GPT-5.6 Sol/medium 顶层任务 `01a00969-b2ea-7ff0-9f49-d7af64438e00` 负责规格/计划/实现/复审/handoff；官方 `v0.1.177` 发布完成前只允许正式规格草案，不创建实现 worktree、不写代码、不合并、不推送、不部署或访问生产。
