...
- 队列状态：官方 Sub2API `v0.1.177` 更新是当前最高优先级插队任务；只解决合并冲突并直接通过既有发布链生效，不做额外测试或验收。用户指定“快速迭代-指挥（5）”继续统筹、T09 可见任务执行；旧 T09“冲突即停止”方向被新全局策略取代，现处于 `REFRESH_REQUIRED`。T12 暂停在设计阶段，S1-R2 排在其后。
- 唯一发布总控：根目录 `/Users/gongtengxinwen/Documents/sub2api搭建` 的 `main`。只有发布总控可以修改全局队列/总账、根 `main`、发布证据和生产状态记录。
- 当前发布状态：官方 `v0.1.177` 正在交接至指定总控与 T09 执行任务，独占发布车道；当前任务不再操作更新，T12 不得进入合并或发布。
