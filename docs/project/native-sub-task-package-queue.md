...
**T137 账号卡片账号级统一有效观测修复（2026-09-08）：** 状态 `DESIGNING`。账号卡片健康/性能指标统一按 `account_id` 聚合，分组只筛选卡片并提供明确的分组账务与调度上下文；有效观测固定为去重后的真实逻辑请求终态与去重后的主动探测终态直接相加，不再按五分钟桶互斥。真实成功与 `actual_cost` 解耦，摘要、成功率、时间线、账号质量和调度质量输入消费同一账号快照。规格：`docs/superpowers/specs/2026-09-08-t137-account-card-account-only-observation-design.md`。待用户确认后才进入实施计划；当前不改代码、不创建实现 worktree、不合并、不推送、不部署，且未取得主站发布授权。
