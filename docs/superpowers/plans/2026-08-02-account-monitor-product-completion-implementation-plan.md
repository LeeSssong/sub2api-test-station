...
- 管理状态、服务状态和分组资格必须分开；成本不合格不能计为服务不可用。
...
- Produces: 顶层 `accounts` 包含暂停账号；顶层与分组账号均返回独立 `management_state`、`service_state`、`group_eligibility` 和派生 `monitor_bucket`；全站与分组 `health` 的服务计数互斥且不受成本资格影响；关闭分组保留账号投影。

- [x] **Step 1: 写失败测试**：覆盖非 `active`、不可调度、启用可用、启用不可用、启用待确认、倍率待确认和分组成本不合格账号；断言三层状态独立、`monitor_bucket` 唯一、成本不合格但服务成功仍计为服务可用，且四类服务计数之和等于唯一账号数。
- [x] **Step 2: 运行目标测试并确认因现有 active 过滤、关闭分组清空账号而失败。**
- [x] **Step 3: 最小实现**：移除 active-only 投影过滤；独立生成管理状态、服务状态和分组资格，再按“暂停 > 服务待确认或倍率待确认 > 成本不合格 > 服务可用/不可用”派生 `monitor_bucket`；健康计数只使用管理/服务状态；评分只适用于分组资格合格且服务可用的账号；关闭分组保留账号但保持 `closed`。
- [x] **Step 4: 排除暂停账号对页面整体 `stale` 的影响，并新增“暂停账号无探测、启用账号新鲜”的回归测试。**
- [x] **Step 5: 运行 service 目标包测试、`go test ./internal/service/...` 与 `git diff --check`。**
- [x] **Step 6: 提交独立代码与测试。**
...
- [x] **Step 1: 写失败测试**：API 给出多分组、未分组和暂停账号时，首次加载显示全部唯一账号，且没有分组账务请求。
- [x] **Step 2: 运行 Vitest 并确认现有默认首分组和前端 active+schedulable 过滤导致失败。**
- [x] **Step 3: 最小实现**：保留完整账号数组；新增全站 Tab；删除默认选首分组逻辑；移除重复的分组下拉筛选，以 Tab 作为唯一范围选择。
- [x] **Step 4: 覆盖全站与分组切换、搜索和服务状态筛选组合；运行两个目标测试文件。**
- [x] **Step 5: 提交独立代码与测试。**
...
- [x] **Step 1: 写失败测试**：断言全站经营与账号服务是独立区块；分组经营与服务均存在；权重摘要与账号状态摘要同行；五类账号进入正确分区。
- [x] **Step 2: 运行 Vitest 并确认当前混排与单一卡片网格导致失败。**
- [x] **Step 3: 建立按 `monitor_bucket` 分区的纯计算函数/计算属性**；全站不展示分组质量分，分组可用区按组内质量分降序。
- [x] **Step 4: 重排模板**：经营/服务分区、权重显式入口、分区标题与双列卡片；空分组或搜索零结果仍保留分组摘要与评分入口；复用现有卡片和弹窗，不改评分/调度接口。
- [x] **Step 5: 将历史状态、空态和状态标签统一为中文；运行 AccountMonitorView、卡片、评分弹窗测试和前端类型检查。**
- [x] **Step 6: 提交独立代码与测试。**
