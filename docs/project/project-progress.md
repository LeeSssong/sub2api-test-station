...
**T20 生产收口与 T21 启动（2026-08-18）：** T20 候选 `3b120046e328535ce587db60a5ef750586d652d0` 已合入并推送根 `main@c2a1429623b22d0e5c3d4746a508d0f34e0a93e9`，tree `e41c1460b05ad1b040c2700fb54227b2dad0f947`。既有预加载蓝绿链返回 `succeeded/promoted`、`rolled_back=false`、`downtime_required=false`，活动槽 `blue`，宿主记录 `/var/lib/sub2api/release-records/20260818T051214Z-production-2858199.json`，迁移哈希保持 `bb6ebff31f0ffe9be5ad204ba79ef896d98522ccdd7b3933843c94d6c9ad5951`，公网 `/healthz`、`/readyz`、`/health` 均 200。生产用量样本 125444/125509/125512 的有效账号成本与 T17 口径一致，evidence `unavailable` 保留为辅助状态；24h 财务 API 已返回多个零流水有效绑定账号，请求、成本、用户扣费和利润均为零。T20 状态为 `DONE`。按队列顺序，T21 现进入 `DESIGNING`，先盘点 T15 原生检测路由、sidecar 样本、宿主配置与许可边界；T22 保持 `BACKLOG`。**状态：T20 已完成；T21 进行中。**

**T20/T21/T22 新任务登记（2026-08-18）：** 用户已明确批准三项后续工作并要求按优先顺序继续：T20“用量详情过时提示清理 + 盈利页零流水账号补齐”先行，T21“生产模型检测 sidecar 接入与离线状态纠正”排队，T22“官方 Channel Monitor V2 简洁运营视图”随后排队。根总控已扫描全部注册非 `main` worktree：未发现可直接并入的新候选；保护/历史 worktree 保留原状。T20 已完成规格/计划、实现、直接相关门禁、根合并、推送、无停机蓝绿发布与线上验收；T21 进入 `DESIGNING`，T22 保持 `BACKLOG`。发布继续遵守本地/宿主蓝绿链、非 GitHub Actions 和 `downtime_required` 门禁。
