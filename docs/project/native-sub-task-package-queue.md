...
**T132 账号监控检测判定与证据可见性修复（2026-09-05）：** 状态 `READY_FOR_ROOT_REVIEW`。当前候选 `codex/monitor-probe-failure-exclusion`，基线 `origin/main@171526060`；用户明确要求把候选已有的 `0b53b67b8`、`f6ad3d34f` 与本次模型检测修复合并发布。已恢复 v4.1.1 官方预设及正式模型判定；账号卡片拆分 Juice/模型两行结果；证据不明确不再误报整次检测失败；历史详情展示 Juice 申报冲突证据，并为升级检测保存上一轮具体可疑证据白名单快照。旧记录不回填，无凭据、提示词或完整响应落库。适配器 9/9、前端 69/69 与 typecheck、后端模型检测/监控仓储/runner/handler/migration 定向测试及 server build 通过。用户已明确授权“快速部署主站，不同步验收站”；发布仍须从已推送且干净一致的根 `main` 发起，若预检返回 `downtime_required=true` 则停止等待额外授权。
