<template>
  <article class="w-full min-w-0 overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-950" data-test="monitor-card" :data-account-id="account.account_id">
    <div class="grid min-w-0 gap-x-4 gap-y-3 px-[18px] py-4 max-[430px]:px-[14px] max-[430px]:py-[14px] lg:grid-cols-[minmax(15rem,1.45fr)_minmax(10rem,.9fr)_minmax(11rem,1fr)_minmax(15rem,1.35fr)_minmax(13rem,1.1fr)_auto]" data-test="monitor-card-header">
      ...
      <section class="flex min-w-0 flex-wrap items-start justify-end gap-1 lg:flex-col lg:items-stretch" data-test="account-actions" aria-label="账号操作">
        <span class="sr-only">账号操作</span>
        <button class="icon-button h-8 w-8 lg:w-full" data-test="account-info" type="button" title="查看账号信息" aria-label="查看账号信息" @click="emit('accountInfo', account)"><Icon name="eye" size="xs" /><span class="sr-only lg:not-sr-only lg:ml-1 lg:text-[11px]">账号信息</span></button>
        <button class="icon-button h-8 w-8 lg:w-full" data-test="account-edit" type="button" title="编辑账号" aria-label="编辑账号" @click="emit('accountEdit', account)"><Icon name="edit" size="xs" /><span class="sr-only lg:not-sr-only lg:ml-1 lg:text-[11px]">编辑</span></button>
        <button class="icon-button h-8 w-8 lg:w-full" data-test="account-delete" type="button" title="删除账号" aria-label="删除账号" @click="emit('accountDelete', account)"><Icon name="trash" size="xs" /><span class="sr-only lg:not-sr-only lg:ml-1 lg:text-[11px]">删除</span></button>
        <button class="icon-button h-8 w-8 lg:w-full" data-test="account-more" type="button" title="更多账号操作" aria-label="更多账号操作" @click="emit('accountMore', account, $event)"><Icon name="more" size="xs" /><span class="sr-only lg:not-sr-only lg:ml-1 lg:text-[11px]">更多</span></button>
        <button class="icon-button h-8 w-8 lg:w-full" data-test="refresh-account" type="button" title="刷新当前账号" aria-label="刷新当前账号" :disabled="running" @click="emit('refresh', account.account_id)"><Icon name="refresh" size="sm" :class="{ 'animate-spin': running }" /><span class="sr-only lg:not-sr-only lg:ml-1 lg:text-[11px]">刷新</span></button>
      </section>
      ...
      <section class="min-w-0 border-t border-gray-100 pt-3 lg:col-span-6 dark:border-slate-800" data-test="model-detection-section">
      ...
      <section class="min-w-0 border-t border-gray-100 lg:col-span-6 dark:border-slate-800" data-test="calls-disclosure">
      ...
      <footer class="flex min-w-0 flex-wrap items-center justify-between gap-2 border-t border-gray-100 pt-3 text-[11px] text-gray-500 max-[430px]:flex-col max-[430px]:items-start max-[430px]:gap-[3px] max-[430px]:py-[9px] lg:col-span-6 dark:border-slate-800 dark:text-slate-400" data-test="card-footer"><span class="break-words">检查于 {{ checkedAtLabel }} · 统计截止 {{ statisticsCutoffLabel }}</span></footer>
    </div>
...
const qualityRankValue = computed(() => props.account.quality_rank ?? (props.rankingScope === 'global' ? props.account.group_rank : null))
