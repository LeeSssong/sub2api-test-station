...
**T06 利润页移除外部控制面状态（2026-08-14）：** 状态：进行中（`DEPLOYING`）。候选已通过顶层任务规格/计划、实施、独立复审、全分支终审和根授权合并；当前根合并提交 `main@04cf031f54f23dd394c6189e091dc07410b44b10`，合并后定向/相邻 Vitest、typecheck、production build、diff-check 均通过。无运行时代码、API、数据库迁移、配置或 GitHub Actions 变化，预检目标 `downtime_required=false`；待推送、蓝绿发布、利润页登录态线上验收和归档清理。
