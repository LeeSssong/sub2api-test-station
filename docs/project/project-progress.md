...
**T15 账号监控原生探测模型与异步模型检测（2026-08-17）：** 独立 worktree `/Users/gongtengxinwen/Documents/sub2api搭建/.worktrees/t15-native-probe-model-detection`、分支 `codex/t15-native-probe-model-detection` 已将根 `main@b59baac1434f54024c5d5ea15e1d6f804d511aae` 整合进候选；刷新后最终候选为 `fcefd06f86bc2aa43de6ce33737ac3f6fda285e7`，tree `8cf5d21f0b8536497517124c3c8daa4758522fa3`，根已合入生成提交 `7c754d8c00f5a7730b5b409fcd819f0450f8442a`，并以状态提交 `06040ebd5225b313e10159b21bb97e1d1d111a28` 推送至 `origin/main`。合并后 migration/service/repository/routes focused、受影响后端 compile-only、前端 AccountMonitorCard/AccountMonitorView 两文件 93 tests、`vue-tsc --noEmit`、`vite build`、gofmt 与 diff-check 均通过。预加载控制器完成候选镜像构建后，宿主预检因新增迁移 `225_account_model_detection.sql` 返回 `downtime_required=true`、`reason_code=migration_set_changed`、预计不可用 300 秒，并在候选启动/流量切换前停止；生产活动槽和流量未变。0600 测试证据：`/Users/gongtengxinwen/.codex/release-evidence/sub2api/2026-08-17-main-06040ebd5-t15-native-probe-model-detection-v1.json`；停机门禁证据：`/Users/gongtengxinwen/.codex/release-evidence/sub2api/2026-08-17-main-06040ebd5-t15-downtime-gate-v1.json`。实现继续保持 API Key/OAuth 隔离、敏感数据保护和 detector 许可证门禁：未获书面商业授权或合法独立实现前生产不得配置 `SUB2API_MODEL_DETECTOR_URL`/`SUB2API_MODEL_DETECTOR_TOKEN`。**状态：进行中（DEPLOYING；停在停机授权门禁，未部署、未线上验证）。**

**T18 渠道状态官方聚合/自建监控可切换（2026-08-17）：** 用户可见顶层任务已在独立 worktree `/Users/gongtengxinwen/Documents/sub2api搭建/.worktrees/channel-status-official-toggle`、分支 `codex/channel-status-official-toggle` 刷新到根 `main@096f37f25688b72eca901173828965133051acf0`；候选 tip `6d38cabe33b15adcd2be234c04d4939e824e1da4`、tree `34f77c904caf09e2fff1a370c66ff0c6d7735d06`，worktree 干净。功能提交 `ee52dd5ea`；仅改 `MonitorV2RouteView` 入口与专项测试，复用 `channel_monitor_mode=v1|v2`，无后端/迁移/workflow 变化。刷新后 `MonitorV2RouteView` 3/3、`pnpm typecheck`、`pnpm build` 与 `git diff --check` 通过；上线参数为 `channel_monitor_enabled=true`、`channel_monitor_mode=v2`，回滚为 `channel_monitor_mode=v1`。**状态：进行中（READY_FOR_ROOT_REVIEW；用户最新指令为暂不发布，排队等待 T15 停机门禁释放，未合并、未推送、未部署、未线上验收）。**

**T19 Monitor V2 缓存命中率有效样本口径修正（2026-08-17）：** 用户明确批准按既有方案修正并加入全局任务队列；候选已在独立 worktree `/Users/gongtengxinwen/Documents/sub2api搭建/.worktrees/t19-monitor-v2-cache-eligibility`、分支 `codex/t19-monitor-v2-cache-eligibility` 刷新到根 `main@096f37f25688b72eca901173828965133051acf0`；候选 tip `804c0db63c56483e43cfdf6ca1fd85bc1cf69da9`、tree `1f16c263ac59599b5b17e997ed4a148ba1cd27b7`，功能提交 `0f9ef38f2a0621d9afe5b5c965da025161dba399`，交接为 `docs/handoffs/2026-08-17-t19-monitor-v2-cache-eligibility-handoff.md`。范围仅限 `upstream/sub2api/backend/internal/repository/monitor_v2_repo.go` 的 Monitor V2 缓存统计 SQL 与直接相关 sqlmock 测试：分子/分母限定为 `actual_cost > 0`、成功流水及文本 Token Prompt Cache 语义（`billing_mode='token'`，或历史 `billing_mode` 为空且图片/视频字段全零），排除图片/视频/按请求计费及零成本失败占位；不改 API、前端、账务/价格/倍率或缓存策略，无迁移、无生产数据写入，预期 `downtime_required=false`。TDD RED/GREEN、仓储/服务聚焦测试、后端 compile-only/build、gofmt、diff-check 均通过。**状态：进行中（READY_FOR_ROOT_REVIEW；用户最新指令为暂不发布，候选未合并、未推送、未部署；排在 T15 发布车道完成后、T18 候选之后）。**
