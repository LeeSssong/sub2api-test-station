...
- 当前唯一发布候选：T05 候选进入 `REFRESH_REQUIRED`。功能候选 `9dcdce2101413c6b710e867c8f0293f6edc3f29c` 已完成实施/复审，但基于旧 main，必须先整合当前根 `main@689153da8` 并重新验证，才可进入根授权合并。
...
- 当前状态：`REFRESH_REQUIRED`。T05 已完成规格批准、计划、实现和独立复审；候选基于旧 main，必须在同一 worktree 整合当前根 `main@689153da8`，保留根总账版本，并重新完成专项测试和最终全分支复审。旧 detached `a71c675b1` 仅作启动审计，不继续使用。
