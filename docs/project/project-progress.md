...
**本轮登记（2026-08-04）：** 账号监控卡片收敛与营收页面职责拆分，状态：进行中（本轮先修订最小样本 HTML 原型，不改生产页面；新增一次性采购成本、卡片内全局优先级编辑、每组双列排名卡片、七项原生分组汇总和 5 秒实时并发演示。采购成本非空时使用采购模式，为空时使用原生倍率；分组倍率只出现在分组汇总。营收、成本、利润、对账、账务历史和异常处理后续统一迁移到独立“营收”页面。评分在选定 24 小时、7 天或 30 天窗口内优先使用真实请求，真实请求少于 3 次时才使用同期探测补样并明确标识；账号倍率缺失时成本项计 0 分，但账号继续参与评分和排名。未推送、部署并在线验证前不得标记完成）。

**本轮生产实施登记（2026-08-04）：** 账号监控卡片数据完善 V3，状态：进行中（HTML 原型、视觉证据和本地实现已完成；`codex/account-monitor-completion` 已推送到 `e02914ac969c886cdeec5a2184dff422eb8d3ce0`。新鲜后端/前端验证通过，绑定当前树的 0600 发布证据已生成，候选镜像已预加载。普通生产发布在启动候选前正式返回 `migration_set_changed` 和 300 秒维护窗口；生产仍运行 `d4fb5e4a4`，PostgreSQL、Redis、Caddy、活动 API 和 Worker 未变化。用户已确认维护部署，当前正以测试驱动方式把发布器限定为 `e95b… -> 337212…` 这一条采购成本附加迁移转换；只有新提交重新推送、维护发布完成且线上验收生效后才可标记完成）。

**本轮视觉纠偏登记（2026-08-04）：** 账号监控 V3 设计稿 1:1 还原，状态：进行中（本地生产组件验收确认 Task 4 重写时遗漏已确认卡片中下部：绿色卡头/左边框、彩色指标格、近期探测柱状图、当前窗口调用折叠、检查时间与单卡刷新，并额外展示了设计稿没有的平台筛选。已新增 `docs/project/account-monitor-v3-acceptance-contract.md` 作为最高优先级不可漂移合同，并更新交付总表覆盖旧的账号监控经营职责；后续实现、独立审查、桌面/移动同视口截图对比均逐条依合同执行。本轮继续停在部署前，未推送、部署和线上验证前保持进行中）。
...
0.1. **账号监控 V3：账号质量、评分、排名与调度信息**：2026-08-04 最新确认已覆盖旧的“账号监控同时承接经营/账务”方案。当前唯一目标是按最终截图 1:1 实现原生分组 Tab、七项分组汇总、真实请求时间窗、完整六段式账号卡片、卡内全局优先级、一次性采购成本、原生并发与桌面双列/移动单列；营收、利润、账务和对账迁往后续独立“营收”页面。**状态：进行中（本地实现、最终同视口视觉验收和受影响测试矩阵已完成，分支已推送；普通发布已在生产变更前被采购成本新迁移门禁拦截，用户已确认最长 300 秒的维护部署。当前实施并复审唯一迁移转换门禁，随后重新推送、部署和在线验收；这些证据齐全前禁止标记完成）**。
...
**本轮维护门禁登记（2026-08-04）：** 采购成本附加迁移维护门禁已完成本地实现与回归，状态：进行中（发布器与宿主执行器现仅允许 `e95b3512ccfc5b5103b4547857c437338921fd6bb463b7f2078c9ee24da4f0fc -> 337212b4af85839c9497d0fef3153e5c858bd976fed268086459c21a12abcc76`；旧 `c618fc… -> e95b…`、未授权和非法目标均被拦截；维护路径只停 API/Worker，失败恢复旧 API/Worker 且不触碰 PostgreSQL、Redis、Caddy。已更新 runbook 与测试。尚未推送、部署或线上验证，保持进行中）。
