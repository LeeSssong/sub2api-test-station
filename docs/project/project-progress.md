...
**本轮 Task 6 实施登记（2026-08-10）：** 在已通过 Task 4 独立复审的 `/Users/gongtengxinwen/Documents/sub2api搭建/.worktrees/fix-official-update-stuck`、分支 `codex/fix-official-update-stuck@221440545` 继续迁移余额、账单、费率/倍率采集与受控写命令；外置服务只写控制面事实表，核心账号更新必须经官方 API/窄命令适配器，不得直接 SQL 写核心业务表。已复核全部已注册非 `main` worktree：其他候选仍无“已推送 + 已部署 + 已线上验证”的完成证据，无需先合并的已完成领先分支；根 `main`、发布证据和其他 worktree 继续只读保护。**状态：进行中；本阶段仅本地 TDD、审查和提交，不推送、不合并、不部署、不访问生产。**
