...
- **T47-R1 operational timeline color correction**: status INTEGRATING. Candidate `codex/t47-r1-operational-green@8d38819d0` has been fast-forwarded into root `main`; focused Vitest 20/20, `pnpm typecheck`, `pnpm build`, and `git diff --check` passed in the candidate. The correction removes T47's frontend-only latency `>= 2000ms` degraded override so every runnable/`operational` bucket is green with `UP`; unavailable remains red and no-probe remains gray. No API, data source, migration, config, or production-data changes. Production push, blue-green deployment, and online verification remain pending.
