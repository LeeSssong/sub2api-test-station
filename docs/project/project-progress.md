...
**本轮 T04 账号监控移除外部控制面状态（2026-08-12）：** 状态：进行中（已获精确 `main@e8f93a57a1e59aedff694bf3591bdf3dcbd4a03a` 授权并合入本地 `main`，尚未推送、部署或线上验收，不得标记已完成）。T03 已完成推送、生产验收、可恢复归档和候选清理后，T04 从当时最新干净 `main@eaa73610238f1effc253dc84a98eae4965390525` 创建用户可见的独立顶层 Codex 任务 `019ff559-b4d6-7411-81c7-d0e631584d70`，独立 worktree 为 `/Users/gongtengxinwen/.codex/worktrees/72c7/sub2api搭建`，分支为 `codex/t04-account-monitor-native-only`。只读探索曾确认账号监控先请求原生 `/admin/accounts/monitor`，再请求外部页面决策与监控投影，外部结果可替换原生数据，失败时显示“控制面暂时不可用 / 完整性 unknown”并延长加载链。用户随后逐节批准了推荐方案、架构与范围边界、端到端数据流与失败语义、接口/字段契约与兼容边界、最小 MVP 验证及其余设计章节，并确认账号监控所有场景对 `/xingqiao/**` 零请求、原生 `/admin/accounts/monitor` 为唯一数据源、共享控制面文件留给 T05/T06、原生失败不回退外部数据、正常态不增加替代状态条，且不修改现有账号卡片、页面布局、视觉样式或交互行为，必须无感优化。设计提交为 `5b58197e90063e92793793b1326770068843c3c2`，计划提交为 `e123fc472`，实现候选为 `8f14556dfc327ab56d7004fbe33c0fb8cfb965bd`，最终候选为 `63019434684a53b7b856a6acea5605e3e8b4aede`。账号监控现已在首次加载、时间窗切换、刷新和代表性单账号探测后重载中只调用 `/admin/accounts/monitor`，且页面不再请求 `/xingqiao/**`；页面控制面投影、状态及状态条已删除，未引入替代常驻状态，也未改动卡片、布局或写操作。原生首次加载失败显示独立的可重试空态；保留已有快照时继续显示范围错误和旧卡片。独立任务复审结论为规格符合性 PASS、任务质量 PASS；最终全分支终审及终审后总账轻量复核的 Critical/Important/Minor 均无发现。候选上新鲜专项 `pnpm exec vitest run src/views/admin/__tests__/AccountMonitorView.spec.ts` 通过（28/28），`pnpm typecheck`、`pnpm build`、`git diff --check` 与范围检查通过，生产页面的控制面符号与 `/xingqiao/` 扫描为零匹配；仅有既有非致命 pnpm、Browserslist、Vite 动态导入/分块警告。共享控制面文件未改动；无迁移、无配置变更，`downtime_required=false`。正式设计见 `docs/superpowers/specs/2026-08-12-t04-account-monitor-native-only-design.md`，实施计划见 `docs/superpowers/plans/2026-08-12-t04-account-monitor-native-only.md`。后续仍须在已合并的 `main` 上完成专项回归、发布预检、推送、既有本地/宿主链部署和登录态浏览器零 `/xingqiao/**` 线上 MVP 验证；不得使用 GitHub Actions。
