...
**本轮 Task 2 实现（2026-08-09）：** 已增加结构化账号成本解析：严格按 `ClassifyImageBillingTier` 匹配 1K/2K/4K 图片区间，固定图片成本不应用账号倍率；Token、普通按次和模型文件成本保留倍率语义；未知档位继续旧回退。OpenAI 两条 usage 记录路径均写入新请求的 `account_cost` 快照。聚焦账号统计/记录用量测试、`go vet ./internal/service` 与 diff 检查已通过；尚未任务审查、合并、推送、部署或线上验证，状态：进行中。
