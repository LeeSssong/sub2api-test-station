...
export default {
  ...
  schedulerSettings: {
    ...
    description: 'Configure unified quality routing for ordinary text requests and group-level cross-account recovery.',
    enabled: 'Enable unified quality routing',
    group: 'Select group',
    ...
    guardDescription: 'Eligibility, native account slots, cooldown, and health checks always come first; image requests remain on native scheduling.',
    fixed: 'Fixed guardrail',
    recoveryTitle: 'Cross-account recovery',
    recoveryHint: 'Ordinary text is ordered by success rate, trimmed-mean first-token time, live effective cost, then account ID. Set only the extra attempts for this group.',
    extraRetryCount: 'Extra recovery attempts',
    extraRetryCountHint: 'Counts only attempts that actually enter another account after the first upstream request. Valid values are 0–3; the same account is never replayed.',
