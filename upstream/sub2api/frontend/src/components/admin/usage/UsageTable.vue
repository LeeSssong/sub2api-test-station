        ...
        <template #cell-cost="{ row }">
          <div class="text-sm">
            <div class="flex items-center gap-1.5">
              <span class="font-medium text-green-600 dark:text-green-400">${{ formatMoneyFixed(row.actual_cost) }}</span>
              <span
            ...
            <div v-if="showAccountBilling && row.account_rate_multiplier != null" class="mt-0.5 text-[11px] text-orange-500 dark:text-orange-400">
              A ${{ formatMoneyFixed(accountBilled(row)) }}
            </div>
          </div>
        </template>
...
import { formatDateTime, formatMoneyFixed, formatReasoningEffort, reasoningEffortValuesEqual } from '@/utils/format'
