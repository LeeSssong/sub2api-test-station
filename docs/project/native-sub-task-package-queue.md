...
**T103：** 状态 `DEPLOYING`（停在 `downtime_required=true / migration_set_changed` 门禁）。用户报告同类错误回归；主站只读证据显示活动 handler 无额外 admission 接线，账号 `293` 在 T100 后无新 `openai.admission_rejected`，最新错误为真实上游 502，其中一条是上游 OAuth token 撤销。修复已合入并推送根 `main@5833d36d664dfc15942a0fb5ea3735170577a0c0`（tree `80fcc51ee76a3925e7ce3d50e3978e48a13c1071`），admission/slow-session service 入口硬性 no-op，回归测试确认配置开启且 store 拒绝时仍完全放行、零 store 调用；0600 证据 `/Users/gongtengxinwen/.codex/release-evidence/sub2api/2026-08-31-main-5833d36d6-t103-admission-permanent-guard.json`。候选构建已完成，主站未停服/迁移/重启/切换；发布链发现当前 `main` 含 T98 待授权的 231 号迁移（候选哈希 `0bda54bb…`），生产活动哈希为 `88a0ff14…`，因此暂停等待明确“允许停机部署”授权及受控迁移转换。账号并发只保留 Sub 原生槽位；不恢复、轮换或写入账号 `293` 的 OAuth 凭据，不改账号/分组数据。
