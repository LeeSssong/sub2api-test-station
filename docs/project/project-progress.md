...
**官方 Sub2API v0.1.177 最高优先级更新（2026-08-16）：** 状态：受阻（`BLOCKED`，等待停机门禁人工确认）。用户明确要求官方更新插队为最高优先级，并建立全局例外：只解决上游合并冲突后直接通过既有本地/宿主发布链使更新生效，不运行功能测试、回归测试、类型检查、独立构建验证或上线专项验收；发布链内置的候选制品生成、迁移/停机保护、原子切换和失败回滚不得绕过。T09 可见任务 `01a008e1-55d1-74f1-a659-fac363dcfd28` 已保全旧候选并交付官方 `v0.1.177@073e92d17178a1ccdb0a27017f572f10c9c7ab62` 刷新候选 `458c27c1cc4511a029884fbe3e34d907cd4ab47b`；5 个冲突已人工解决。根总控已补齐官方 Go `1.26.6` 构建基线并形成 `main@716f6876f19c7781afe1df5e43bf0ab889e6e325`，已推送；既有发布链已完成候选制品生成，但预检返回 `downtime_required=true`、`reason_code=migration_set_changed`、预计不可用约 300 秒，原因是候选迁移集合与活动版本不同。按停机门禁，候选未启动、未切换槽位、未部署，当前活动版本保持不变；证据 `/Users/gongtengxinwen/.codex/release-evidence/sub2api/2026-08-16-main-716f6876f-official-v0177-downtime-gate.json`，候选 worktree `/private/tmp/sub2api-release-v0177-716f6876f` 保留。T12、T13 与 T14 继续暂停在设计/排队阶段。

**T13 NewAPI 上游倍率自动登记（2026-08-16）：** 状态：进行中（`DESIGNING`，仅登记/排队）。用户已确认 NewAPI 精确匹配日志 `other.group_ratio` 是权威倍率；范围仅限 NewAPI API-key 且没有原生 Sub 倍率声明的账号：首次真实成功请求后登记到 `accounts.rate_multiplier` 并在 `accounts.extra` 标记，已登记账号每个北京时间自然日仅首笔合格请求刷新一次；必须具备并发 CAS、失败不覆盖和管理员可见“已登记”。不做迁移、历史回填或生产修改。用户可见 GPT-5.6 Sol/medium 顶层任务 `01a00969-b2ea-7ff0-9f49-d7af64438e00` 仅负责规格/计划/实现/复审/handoff；当前只允许正式规格草案，不创建实现 worktree、不写代码。官方 `v0.1.177` 更新继续独占合并与发布车道。
