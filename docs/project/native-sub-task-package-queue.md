...
**T107 quota wallet 充值与可消费额度注入紧急修复（2026-09-01）：** 状态 `READY_FOR_ROOT_REVIEW`，候选 `codex/t107-quota-wallet-fix@a3e8b3adb2196c6e94374923f4747bbe28789985`（tree `46780f47f5a6f39f12aa4066118af6661e454ca6`）。根因为官方 wire 重生成覆盖了 `UserHandler.SetQuotaWalletService` 注入，导致 quota-summary、quota-ledger 摘要/充值/流水均返回 `quota wallet service not available`。候选仅恢复原生 QuotaWalletService wiring 并补直接回归测试；admin handler/service focused、admin 全包、server build、diff-check 均通过；无迁移、配置或生产数据变化。按用户“紧急修复、插队部署生产”走快速主站路径，主站成功后同 commit 同步验收站；不得与 T106 SQL 修复混批。
