...
**本轮登记（2026-08-06）：** Sub2API `0.1.171` 自动发布与生产切换修复，状态：进行中。范围包含对官方 commit `f0e7a9c7a23a7d02fb159b62fa809621eb0475a6` 的版本锁定、fail-closed 语义冲突解析，候选镜像构建/推送/生产装载，host updater 蓝绿拓扑适配，最终生产切换与主链路验收。仅在“已推送到服务端 + 已部署 + 已验证生效”同时成立后才可标记完成。验证采用敏捷分层：本地只跑受影响测试，CI 只保留一次权威 Sub2API 资格门禁，生产仅验版本、健康、管理端、`/v1/models`、worker、容器身份与发布记录。
...
0.2. **Sub2API `0.1.171` 自动发布与生产切换**：修复 `0.1.170/0.1.171` 官方发布后的多文件语义冲突，在不放宽未知冲突门禁的前提下恢复可重复候选构建；将旧单实例 host updater 适配到现有蓝绿发布执行器，并完成候选发布、生产切换和精简主链路验收。**状态：进行中（已授权本轮最终生产切换；未完成推送、部署与线上验证）**。
