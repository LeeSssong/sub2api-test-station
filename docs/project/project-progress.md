...
1. **账号监控真实上游成本与对账闭环**：已确认采用统一真实账单适配器：请求结束立即写入本地 Attempt，异步合并采集上游逐笔账单，管理员刷新强制对账，超 10 分钟进入异常；补登记仅填写上游实际扣费，并以追加式流水保留审计。每日 00:00 进入 closing、00:10 结算，并在后续 3 日滚动重算，接入现有全站总账和飞书日报。**状态：进行中（工程实施中）**。

2. **2026-08-01 main 全局低延迟配置蓝绿发布与待部署事项联合评估**：`main@17b229eeb` 已合并并推送全部已收口分支；发布 controller 已补足与 host executor 的绝对 deadline 参数契约并通过回归。生产只读预检显示仍为单实例拓扑，不存在 host executor 或 `release-state`，Caddy 仍指向静态 `sub2api:8080`；首次安装双槽拓扑需维护窗口。**状态：进行中（停机评估完成前）**；账务总账、倍率同步和 Monitor 卡片精简已统一排队待部署。
...
11. **`api.xingqiaolab.top` TLS 兼容性修复**：轮换站点证书并显式设置兼容 TLS 策略，仅对 Caddy 的 API 站点执行配置校验和平滑 reload；上线后以 TLS 1.2/1.3 握手、`/v1/usage` 请求及 CC Switch 3.16.5 刷新用量成功为完成门禁。**状态：进行中**。
