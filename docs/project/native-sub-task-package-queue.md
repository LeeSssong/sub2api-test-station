...
**T103：** 状态 `DESIGNING`。用户报告同类错误回归，主站只读证据显示活动 handler 无额外 admission 接线，且账号 `293` 在 T100 后无新 `openai.admission_rejected`；最新错误为上游 502，其中一条是上游 OAuth token 撤销。本任务永久防线仅将 admission/slow-session service 入口硬性 no-op 并加回归测试，账号并发只保留 Sub 原生并发槽；不擅自恢复被上游撤销的 OAuth 凭据。基线 `main@e84a51d8e`，不改账号/分组数据、迁移或凭据。
