...
- **T35 采购保存 PostgreSQL 参数类型热修**：状态 `DESIGNING`。独立用户可见任务 `01a01b11-3613-7ce0-811e-93d986e5cf16` 使用 `gpt-5.6-terra/high`，worktree `/Users/gongtengxinwen/.codex/worktrees/eeb6/sub2api搭建`，从最新根 `main` 初始化；实施分支预定为 `codex/t35-procurement-audit-type-hotfix`。用户已批准推荐最小方案：为采购录入、清空和结算三条 `jsonb_build_object` 审计 SQL 的动态参数补齐 PostgreSQL 显式类型（账号 `bigint`、金额/额度 `double precision`、原因 `text`），保留采购版本台账、事务、幂等、审计、成本公式和前端合同。范围不含迁移、历史回填、生产数据改写、账务公式或其他页面；先以真实 PostgreSQL 参数推断回归或等价最窄 RED 证据锁定根因，再执行 focused Go tests、后端 build 与 diff-check，预期 `downtime_required=false`，最终以根发布预检为准。
