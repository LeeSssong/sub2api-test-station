...
cleanup() {
  rm -f -- "$guard_body" "$post_counts" "$admin_header" "$gateway_header"
}
trap cleanup EXIT
...
curl "${curl_bounds[@]}" -fsS -H "@$admin_header" \
...
require_guard() {
  ...
  status=$(curl "${curl_bounds[@]}" -sS -o "$guard_body" -w '%{http_code}' -X POST \
    -H "@$admin_header" "$base_url$endpoint" 2>/dev/null) || fail
  [[ "$status" == 409 ]] || fail
...
curl "${curl_bounds[@]}" -fsS -H "@$gateway_header" \
