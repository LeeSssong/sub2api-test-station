...
**OAuth 图片编辑上传 MIME 兼容热修登记（2026-08-16）：** 状态：进行中（`INTEGRATING`，当前唯一发布车道任务）。T11-R1 已完成生产验收与收口，用户可见顶层任务 `01a00892-2294-7083-aa01-7aa6f94d1dc4` 使用 GPT-5.6 Sol/应用默认 medium 在独立 worktree `/Users/gongtengxinwen/.codex/worktrees/d883/sub2api搭建` 完成规格 `3a1d97d2a`、计划 `37cdc9bce`、fresh implementer、任务复审与 fresh whole-branch 终审；最终候选 `554e19dab9b63a2bf7beff9889efac52177396b1` 经非破坏性刷新为 `de462d34837a8d6ac4605e65f2f6193e1bfa867a`、tree `f856bcd3173baecb490136ba40b2b579bffdd3c0`，已由发布总控授权并无冲突合入根 `main@788ca6aeb`。实现仅在 `openai_images_responses.go` 新增 OAuth Responses 私有 helper，并让 image/mask 两个 Responses call site 使用；空 MIME 或规范化后的 `application/octet-stream` 按字节识别且仅接受 `image/*`，无法识别或非图片 fail closed，显式 `image/*` 保持。共享 helper、Grok/API-key/multipart、错误码/文案、中文提示、`ErrorPassthroughRule`、依赖、配置、迁移、前端、Actions、发布脚本和生产数据均未修改。候选与刷新后 focused tests、compile-only、diff/范围/禁区/邮箱 guards 均通过；当前执行 merged-main 最小门禁、发布证据、推送和蓝绿预检，尚未部署或线上验收。生产临时规避仍为 10:02 将 OAuth 账号 `222/223` 移出 `group_id=19`；本任务不改变该分组策略。T09 与 S1-R2 继续暂停。
