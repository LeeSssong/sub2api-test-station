  ...
  <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
    <div class="card p-4 flex items-center gap-3">
      ...
      <div>
        ...
        <p class="text-xl font-bold">{{ stats ? (stats.total_requests ?? 0).toLocaleString() : '—' }}</p>
            ...
            >
              ...
              <span class="flex items-center justify-between gap-3">
                ...
                <span class="tabular-nums">
                  {{ stats ? formatTokens(stats.total_cache_creation_tokens ?? 0) : '—' }}
                </span>
              </span>
              <span class="mt-1 flex items-center justify-between gap-3">
                <span>{{ t('usage.cacheReadTokensLabel') }}</span>
                <span class="tabular-nums">
                  {{ stats ? formatTokens(stats.total_cache_read_tokens ?? 0) : '—' }}
                </span>
              </span>
            </span>
          </span>
    ...
    <div class="card p-4 flex items-center gap-3">
      ...
      <div class="min-w-0 flex-1">
        ...
        <p class="text-xl font-bold text-green-600">
          {{ formatUsdMoney(stats?.total_actual_cost) }}
        </p>
        <p class="text-xs text-gray-400">
          <template v-if="showAccountCost && totalAccountCost != null">
            <span class="text-orange-500">{{ t('usage.accountCost') }} {{ formatUsdMoney(totalAccountCost) }}</span>
            <span> · </span>
          </template>
          <span>
            {{ t('usage.standardCost') }}
            <span :class="{ 'line-through': strikeStandardCost }">{{ formatUsdMoney(stats?.total_cost) }}</span>
          </span>
        </p>
      </div>
    </div>
