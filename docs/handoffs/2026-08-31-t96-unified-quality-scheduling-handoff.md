...
## Recovery checkpoint (2026-09-01, opt-in boundary fix pending)

- Authoritative worktree: `/Users/gongtengxinwen/Documents/sub2api搭建/.worktrees/t96-unified-quality-scheduling`; branch `codex/t96-unified-quality-scheduling`; current committed HEAD/tree `e4c36160848e0ab5b14ce271156ea5c099bed844` / `4e7a8ecced73246316cb51c3fc0bb55e0e80e74a`.
- Root source is clean `main == origin/main == 7d6c4a178d394b756e8c6428a4508e7230b4a73c`, tree `262776dba20cde897ca446ab30252b6577069684`. Candidate is 11 commits ahead and 1 root commit behind before refresh.
- Preserved worktree state: six modified Go files and the untracked T96 verification report. No staged files and no T96 test, merge, push, deploy, migration, restart, or production command is running.
- The six Go changes are the in-scope explicit opt-in correction prompted by review: unified quality must apply only to ordinary HTTP text entrypoints; Responses image intent, Responses WebSocket, and alpha-search must keep their existing/native scheduling and recovery paths.
- A concrete defect remains before commit: the Responses handler first calls `WithOpenAIUnifiedQualityScheduling` and then passes that already-marked context to the image-aware helper, so image intent still retains the marker. The current helper-only test does not exercise that composition. This must be corrected before latest-main refresh.
- T95 dependency remains present (`EffectiveCostForAccount`, `EffectiveCostStatusReady`, live `U`). T104 occupies the release verification lane but does not block candidate-only implementation. Native-only remains permanent: no admission, slow-session, first-output lock, or custom account concurrency may be introduced.
- Validation already established for the committed T96 body is historical evidence only. After the opt-in correction and latest-main refresh, rerun only direct functional coverage: service quality/profit/scheduler tests, repository quality SQL tests, focused handler opt-in/retry helpers, server build, scheduler settings UI 4/4, frontend typecheck, native-only guard, gofmt, and diff-check.
- Next executable action: fix the Responses context construction and strengthen the direct boundary test, run its focused test, commit the preserved opt-in patch/report checkpoint, merge `main@7d6c4a178` only inside this candidate, rerun the narrowed gates, then finalize this handoff and report. Rollback/stop condition: any merge conflict outside T96 scope or root movement requires stopping with all local content preserved.

## Final handoff checkpoint (2026-09-01)

- Candidate was safely refreshed inside its own worktree by merging clean `main@7d6c4a178d394b756e8c6428a4508e7230b4a73c` (tree `262776dba20cde897ca446ab30252b6577069684`) with no conflicts. Current pre-report commit is `509b631dc194108397c7ea393508738db888abe8` (tree `eda9659f93307b78819d656dc9a44ccbdd79c823`); report commit follows after this checkpoint.
- Final implementation boundary: ordinary HTTP Chat Completions, Embeddings, Messages, and non-image Responses opt into T96; Responses image intent, Responses WebSocket, and alpha-search stay on their existing/native paths. The accidental image-marker composition was corrected and covered by the boundary test.
- Narrow functional verification after refresh: service quality/profit/scheduler tests PASS; repository quality SQL tests PASS; retry/OAuth helper tests PASS; `go build ./cmd/server` PASS; scheduler settings Vitest 4/4 PASS; `pnpm typecheck` PASS; native-only guard PASS; gofmt and diff-check PASS.
- Full handler package remains baseline-blocked by the pre-existing `ProvideHandlers` arity and missing `openAIAccountScheduleModel` test compile defects; no T96 production compile error observed. No acceptance-station behavior, merge to root, push, deployment, migration, restart, or production data/config mutation was performed.
- T95 contract is consumed from main (`EffectiveCostForAccount`, `EffectiveCostStatusReady`, live U). T103 remains abandoned; only Sub native account slots are used. No migration introduced; `extra_retry_count` remains existing scheduler JSON with default 0 and native Groups profit fields remain authoritative.
- State: `READY_FOR_ROOT_REVIEW`. Next root-only action: review candidate commit/tree, then merge to clean root main and run root direct gates under release-control rules.

## Refresh recovery checkpoint (2026-09-01, main@a37491587)

- Full recovery sources reread: `AGENTS.md`, native incremental-delivery constraints, task queue, acceptance-station constraints, approved T96 spec/plan/ranking report, verification report, and this handoff. Git/files are authoritative; no chat-only completion claim is used.
- Worktree/branch/HEAD/tree before refresh: `/Users/gongtengxinwen/Documents/sub2api搭建/.worktrees/t96-unified-quality-scheduling`; `codex/t96-unified-quality-scheduling`; `d161667ee24d08bebba3dfe2daa83d19aa0d5b75`; `efaa55edc3fe1f25ceafc3613ca5c576130a733d`. Worktree is clean with no staged/untracked files.
- Refresh target: clean root `main == origin/main == a37491587e97a9f954cee1d4593f5a4a3df9978d`, tree `3ba719b02aa3a9159ba069e7ff1a4c0b15889390`; candidate relation before refresh is ahead 16 / behind 12. No test, merge, push, deployment, migration, restart, or production command is running.
- Scope to preserve during refresh: ordinary HTTP text opt-in; image/Responses WebSocket/alpha-search exclusion; live T95 U; native Groups profit qualification with availability fallback; group `extra_retry_count`; explicitly safe and unbilled cross-account replay only; T105/T105-R1 OAuth 429 semantics; Sub native account slots as the only concurrency mechanism.
- Non-goals remain unchanged: no root main/global queue/project ledger edit, no production account/group/priority/config/data mutation, no push/deployment/cleanup, and no admission/slow-session/first-output lock/custom account concurrency.
- Next executable action: merge exact `main@a37491587` inside this candidate. Stop and preserve all content if a conflict falls outside T96 scope; otherwise resolve only T96 overlap, run the approved narrow direct verification, update verification/handoff, and recommit `READY_FOR_ROOT_REVIEW` evidence.
