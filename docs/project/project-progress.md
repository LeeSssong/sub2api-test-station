...
7. **账号上游倍率自动同步**：上游 billing probe 已解析账号有效倍率，但当前只写入观测快照，未回写 `accounts.rate_multiplier`；本次将增加托管/手工覆盖策略、审计、缓存同步和账号生命周期/定时触发。Task 1 的托管/手工覆盖策略与纯决策逻辑已准备完成；整体仍需后续接线、推送、部署和线上验证。**状态：进行中；待代码实现、推送、部署和线上验证**。
