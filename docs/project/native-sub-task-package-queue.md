...
**T138 自购账号首选层与 API Key 冷启动调度（2026-09-09）：** 状态 `DESIGNING`。规格 `docs/superpowers/specs/2026-09-09-t138-self-owned-tier-and-api-key-cold-start-scheduling-design.md` 已形成，待用户书面审阅批准后才能调用 writing-plans。范围固定为：在现有统一普通文本 OpenAI HTTP 调度中，先经过原生硬门禁，再将自购账号置于首选资源层；自购账号不伪造质量评分，层内按原生 priority/负载/公平性排序；无可用自购账号时进入 API Key 层；API Key 质量证据不足时 priority 提供有界冷启动先验，真实质量积累后接管。默认 priority=50 保持中性；不新增账号归属事实源、质量系统、迁移或配置；不改变生图、Responses WebSocket、alpha-search、账务、重试、安全重放、原生槽位和部署授权边界。当前仅文档变更，未创建 worktree、未实施、未合并、未推送、未部署。
