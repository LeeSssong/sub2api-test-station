...
- **T90 账号监控卡片真实请求证据、利润率与性能柱状图：** 状态 `DONE`。R2 已实现并推送根 `main@95a9ceef7`，验收站已用同一 tree 的 `0.1.177` 候选部署并完成人工线上核对；未部署主站。卡片只保留判断字段和真实性能柱状图；详情弹窗展示账号管理页全部可见字段且不脱敏；账号操作复用原生入口；主动探测保留；模型监测页面/历史/设置保留且仅手动触发。直接相关测试、类型检查、构建、Go build、diff-check 及验收站健康/页面检查均通过。发布证据：`/Users/gongtengxinwen/.codex/release-evidence/sub2api/2026-08-30-main-95a9ceef7-t90-acceptance-r2.json`。
