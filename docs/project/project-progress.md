...
**T15 账号监控原生探测模型与异步模型检测（2026-08-17）：** 独立 worktree `/Users/gongtengxinwen/Documents/sub2api搭建/.worktrees/t15-native-probe-model-detection`、分支 `codex/t15-native-probe-model-detection` 已从根 `main@25310c2379ec10807f5dccd9dd5bf8997b491646` 完成正式规格、计划、实现、直接相关测试和候选交接；worktree 干净，候选为 `bc108251c044cd038ceaa54bca061ff28d2311dc`。实现保持已批准原生边界：连接测试继续走 `AccountTestService.ProbeAccountConnection`；API 进程只持久化 queued run，singleton worker 轮询 claim 并最多四路执行仅执行 sidecar；固定北京时间六时隙及 30 分钟补触发；API Key/OAuth 严格隔离；执行前重读账号、原生模型和 fresh catalog；卡片提供收缩状态行、详情弹窗和中英文文案；检测结果不影响评分、调度或账号状态。migration focused、service/repository/routes focused、backend compile-only、前端 2 files / 93 tests、typecheck、build 与 diff-check 均通过；未扩大到全仓测试、额外 reviewer、压力/mutation 或浏览器矩阵。交接为 `docs/handoffs/2026-08-17-t15-native-probe-model-detection-handoff.md`。参考目录 `tools/gpt56_api_detector-git` 为 PolyForm Noncommercial 1.0.0；候选未复制其核心实现或基线，未获商业书面授权或合法独立实现前不得在生产配置 `SUB2API_MODEL_DETECTOR_URL`/`SUB2API_MODEL_DETECTOR_TOKEN`。本候选继续受保护且不属于当前 S2 串行车道，未经发布总控明确排期，禁止合并、push、发布预检或触碰生产。**状态：准备完成（READY_FOR_ROOT_REVIEW；未合并、未推送、未部署、未线上验证）。**
