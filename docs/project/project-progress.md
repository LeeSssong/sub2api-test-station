...
**本轮 Task 1 实现（2026-08-09）：** 已在候选工作区完成 `usage_logs.account_cost NUMERIC(20,10) NULL` expand-only 迁移、UsageLog 字段、单条/批量/best-effort 全部写入路径与查询扫描，并补充固定成本与历史 `NULL` 兼容测试；聚焦 unit 测试与 `go vet ./internal/repository` 已通过。集成测试因当前环境缺少 rootless Docker 未能启动；任务仍保持进行中，尚未合并、推送、部署或线上验证。
