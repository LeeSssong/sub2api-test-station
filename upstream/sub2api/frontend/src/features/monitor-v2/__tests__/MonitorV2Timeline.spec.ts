...
describe('MonitorV2Timeline', () => {
  it('shows a custom time-point tooltip without moving the hovered operational bar', async () => {
    ...
    expect(bar.classes()).toContain('h-8')
    ...
    const tooltipRow = wrapper.get('[data-timeline-tooltip-row]')
    expect(tooltipRow.element.contains(tooltip.element)).toBe(true)
    expect(tooltipRow.classes()).toContain('absolute')
    expect(tooltipRow.classes()).toContain('top-full')
    expect(tooltipRow.classes()).not.toContain('min-h-[68px]')
  ...
  it('uses the unavailable color and DOWN tooltip for an unavailable service point', async () => {
    ...
    expect(bars[0].classes()).toContain('h-8')
  ...
  it('keeps fixed-size dense timelines in a controlled inner scroller on narrow screens', () => {
    ...
    expect(track.classes()).toContain('gap-[6px]')
