...
- **T34 渠道状态原生探测重构**：状态 `REFRESH_REQUIRED`。独立用户可见任务 `01a01ad3-774a-7e80-a0b7-9bc9bcde54ed` 初始使用 `gpt-5.6-sol/high`，本次刷新按用户最新默认切换为 `gpt-5.6-terra/high`；基线 `main@bf237bbaa8ca46f7d8ce8a17d7a07dee1abc15ff`，worktree `/Users/gongtengxinwen/.codex/worktrees/5657/sub2api搭建`，实施分支 `codex/t34-native-probe-monitor-v2`，旧候选 `2e4ac36bc`。功能和直接验证已经闭合，但根 `main` 已推进到包含最新总账状态与 T35 登记的提交，旧候选必须整合最新 `main`、仅解决本任务范围冲突并重跑后端 targeted tests、前端 8 files/35 tests、typecheck、production build、source guard 与 diff-check，随后才回到 `READY_FOR_ROOT_REVIEW`。Monitor V2 继续只读 `account_monitor_results` 原生主动探测，不使用真实请求或旧 Channel Monitor 主路径；固定 24/28/30 桶、倍率紧邻名称、移除“旗舰”和当前页面宽度合同保持不变。
- **T35 采购保存 PostgreSQL 参数类型热修**：状态 `IMPLEMENTING`。独立用户可见任务 `01a01b11-3613-7ce0-811e-93d986e5cf16` 使用 `gpt-5.6-terra/high`，worktree `/Users/gongtengxinwen/.codex/worktrees/eeb6/sub2api搭建`，基线 `main@101357776e1af9dbf83df282afd96cdb284ffcf4`，分支 `codex/t35-procurement-audit-type-hotfix`。正式规格 `5dca4b13104bad06658b39acd7d5caf79d5b38be` 与实施计划 `c914a35e77b14d2c0fb98379fb061d71e5a38099` 已由根总控审查批准；当前按计划执行真实 PostgreSQL 生产 service RED、三路径零残留/原子提交、源码精确合同、sqlmock 回滚、精确 casts、focused unit/handler/build、零迁移与 diff-check。三条 `jsonb_build_object` 动态参数分别固定为账号 `bigint`、金额/额度 `double precision`、原因 `text`；采购版本台账、事务、幂等、审计、成本公式和前端合同不变。范围不含迁移、历史回填、生产数据改写、账务公式或其他页面；预期 `downtime_required=false`，最终以根发布预检为准。
