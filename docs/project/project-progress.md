...
**T12 经营页本站探测花费与排序/美元字段优化（2026-08-17）：** 原候选 `c7587599a` 曾合并后因 P0 插队在发布前被可追溯撤回，生产未构建、未迁移、未切换。P0 收口后，唯一发布总控解除冻结；T12 worktree 快进 `main@b16d45203`，仅反向撤销移除 T12 运行时/任务文档的 `9d9e2b758`，保留当前根全局文档和全部 P0 行为，生成运行时候选 `35baf14ae`、最终 handoff 候选 `4029240f4`。最新候选的 migration/repository/service/handler focused Go 测试、前端 19/19、typecheck/build、gofmt/diff 与 migration/范围安全扫描全部通过，P0 auth/config/session 文件相对基线零差异；候选已无冲突快进合入并随根 `main@59316b4a9a296a264efc59b33c9e66f53ee3d90c` 推送。既有蓝绿链完成候选镜像构建后，因迁移集合变化返回 `downtime_required=true`、`reason_code=migration_set_changed`、预计不可用约 300 秒，并明确在候选启动前停止；没有执行迁移、没有切换活动槽，生产仍运行 P0 修复版本 `e554b7d2e` 的 `green` 槽。完整 0600 门禁证据为 `/Users/gongtengxinwen/.codex/release-evidence/sub2api/2026-08-17-main-59316b4a9-t12-probe-cost-v2.json`。不得复用历史停机授权或自行带维护参数重试；worktree/分支继续保留，旧 docs-only T12、v0.1.175 和 S1 分支继续冻结。**状态：受阻（BLOCKED；等待用户明确授权本次约 300 秒维护窗口，随后才能重试部署和线上验收）。**
