...
- **T55 原生额度钱包与手动充值退款账本**：状态 `VERIFYING`（当前仅测试站）。用户已明确先部署到隔离 admin-lab，主站保持不变。测试站继续运行独立 T55 镜像并已验证登录、管理员身份、用户列表、额度摘要、额度流水及充值/退款/幂等路径；主站本次未执行迁移、重启或切换。生产发布暂缓，待用户另行恢复主站发布范围后再按迁移停机门禁继续。当前根 `main@5d6cb7ff8a53576b73001214ec317990a437ba95` 保持不变；T57 只能读取 T55 稳定契约，不能修改 T55 worktree。
