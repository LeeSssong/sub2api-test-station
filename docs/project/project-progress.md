...
**本轮非 main 工作区整合与发布准备（2026-08-09）：** 状态：进行中。所有候选已并入并推送至 `main@d73e82254dd0074557712f1903a1adc9d0901928`。获得停机授权后的首次发布在镜像传输阶段被 SSH 连接重置，第二次在停机前被 host executor 父目录校验误报拦截；生产 API/worker/PostgreSQL/Redis/Caddy 均保持运行。现已确认根因为远端 `bash -s -- name path` 传参后校验脚本误读 `$2` 而非 `$1`，正在修复、推送并重新冻结发布证据；未完成生产部署与线上验证前保持“进行中”。
