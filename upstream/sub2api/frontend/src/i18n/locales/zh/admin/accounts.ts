export default {
    accounts: {
      ...
      modelDetection: {
        ...
        conclusion: { normal: '与申报一致', abnormal: '检测异常', insufficient: '证据不足', failed: '检测失败' },
        juiceDetail: { pass: '启用档位均获得足够申报型号命中，且未出现明确冲突。', mismatch: '出现仅属于其他已知型号的明确证据，与申报型号冲突。', insufficient: '至少一个启用档位未取得足够有效命中。', non_gpt: 'Juice 未形成已知 GPT 指纹。', unavailable: '未取得可用 Juice 证据。' },
        fingerprintDetail: { strong_match: '仅一个模型严格超过当前档位阈值，运行契约匹配。', unclear: '样本不足、没有模型越线或多个模型同时越线。', unavailable: '未取得可用行为指纹证据。' },
        fingerprintCandidateDetail: '当前证据强烈指向 {candidate}。',
        abnormalDisclaimer: '检测器观察到异常；该结果不代表上游确认替换。', saveModels: '保存模型', detecting: '已排队…', detectNow: '立即检测', section: '模型检测', editConnectionProbeModel: '修改连接测试模型', viewRecent: '查看检测记录', historyTitle: '检测记录', historyEmpty: '暂无检测记录', historyLoadError: '检测记录加载失败', loadMore: '加载更多', loading: '加载中', profile: '档位', mode: '模式', reason: '触发原因', samples: '有效样本', fingerprint: '行为指纹', details: '查看详情', historical: '历史记录', observedAbnormal: '检测器观察到异常',
      ...
      monitor: { historyFallback: '评分沿用最近一次有效结果', historyFallbackAt: '历史最终结果时间' },
      createAccount: '添加账号',
      autoRefresh: '自动刷新',
      ...
      upstreamBilling: {
        ...
        failed: '失败',
        notProbed: '未探测',
        groupRate: '分组默认：{value}x倍率',
        userRate: '用户专属：{value}x倍率',
        peakRate: '高峰：{start}-{end}，{value}x倍率（{timezone}）',
        noPeakRate: '高峰倍率：未启用',
        effectiveRate: '当前：{value}x倍率',
        updatedAt: '更新时间：{value}',
        nextProbeAt: '下一次探测：{value}',
        lastDetectedRate: '上次探测：{value}x倍率',
