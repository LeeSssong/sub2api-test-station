...
- 队列状态：S1-R2、S2、S3 与 T17 均已完成推送、蓝绿发布和线上验收，当前为 `DONE`；T15 已合并并推送根 `main`，发布预检返回 `downtime_required=true`，用户已于 2026-08-17 明确授权停机部署，当前正在补齐并验证唯一精确的 `aaebed88… -> bb6ebff3…` 维护迁移 allowlist；T18、T19 均为 `READY_FOR_ROOT_REVIEW` 排队候选，T16 亦为 `READY_FOR_ROOT_REVIEW`，均不得插入 T15 发布车道。禁止使用 GitHub Actions。
...
- 当前状态：`DEPLOYING`（停机授权已取得，维护门禁补丁验证中）。独立 worktree `/Users/gongtengxinwen/Documents/sub2api搭建/.worktrees/t15-native-probe-model-detection`、分支 `codex/t15-native-probe-model-detection` 的最终候选 `fcefd06f86bc2aa43de6ce33737ac3f6fda285e7`、tree `8cf5d21f0b8536497517124c3c8daa4758522fa3` 已合入根 `main`。合并后 migration/service/repository/routes focused、受影响后端 compile-only、前端两文件 93 tests、`vue-tsc --noEmit`、`vite build`、gofmt 与 diff-check 均通过。预加载控制器完成候选镜像构建后，宿主预检因新增迁移 `225_account_model_detection.sql` 返回 `downtime_required=true`、`reason_code=migration_set_changed`、预计不可用 300 秒，并在候选启动/流量切换前停止；生产未变。用户已于 2026-08-17 明确授权停机部署。根总控确认生产迁移哈希为 `aaebed88f7fb712e1f518e73cc89bd44eb214f365f3b49f003598c93883a4604`、当前根候选为 `bb6ebff31f0ffe9be5ad204ba79ef896d98522ccdd7b3933843c94d6c9ad5951`，正在以 TDD 补齐唯一精确 `MAINTENANCE_11` 转换；未授权、错误 old/new 和未知转换继续在停服前拒绝。0600 门禁证据：`/Users/gongtengxinwen/.codex/release-evidence/sub2api/2026-08-17-main-06040ebd5-t15-downtime-gate-v1.json`。
