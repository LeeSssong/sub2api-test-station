...
- **T65 账号监控历史最终结果回退**：状态 `INTEGRATING`。合并提交 `main@4cf11cd05`；候选已刷新到最新根 `main` 并通过直接相关门禁，待推送、发布预检、蓝绿部署与线上验收。范围：模型检测证据不足/失败时沿用最近一次已完成且证据充分的最终检测结果；当前窗口评分不可用时沿用最近一次有效评分；页面明确展示当前状态、回退来源和时间。不新增事实源。
