...
- 队列状态：官方 Sub2API `v0.1.177` 更新是当前最高优先级插队任务；5 个冲突已人工解决，候选已合入并推送为 `main@716f6876f19c7781afe1df5e43bf0ab889e6e325`，发布链在候选制品生成后以 `migration_set_changed` 返回 `downtime_required=true`（预计不可用约 300 秒），现处于 `BLOCKED`，等待用户人工确认；未启动候选、未切换槽位、未部署。T12 与 T13 暂停在设计阶段，S1-R2 排在其后。
- 唯一发布总控：根目录 `/Users/gongtengxinwen/Documents/sub2api搭建` 的 `main`。只有发布总控可以修改全局队列/总账、根 `main`、发布证据和生产状态记录。
- 当前发布状态：官方 `v0.1.177` 已完成候选交接、根合并和 push；既有本地/宿主发布链已完成候选制品生成，但停机预检返回 `downtime_required=true` / `migration_set_changed`，已在候选启动前停止。T12、T13 不得进入实现、合并或发布。
