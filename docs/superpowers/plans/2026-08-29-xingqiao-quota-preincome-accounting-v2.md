...
Goal: 在 Sub2API 原生支付、Q 余额和使用记录基础上，落地 paid/gift 两类 Q 归因、预收入、聚合管理员账务退款和只读经营分析；兑换码分类在用户确认前不实现。

Architecture: 保留 users.balance 作为唯一对外 Q 总余额，内部只区分 paid_q 和 gift_q 两类额度池。真实支付继续由 payment_orders 管理；所有额度发放进入统一发放记录；消费通过内部事件和只读归因投影关联 usage_logs；管理员退款按 paid_q 聚合池处理，不按订单分摊。
...
- 系统内部记录额度单位 Q；页面统一显示 `$` 作为 Q 的标记符号，`$` 不是 USD，不得按美元或汇率解释。
- 实收入 = paid_consumed_q；charged_quota_q 是本次总扣费，且等于 paid_consumed_q + gift_consumed_q。
- 真实支付必须关联 payment_orders；非支付来源不得伪造支付渠道字段。
- usage_logs 不可变，只能增加只读额度归因投影。
- v1 退款是管理员账务退款，不要求关联 payment_orders，不按充值订单分摊。
- 返利进入 gift_q；兑换码归类待用户确认且暂不实现；管理员账务退款默认不清零赠送额度。
...
- Produce QuotaIssuanceRecord with source_type、amount_cny、paid_q、gift_q、payment_order_id、source reference、operator、policy snapshot 和 status；不新增 `cash_backed_q`、`non_cash_paid_q` 等第三类额度字段。
- Produce CreateIssuance(ctx, input) returning record and error；服务端生成 operator 和 confirmation status。
...
- [ ] Step 6: affiliate 创建 gift_q 发放且不创建 payment_order；兑换码暂不接入统一发放，等待 paid/gift 分类决策。
...
### Task 2: 两池 Q 状态和只读使用归因

Deliverable: 用户只有一个总 Q；内部按 paid/gift 两池消费；usage_logs 只读关联出实收入和赠送消耗。
...
- QuotaWallet exposes PaidQ、GiftQ、TotalQ。
- ConsumeUsage returns PaidConsumedQ、GiftConsumedQ。
...
- [ ] Step 1: 写失败测试：全 paid、全 gift、paid 2Q + gift 1Q，以及归因和 charged_quota_q 守恒；兑换码测试等待分类决策。
...
- [ ] Step 4: 实现 paid/gift 两池扣减，原生余额、来源池和内部事件同一事务。
- [ ] Step 5: 按逻辑请求 ID 记录两类消耗，不更新 usage_logs。
...
Deliverable: v1 只支持订单无关的管理员账务退款，按 paid_q 聚合池校验。
...
- refundable_q = paid_issued_q - paid_consumed_q - confirmed_admin_refund_q。
- payment_order_id optional，仅作说明，不参与退款资格。
- 默认只减少 paid_q，gift_q 不变。
...
- [ ] Step 4: 在用户锁内聚合已确认 paid_q 发放、paid_consumed_q 和历史退款，负值只用于显示修正，非法写入必须拒绝。
- [ ] Step 5: 同事务扣减 paid_q、写退款记录、写内部事件并更新 users.balance。
...
Deliverable: 管理员可分别查看真实支付/预收入、额度发放、管理员账务退款和使用归因；系统字段使用 Q，页面统一以 `$` 显示 Q，绝不解释为 USD。
...
- Q summary returns total_quota_q、paid_q、gift_q、refundable_q、refundable_cny。
- UI modes are manual_recharge、admin_gift、admin_refund。
- Business overview returns native total consumption plus paid_consumed_q、gift_consumed_q、legacy_unattributed_q；预收入单独来自 payment_orders 已确认收款金额。
...
- [ ] Step 3: 更新 API 类型和后端 DTO；兼容保留旧 *_usd 字段，但新页面用 `$` 表示 Q，不解释为 USD。
- [ ] Step 4: 拆分三种 UI 操作；提交前读取摘要，成功后使用服务端摘要，不用本地浮点覆盖。
- [ ] Step 5: 将支付/预收入、发放、退款和使用归因分组展示，显示来源引用和操作人。
- [ ] Step 6: 扩展 business overview，保留原生成本/总扣费公式，新增 `paid_consumed_q` 实收入、预收入和历史未归因消耗。
