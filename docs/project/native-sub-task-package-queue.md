...
- 队列状态：T01、T02、T03、T04 已部署并完成线上验证。T04 已通过用户可见独立顶层任务完成完整 brainstorming、书面规格、实施计划、实现、独立任务复审、全分支终审、根任务精确 SHA 授权合并、无停机发布和登录态浏览器验收；当前可以从最新干净 `main` 创建 T05 的用户可见独立顶层任务，但不得并行启动 T06 或后续任务包。
...
- T04 已完成合并、推送、无停机部署和线上验收；完成收口文档推送、候选归档及 worktree/分支清理后，根任务可以从届时最新且干净的 `main` 创建 T05 用户可见顶层任务。
- T05 顶层任务只执行完整 brainstorming 和规格批准流程；未经用户明确批准书面规格书，不得调用 writing-plans 或实施。T05 未完成线上验收前不得创建 T06。
