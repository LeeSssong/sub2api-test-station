...
**T13 NewAPI 上游倍率自动登记恢复（2026-08-16）：** P0 会话热修已生产收口，按持续目标恢复用户可见顶层任务 `01a00969-b2ea-7ff0-9f49-d7af64438e00`。实施前已盘点全部非 main worktree：历史 release 候选均已为当前 main 祖先，T12/T13/S1 独有提交都是未完成或冻结候选，不存在必须先合并的已完成领先分支。T13 已把原 17 个 Task 2/3 文件收口为干净候选 `c8ec34498ccc4d18eb42cdb250c90797d13b2ddd`；直接相关 service/repository 测试、编译、gofmt 与 diff-check 通过，无迁移、配置、依赖或工作流变化。候选相对根 `main@92db09644` 落后，现仅在原顶层任务整合最新主线并重跑同一组直接相关测试，不增加 reviewer 或扩大验证。T12 继续冻结，待 T13 推送、部署和线上验收后再恢复。**状态：进行中（REFRESH_REQUIRED；尚未合并根 main、推送或部署）。**
...
**T13 NewAPI 上游倍率自动登记（2026-08-16）：** 状态：进行中（`REFRESH_REQUIRED`）。用户已批准规格提交 `f2fc807d4fbca5f5917a00b3fadb890061cf3522`；范围仅限 NewAPI API-key 且没有原生 Sub 倍率声明的账号，以精确匹配日志 `other.group_ratio` 为权威倍率：首次真实成功请求后登记到 `accounts.rate_multiplier` 并在 `accounts.extra` 标记，已登记账号每个北京时间自然日仅首笔合格请求刷新一次；必须具备并发 CAS、失败不覆盖和管理员可见“已登记”。不做迁移、历史回填或生产修改。候选 `c8ec34498` 已完成功能相关验证，正在原 worktree 整合最新根 `main`；尚未进入根合并、推送、部署或线上验收车道。
