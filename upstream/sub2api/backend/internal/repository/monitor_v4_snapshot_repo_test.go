...
func TestMonitorV4SnapshotReplaceIsAtomic(t *testing.T) {
	...
	window.Groups[8] = service.MonitorV4GroupProjection{RequestCount: 1, SuccessCount: 1}
	mock.ExpectBegin()
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			rows := sqlmock.NewRows(columns).
				AddRow("24h", 7, base.SnapshotID, base.GeneratedAt, base.WindowStart, base.WindowEnd, base.ContractVersion, 75.0, 4, 3, 2, 1, 0, 0, 0, nil, 0, nil, 0, nil, nil, true).
				AddRow(tc.window, 8, base.SnapshotID, tc.generated, tc.start, tc.end, tc.contract, 75.0, 4, 3, 2, 1, 0, 0, 0, nil, 0, nil, 0, nil, nil, true)
