...
**T03-R1 Task 7（2026-08-13）：** 状态：进行中（source projection 最小修复已本地验证，待 fresh scoped re-review；尚未合并、推送、部署或线上验证）。当前工作区 `/Users/gongtengxinwen/.codex/worktrees/7292/sub2api搭建`，分支 `codex/t03-r1-upstream-cost-persistence`，基线 `4cc1d8bb825c5e2f266ffaec1f0bcedb6b74adb3`。Task 6 已完成独立复审；本任务在管理员 UsageView 增加异常流水过滤 Tab，消费 Task 5 本地证据/核对 API，支持逐笔、选中项和当前筛选核对、同筛选分页导出与 route `tab/range/account/evidence/review` 恢复，并将管理员详情改为只读本地证据。Task 7 初审发现 NewAPI unavailable 且 quota 为空时 UI 会错误推断为 Sub；最小修复将后端已持久化的 `source` 纳入前端类型，并令异常列表/CSV 直接使用该字段，新增 empty-quota NewAPI RED/GREEN 回归。定向 Vitest（5 files / 55 tests）、`pnpm typecheck`、`pnpm build` 与 `git diff --check` 已通过；无后端、schema、migration、Task 8+、main、其他 worktree、生产或 GitHub Actions 变更。
