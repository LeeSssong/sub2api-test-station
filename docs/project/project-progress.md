...
**T19 Monitor V2 缓存命中率有效样本口径修正（2026-08-17）：** 用户明确批准按既有方案修正并加入全局任务队列；当前登记为 `BACKLOG`，排在 T15 发布车道完成后、T18 候选之后，不打断 T15，不解冻 T16。正式规格为 `docs/superpowers/specs/2026-08-17-monitor-v2-cache-hit-rate-eligibility-design.md`，实施计划为 `docs/superpowers/plans/2026-08-17-monitor-v2-cache-hit-rate-eligibility.md`。范围仅限 `upstream/sub2api/backend/internal/repository/monitor_v2_repo.go` 的 Monitor V2 缓存统计 SQL 与直接相关 sqlmock 测试：分子/分母限定为 `actual_cost > 0`、成功流水及文本 Token Prompt Cache 语义（`billing_mode='token'`，或历史 `billing_mode` 为空且图片/视频字段全零），排除图片/视频/按请求计费及零成本失败占位；不改 API、前端、账务/价格/倍率或缓存策略，无迁移、无生产数据写入，预期 `downtime_required=false`。当前未创建实现 worktree、未修改运行时代码、未部署；后续按单车道从届时最新干净 `main` 独立启动，完成计划中的 TDD、聚焦测试、compile-only/build、gofmt、diff-check 及发布后 24h/7d 只读交叉验收。**状态：进行中（已排队，未启动）。**
