...
export default {
  ...
  schedulerSettings: {
    title: '调度设置',
    description: '为普通文本请求配置统一质量调度，并按分组设置跨账号恢复次数。',
    enabled: '启用统一质量调度',
    group: '选择分组',
    groupHint: '选择要编辑的 OpenAI 分组，草稿只在保存后生效。',
    draft: '当前草稿',
    noGroups: '暂无可配置的 OpenAI 分组',
    guardTitle: '服务不中断',
    guardDescription: '资格、原生并发槽位、冷却与健康检查始终优先；生图请求继续使用原生调度。',
    fixed: '固定护栏',
    recoveryTitle: '跨账号恢复',
    recoveryHint: '统一按成功率、截尾平均首字时间、有效成本倍率和账号 ID 排序；这里只设置本组失败后的额外尝试次数。',
    extraRetryCount: '额外恢复次数',
    extraRetryCountHint: '仅统计首次上游请求之后实际进入其他账号的尝试，范围为 0–3；不会重试同一账号。',
    save: '保存调度设置',
    saved: '调度策略已保存',
    saveFailed: '保存调度策略失败',
    loadFailed: '加载调度设置失败',
