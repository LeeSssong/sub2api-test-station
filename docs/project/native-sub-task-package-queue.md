...
- 队列状态：T01、T02、T03、T04、T03-R1 均已部署并完成各自既定线上验证；T03-R1 生产记录为 `20260814T051143Z-production-2876774.json`，活动槽 `green`，不得重复部署同一 SHA。
- 唯一发布总控：根目录 `/Users/gongtengxinwen/Documents/sub2api搭建` 的 `main`。只有发布总控可以修改全局队列/总账、根 `main`、发布证据和生产状态记录。
- 当前唯一发布候选：`优化账号监控卡片`。候选 `codex/account-monitor-card@72e063bca` 保留未提交复审修复，需完成修复、独立复审、全分支终审，并在刷新到最新 `main` 后重新验证，才能进入根授权合并。
...
- 当前状态：`FROZEN`。旧 worktree 为 detached `a71c675b1`，没有有效实现；仅保留启动审计。账号卡片完成生产验收后，从届时最新干净 `main` 创建新的正式分支/worktree，并重新执行完整 brainstorming、规格批准、计划批准、实施和复审。
...
- `优化账号监控卡片` 是当前唯一发布候选；完成生产验收前，不允许 T05 或 S1-R2 进入合并/部署车道。
- T05 保持冻结；账号卡片完成后由唯一发布总控按最新生产 `main` 重建。
