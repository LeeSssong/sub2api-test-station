...
**统一质量调度改用账号级原生 priority 且日常上限 50（2026-09-14）：** 状态 `IMPLEMENTING`。用户确认 A+C：统一质量日常优先级上限 50，读 `accounts.priority` 不读组关系 priority。候选 `feat/unified-quality-account-priority`，worktree `../sub2api-unified-quality-account-priority`。不推送、不部署。飞书 worktree 同 SHA 保留。
