...
**本轮登记（2026-08-06）：** 账号监控评分权重入口、账号成本/预计额度与上游余额刷新修复，状态：进行中（Tasks 1-4 已完成本地实现和逐任务独立审查；Task 5 focused 验证与整分支审查已通过，最终审查修复提交为 `13a318be7`，分支已推送。生产零停机门禁发现候选迁移集合与生产不一致：生产迁移哈希 `ac8b0b…`，候选 `db075a…`；候选相对生产缺少 192/193 并新增 197。已在镜像构建和任何生产变更前停止，待处理生产/main 分叉并重新审查，或另行获得停机部署授权；部署和线上验证前不得标记完成）。
...
0.0. **账号监控成本、余额与评分权重修复**：恢复分组评分权重编辑入口；OpenAI API Key 账号使用上游倍率，OpenAI 非 API Key 账号使用采购成本和预计可用额度计算等效倍率；余额刷新跟随账号健康探测，New API 付费倍率测量每 6 小时自动刷新并支持单卡强制刷新。**状态：进行中（Tasks 1-4 已完成实现、修复和逐任务独立审查；focused 后端/前端验证及整分支审查已通过，分支已推送。生产门禁因迁移集合与生产不一致在构建/变更前停止：生产含 192/193，候选缺少 192/193 且新增 197；尚未部署或线上验证）**。
