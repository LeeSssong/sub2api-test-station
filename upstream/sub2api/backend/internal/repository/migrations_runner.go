...
var migrationChecksumCompatibilityRules = map[string]migrationChecksumCompatibilityRule{
	...
	// 227 was corrected several times before the wallet schema stabilized. Existing
	// lab/production databases may hold one of the previously applied definitions;
	// accept only those exact historical checksums against the current file.
	"227_user_quota_wallet_ledger.sql": newMigrationChecksumCompatibilityRule(
