...
**本轮 T02 原生错误转译与管理员诊断 MVP（2026-08-12）：** 状态：进行中。T01 已完成推送、无停机部署、线上验证、归档和候选清理后，从最新且已推送的 `main@a4b7d8c6e` 启动队列唯一下一任务包。范围严格限定为四类首要错误：本站 RPM/并发限制、上游过载、通用上游失败、请求上传中断；复用 Sub 原生错误透传/转译规则，为用户返回脱敏中文含义与可执行建议，并在现有“管理后台 → 用量明细 → 错误请求 → 错误详情”和运维总览详情中复用“管理员诊断”字段。管理员诊断仅包含错误阶段、责任归属、是否已选择上游、账号/分组和原始上游状态/错误，不返回凭据、完整 API Key 或敏感请求体。不新增页面、错误表、全尝试链、追踪系统、自动根因推断、重试或其他任务包功能；HTTP/SSE 与错误记录口径必须一致。实施必须从该 `main` 创建独立 `codex/t02-native-error-diagnostics` worktree，先写本包规格与计划，按 TDD 实施并独立复审；未完成线上验证前不得启动 T03。
