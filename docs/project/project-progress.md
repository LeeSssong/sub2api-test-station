...
**T23 自购账号独立采购成本与人民币利润模型登记（2026-08-18）：** 用户已明确批准最终业务设计并要求立即登记、按现有单车道约束开始实施。T23 状态为 `DESIGNING`，排在 T22 之后，不打断 T21 生产验证、不解冻 T16。目标是为明确归属的自购账号建立独立、版本化、可审计的人民币采购成本与利润模型：收入按已实际消耗的站内额度 1:1 计入 CNY；成本按倍率前标准 Token 消耗额度确认并以真实采购价封顶；失效结算将剩余待摊成本记为采购损失；未录入成本显示“成本待录入”；渠道账号 USD 成本、用户扣费、原始 `usage_logs` 成本事实和既有账务公式保持不变。保留现有账号快速投影，新增 expand-only 采购成本台账、审计、幂等和并发保护，并在经营页提供独立自购账号人民币视图。预期 `downtime_required=false`，最终以根合并后预检为准；T23 当前尚未创建 worktree、规格或计划，下一步由独立顶层任务完成现状取证、brainstorming、正式规格/计划和 TDD 实现。
