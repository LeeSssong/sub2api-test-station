...
- **T40 错误码/边缘错误中文映射补齐（快速迭代-10）**：状态 `DESIGNING`。独立顶层任务 `01a031fa-eb70-7a90-8396-6136f674e710`、worktree `/Users/gongtengxinwen/.codex/worktrees/c92f/sub2api搭建` 已从最新 T39 生产源启动。范围为复用 Sub 原生错误透传与 `NativeUserErrorProjection`，补齐应用侧 402、507、520、521、522、523、524、525，并保留 499 客户端断开分类；覆盖 JSON/SSE、关键词/正文匹配、管理员诊断及 Cloudflare HTML 413 不误判/不泄露边界。不修改 T39 语义、迁移、生产数据或 GitHub Actions；候选不得自行修改全局总账、合并、推送或部署。
