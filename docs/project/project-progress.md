...
**T14 用量详情上游扣费/利润字段兼容热修（2026-08-16）：** 状态：进行中（`READY_FOR_ROOT_REVIEW`，等待根总控授权合并）。候选分支 `codex/t14-usage-detail-field-compat`，HEAD `d7c7113293f36d9e9bd0b560f346be580806b82c`，基线 `main@4f31ec3dd010dc3d2b6c5caaacadddce1adb84a2`，工作区干净。fresh whole-branch reviewer 已明确 `SPEC: APPROVE`、`QUALITY: APPROVE`，无 Critical/P1/P2；34 个 focused tests、typecheck、build、diff-check 与范围检查通过。已确认根因：`/admin/usage/:id/upstream-cost` 返回 `NormalizedCostCNY`、`EvidenceStatus` 等 PascalCase 字段，详情弹窗仅读取 `normalized_cost_cny`、`evidence_status` 等 snake_case 字段，导致“上游实际扣费 / 利润”显示为 `-`。范围严格限定为该详情弹窗/API 响应字段的向后兼容归一化与最小相关验证；不得并入 T12，不改变账务口径、聚合、持久化或生产数据，不做迁移、历史回填或相邻页面重构。未推送、未部署、未做线上管理员验收，等待根总控授权。
