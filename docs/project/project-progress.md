...
**本轮 T01 大上下文入站上传稳定性（2026-08-12）：** 状态：进行中（已审查并合并，待推送、部署和线上验证）。候选工作区 `.worktrees/t01-upload-stability`，分支 `codex/t01-upload-stability`，基线 `main@54f35713323116ce44646041347bde9ce548295e`，候选 `a3cc13a37f994360f556fc96e8e15f374a5bbcf5` 已经第二轮独立复审批准，并由无冲突合并提交 `129fc6003` 纳入 `main`。范围仅限 Caddy 入站长窗口、现有请求体大小保护兼容、配置合同测试及受控慢速/不完整上传验证；不修改错误转译、重试、调度、CDN、生产或其他任务包。实现：Caddy `read_body 15m` 覆盖慢速请求体读取；公共 fallback `response_header_timeout 15m` 仅覆盖完整请求写入 upstream 后的响应头等待；原生 128/128/16 MiB body limits 保持不变。审查修复将上传测试改为加载真实 `infra/Caddyfile`，只在测试派生配置中关闭证书签发、注入本地监听地址和 upstream；以 `/health` 成功反代作为 readiness，不依赖异步访问日志。验证：生产 `15m` 策略下 301 秒持续上传成功；合并后的 `main` 短模式连续 3 次稳定通过；从同一配置仅把 `read_body` 改为 `2s` 的派生测试证明不完整上传释放且未到达 upstream；Caddy validate/adapt、配置合同、发布控制器合同、Ruby 语法及 `git diff --check` 通过。完整 `tests/infra/validate-baseline.sh` 仍仅在既有无关 Dockerfile `ARG FRONTEND_NODE_MAX_OLD_SPACE_SIZE=1536` 合同不匹配处失败，未修改该无关问题；无数据库迁移。发布预期 `downtime_required=false`，但仍以宿主预检输出为唯一门禁；未完成生产验证前不得标记完成或启动 T02。
