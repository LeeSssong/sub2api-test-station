...
**T139 质量快照刷新与短窗口调度时效优化（2026-09-09）：** 状态 `IMPLEMENTING`。实施范围固定为：在已发布 T138 自购账号首选层与 API Key 冷启动调度之上，将统一质量统计范围改为最近 1 小时，拆分为互斥 `W5=[now-5m,now)` 与 `W55=[now-1h,now-5m)`；质量快照固定每 5 分钟刷新，真实请求完成写入有效结果后触发合并式即时刷新，每次调度读取最新已生成快照；继续使用 Sub 原生不可用/不可调度硬门禁，页面保持账号级排名，不新增模型级排名，不删除冷却账号展示。T138 的自购资源层、API Key 冷启动 priority 先验、priority=50 中性和真实质量接管规则优先，不得覆盖。正式规格：`docs/superpowers/specs/2026-09-09-one-day-quality-window-native-availability-ranking-design.md`。当前仅登记与实施准备，未修改运行时代码、配置、数据库、生产数据、合并或部署。
