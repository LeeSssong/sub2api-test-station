...
- **T37 渠道状态用户可见专属分组裁剪**：状态 `DONE`。已从根 `main@53d5fa19f` 生成 0600 证据 `/Users/gongtengxinwen/.codex/release-evidence/sub2api/2026-08-20-main-53d5fa19-t37.json`，宿主记录 `/var/lib/sub2api/release-records/20260820T035515Z-production-813018.json` 返回 `succeeded/promoted`、`downtime_required=false`、活动槽 `blue`；健康 `/healthz`、`/readyz`、`/health` 均 200，管理员版本接口 200。生产源/测试树 `53d5fa19fcdeeb9e0d5db13d96b0a7c6334b5779` / `b472d1aece4fd5c0216c0c86fafc37b0daabe199`。登录态 Monitor V2 两用户矩阵仍留给用户真机验收，不阻塞后续车道。
- **T38 可调度账号最近原生探测评分保留**：状态 `INTEGRATING`。候选已刷新到 T37 已部署后的 `main@beaf7aebd`，tip `46eb29a045`、tree `ba6d369648`；刷新后 service/handler/frontend focused tests、typecheck、compile、diff-check 和 protected-path guards 均通过。当前发布车道独占 T38；待授权合入、根门禁、推送、预检和无停机蓝绿验证。无迁移、配置或生产数据写入。
