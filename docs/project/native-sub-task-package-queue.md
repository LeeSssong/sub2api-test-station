...
- **T90 账号监控卡片真实请求证据、利润率与性能柱状图：** 状态 `IMPLEMENTING`（验收后修订）。验收站 `usage_logs=0` 说明成功率、TTFT P95 和真实性能柱状图没有真实用户请求样本；主动探测记录不得冒充真实请求。当前修订两点：利润率无真实流量时按分组售卖倍率与上游声明倍率计算预估值；账号详情改为 Sub 原生账号管理页面字段白名单，移除完整对象遍历及 `credentials`、`extra`、API Key、内部状态字段。完成直接相关测试后仅重新部署验收站，不部署主站。
