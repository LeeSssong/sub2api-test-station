...
- **T90 账号监控卡片真实请求证据、利润率与性能柱状图：** 状态 `VERIFYING`（等待管理员视觉验收）。修订保留在官方 `0.1.183` 主线并已推送、部署验收站 `main@07ac12e1b`，未部署主站。验收站 72 个账号均无真实请求样本，主动探测成功/TTFT 样本各 4611 次且未冒充真实请求；101 条分组账号投影中 88 条已按分组售卖倍率与上游声明倍率返回预估利润率，13 条缺少有效倍率保持待确认。账号详情使用 Sub 原生账号管理字段白名单，不渲染 `credentials`、`extra`、API Key 或完整对象。前端直接相关测试 55/55、类型检查、生产构建、验收镜像 Go 1.27 Linux/amd64 构建及六服务健康检查通过；公网验收健康与登录入口均 HTTP 200。证据：`/Users/gongtengxinwen/.codex/release-evidence/sub2api/2026-08-30-main-07ac12e1b-t90-acceptance-r3.json`。
