...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	emailQueueService := service.ProvideEmailQueueService(emailService, configConfig)
	...
	accountRepository := repository.NewAccountRepository(client, db, schedulerCache, auditLogService)
	...
	apiKeyAuthCacheInvalidator := service.ProvideAPIKeyAuthCacheInvalidator(apiKeyService, configConfig)
	quotaWalletService := service.NewQuotaWalletService(repository.NewQuotaWalletRepository(client, db))
	promoService := service.NewPromoService(promoCodeRepository, userRepository, billingCacheService, client, apiKeyAuthCacheInvalidator, quotaWalletService)
	...
	authService := service.ProvideAuthService(client, userRepository, redeemCodeRepository, refreshTokenCache, configConfig, settingService, emailService, turnstileService, tencentCaptchaService, aliyunCaptchaService, emailQueueService, promoService, subscriptionService, affiliateService, serviceUserPlatformQuotaRepository, quotaWalletService)
	userService := service.NewUserService(userRepository, settingRepository, apiKeyAuthCacheInvalidator, billingCache, quotaWalletService)
	redeemCache := repository.NewRedeemCache(redisClient)
	redeemService := service.NewRedeemService(redeemCodeRepository, userRepository, subscriptionService, redeemCache, billingCacheService, client, apiKeyAuthCacheInvalidator, affiliateService, quotaWalletService)
	...
	usageService := service.NewUsageService(usageLogRepository, userRepository, client, apiKeyAuthCacheInvalidator, quotaWalletService)
	...
	deferredService := service.ProvideDeferredService(accountRepository, timingWheelService, configConfig)
	...
	dashboardHandler := handler.ProvideDashboardHandler(dashboardService, dashboardAggregationService, accountProfitabilityService)
	...
	adminUserHandler.SetQuotaWalletService(quotaWalletService)
	...
	paymentService := service.ProvidePaymentService(client, registry, defaultLoadBalancer, redeemService, subscriptionService, paymentConfigService, userRepository, groupRepository, affiliateService, notificationEmailService, quotaWalletService)
	...
	accountFinancialRepository := repository.NewAccountFinancialRepository(client)
	accountFinancialUsageReader := repository.NewAccountFinancialUsageReader(client, db)
	accountFinancialService := service.ProvideAccountFinancialService(accountFinancialRepository, accountFinancialUsageReader, auditLogService)
	adminUsageHandler := handler.ProvideAdminUsageHandler(usageService, apiKeyService, adminService, usageCleanupService, accountFinancialService)
	...
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, monitorV2Handler, codexRadarInsightsHandler, codexRadarCommunityHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, idempotencyCoordinator, idempotencyCleanupService)
	...
	channelMonitorRunner := service.ProvideChannelMonitorRunner(channelMonitorService, settingService, configConfig)
