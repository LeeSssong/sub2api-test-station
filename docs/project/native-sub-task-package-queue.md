...
- **T37 渠道状态用户可见专属分组裁剪**：状态 `DEPLOYING`。用户补充反馈已在根 `main@8233974ac` 收口：Monitor V2 分组先复用 Sub 原生 `channel_monitor_v2_config` 的 enabled/group_ids（空 group_ids 保持原生“全部分组”语义），再按当前用户原生专属授权裁剪；时间线 tooltip 下置并删除标题解释文案。根合并后的 service/handler/routes、cmd/server build、前端 Monitor V2 13/13、typecheck、production build 与 diff-check 通过；待生成 0600 证据、运行发布预检并完成无停机蓝绿发布/线上验证。无迁移、无配置 schema 或生产数据写入。
