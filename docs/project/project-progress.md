...
**本轮 T03-R1 上游扣费缺失与异步持久化修复（2026-08-12）：** 状态：进行中（用户可见独立顶层任务已从干净 `main@747c7fb14d1ded243794a77984778babece7c799` 创建独立 worktree 与分支 `codex/t03-r1-upstream-cost-persistence`；只读生产量化、方案比较和三段设计已完成并获用户批准，正式规格书已写入，等待用户最终书面批准，尚未调用 writing-plans 或实施）。生产最近 500 条自然流水的精确 `upstream_request_id` 覆盖为 500/500（Sub 244、New 256），但持久化上游实际扣费/利润为 0/500，确认缺失根因是 `usage_logs` 没有相应字段且当前仅在管理员打开 `/api/v1/admin/usage/:id/upstream-cost` 时即时查询。用户选择方案 A：部署后新请求完成时复用现有 usage 异步记录链，执行一次 Sub `/api/v1/usage/records.actual_cost` 或 New `/api/log/token.quota` + `/api/status.quota_per_unit` 精确原生账单读取，并与流水原子持久化；完全不回填历史，不建立延迟补查或上游重试，不估算、不模糊匹配、不以 `account_cost` 冒充。不可漂移语义为：精确命中账单记录且实际扣费 blank/null/empty 时 `confirmed 0`；没有精确记录、端点不支持、鉴权/网络/解析失败时 `unavailable`。管理员列表、详情和兼容接口读取同一持久化事实，非管理员不可见。正式规格见 `docs/superpowers/specs/2026-08-12-t03-r1-upstream-cost-persistence-design.md`；未获用户明确批准书面规格前禁止 writing-plans 和实施，T05 继续暂停。
