...
- **T47-R1 operational timeline color correction**: status IMPLEMENTING. Production screenshot shows all `operational` buckets rendered amber because T47 added a frontend-only latency `>= 2000ms` degraded override. User requires every runnable/operational bucket to remain green. Scope is limited to `MonitorV2Timeline.vue` and its direct test; unavailable remains red and no-probe remains gray. No API, data source, migration, config, or production-data changes. Worktree: `.worktrees/t47-r1-operational-green`.
