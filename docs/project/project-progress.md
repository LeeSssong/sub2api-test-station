...
**本轮推进（2026-08-07）：** 管理页官方升级适配蓝绿生产拓扑继续保持“进行中”。提交 `ef6e12560` 已推送到工作分支与远端 `main`，包含过期迁移哈希断言修正和本地 `sub2api-release-metadata.rb discover --force-rebuild`；首次从隔离 Git worktree 执行候选合并时又发现脚本只接受 `.git` 目录，现已用失败回归复现并修正为同时接受合法 `.git` 文件，等待独立审查、提交与重新生成候选，尚未部署或线上验证。

**本轮登记（2026-08-07）：** 管理页官方升级适配蓝绿生产拓扑，状态：进行中（`0.1.171` 候选 readiness 已通过，但首次正式升级在旧单容器执行器的环境/容器识别预检阶段失败，未发生版本切换；正在恢复已有的严格蓝绿委托适配器与回归测试，完成推送、生产安装、重新升级及线上验证前不得标记完成）。

**本轮登记（2026-08-07）：** Sub2API `0.1.171` 生产候选登记，状态：准备完成（本地标签 `xingqiao-sub2api:upstream-0.1.171` 已指向合格镜像 `sha256:57436c65…`；更新器同一套生产解析器已返回 `ready=true`、`target_version=0.1.171`；官方版本/提交、完整资格标签、RepoDigest、二进制版本及 `linux/amd64` 架构均已核对；Sub2API 蓝/绿/worker、PostgreSQL、Redis、Caddy 容器身份、启动时间和重启次数均未变化；本次仅完成候选登记，尚未执行实际版本切换）。
