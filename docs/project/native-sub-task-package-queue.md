...
- **T77 授权 v4.1.1 指纹制品与检测历史清库**：状态 `INTEGRATING`。用户于 2026-08-27 明确确认持有 `chen-006/gpt56_api_detector` v4.1.1 的线下商业授权，并确认删除生产 `account_model_detection_runs` 的全部 `3,676` 条记录、不做迁移或回填。候选已合入根 `main@d023a32ea`：构建固定下载并校验 SHA-256 `70c0c2f092e66cd219f2384e08872e5bedb4559e427c2e320d0070186376f865` 的合规 v4.1.1 制品，通过既有私网 sidecar 合同返回受限 Juice/指纹摘要；不新建平行记录源、不保留密钥或原始响应。镜像、适配器、Go、前端、清库保护、Compose 与完整发布链契约均已通过，且没有迁移。发布前先执行预检；生产清库必须先导出受限备份、外键检查与锁表复核精确计数，只删除非 `4.1.1` 行。不得混入 T76 或其他调度任务，不使用 GitHub Actions。
