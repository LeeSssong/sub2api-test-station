...
**T12 新页面合同恢复对账（2026-08-17）：** 用户确认的 docs-only 规格修订为 `codex/account-probe-cost-design@50567e862`。原 T12 顶层任务已取消归档并完成只读对账：Task 1-3 的 append-only probe ledger、RESTRICT、精确小数、三类 probe、原生定价、fail-open、独立聚合、no-row unavailable 与 query-error report-level null 均兼容新合同；旧 Task 4 未提交 RED 的“独立 probe card/独立列”假设必须废弃。恢复必须从最新 `main` 建立干净候选，只移植 main 尚缺的 Task 1-3，重写“全站 -> 分组 -> 账号”一卡一账号布局及六项排序，桌面最多两列、390px 单列、无页面横向滚动，外部金额 USD 两位、利润率 0.00%。旧 `a54222c535` worktree 和唯一未提交测试继续只读保全。**状态：进行中（DESIGNING；待最小计划修订和干净候选恢复）。**
