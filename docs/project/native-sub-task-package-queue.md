...
**统一质量调度改用账号级原生 priority 且日常上限 50（2026-09-14）：** 状态 `INTEGRATING`。用户确认 A+C：统一质量日常优先级上限 50，读 `accounts.priority` 不读组关系 priority。候选 `feat/unified-quality-account-priority@e7f583d04f` 正在合入根 `main`。用户授权「快速部署主站，不同步独立测试站」并允许停机。工作区 `.worktrees/unified-quality-account-priority`。验收站按授权不同步。
...
**账号监控按当前调度投影排序（2026-09-13）：** 状态 `DONE`。用户确认方案 A：全站与分组打开即读当前调度投影并按其排序，质量分可见但不作主排序键，未投影平台账号排在已排名之后。候选已快进合入并推送根 `main@615203e3b9cd1efa19957c63919450a5ffceb9dc`、tree `9ebc0b36518e7cdc0386ed43bbbdf2e9efca298e`。直接相关后端测试、`go build ./cmd/server`、`git diff --check` 与前端 Vitest 62 项通过；无 migration。按用户授权「快速部署主站，不同步验收站」完成预加载蓝绿发布，宿主记录 `/var/lib/sub2api/release-records/20260913T141807Z-production-347345.json` 为 `succeeded/promoted`、`rolled_back=false`、`downtime_required=false`，活动槽 `blue`；公网 `/healthz`、`/readyz`、`/health` 均 HTTP 200。线上监控 8 组与全站均按 `scheduler_rank` 排序。验收站按授权未同步，当前 `944bbadfb0ec9268d7ab600bbd01d52ac427d0e8` / tree `b476ef25d9ca991c8b86091e4a2851ea88119e31`，两站明确不一致。测试证据 `/private/tmp/sub2api-release-evidence/2026-09-13-main-615203e3b9-monitor-scheduler-rank-production-only-fast.json`。本条不混入官方 v0.2.4 未完成 INTEGRATING 文档。

**2026-09-12 本轮合并修复完成：** 状态 `INTEGRATING`。官方 v0.2.4 为冲突权威：保留官方模型目录/白名单、Codex instructions、allowlist、WebSocket terminal/service tier/pending bare error 以及迁移合同；观测分支的旧 Codex manifest/cache/准入实现未直接移植，改为适配官方接口。已补齐官方 Ent runtime、handler/helper、WebSocket lifecycle、Grok `xhigh`、Anthropic/Antigravity 模型边界、Responses/OAuth system message、OpenAI fast service tier、OpenCode session header、session lineage、compact retry/proxy observability、Gateway error code、API-key slot release、T140 balance snapshot 与 repository 平台白名单。当前直接测试、构建、`git diff --check` 已通过；仍待提交、推送、主站发布和线上验证。

**2026-09-12 本轮续接：** 状态 `INTEGRATING`，唯一车道由根发布总控持有。按用户最新“冲突时以官方为准”继续现有合并，下面 BLOCKED 条目保留为首次失败历史；当前修复 v0.2.4 接口适配、测试 helper 与 Ent schema/生成代码不一致。主站快速发布获授权，独立测试站明确不同步；必须先通过直接验证并推送干净根 `main`，部署和验证成功后才能归档删除非 `main` worktree。尚未推送/部署/清理。

**2026-09-12 全部非 `main` 合并及主站快速发布：** 状态 `BLOCKED`。已盘点全部非 `main` worktree；官方 v0.2.4 与 T140 已在根 `main` 祖先链。合入 `codex/openai-responses-stream-observability` 的尝试因其旧 Codex 模型接口与 v0.2.4 的模型白名单/准入重构发生结构性冲突而无法编译，已保存恢复 bundle `/Users/gongtengxinwen/Documents/sub2api-archives/2026-09-12-all-non-main-rollout-blocked/merge-attempt.bundle`（SHA-256 `53c916dbae6e7ba613f07481e7d6c4e59438c83eeafe41b9bbe8970fd5aae5a2`）。根 `main` 已恢复为 `aa19ac7af0`；本轮未推送、未部署主站、未同步测试站、未清理原有非 `main` worktree/分支。

**官方 v0.2.4 增量同步（2026-09-12）：** 状态 `INTEGRATING`（本地已达到可推送/可部署状态，等待用户指令）。目标 `v0.2.4` 的 source commit 为 `5de5e2bed035d43591a2e10e51f420ef6a84eb98`，annotated tag object 为 `d681d0798064ee0ffff376d19687d12f09fe600f`，发布时间 `2026-09-09T06:43:55Z`。根 `main` 已先纳入 T140 `16c75aafd48475312987dcf2b4ebe39dd6bc2d48`，再快进纳入官方增量提交 `c47bbf985a58a8c5b0e809ba80d84452e8f9ec5a`、tree `0b6c404e68ea8f8b460f09af5bb979cc146eb4bb`；496 个文件变更，新增迁移 235/236/237，并保留星桥定制语义及 T140 的账号级倍率写入与原生 `unsupported` 后 NewAPI 兜底。官方更新例外下未运行功能测试、回归、类型检查或独立构建验证；冲突标记、人工冲突 Go 文件语法格式、版本/来源、敏感路径和 `git diff --check` 已核对。SSH 只读核对显示主站为 `82f8c58a359c616021fce837c460f99a015fe39b` / tree `59810fbc921abf6cf6f638a752d64ddb85b07286`，独立验收站为 `944bbadfb0ec9268d7ab600bbd01d52ac427d0e8` / tree `b476ef25d9ca991c8b86091e4a2851ea88119e31`，两站互不一致且均未运行本地候选。当前未推送、未执行发布预检、未构建部署制品、未部署；未复制任何数据、凭据或运行卷；`.worktrees/openai-responses-stream-observability` 为保护 worktree，保持原样。
...
**T139 质量快照刷新与短窗口调度时效优化（2026-09-09）：** 状态 `DONE`。在已发布 T138 自购账号首选层与 API Key 冷启动调度之上，将统一质量统计范围改为最近 1 小时，拆分为互斥 `W5=[now-5m,now)` 与 `W55=[now-1h,now-5m)`；质量快照固定 5 分钟刷新，真实请求完成写入有效结果后触发合并式即时刷新，每次调度读取最新已生成快照；继续使用 Sub 原生不可用/不可调度硬门禁，页面保持账号级排名，不新增模型级排名，不删除冷却账号展示。T138 的自购资源层、API Key 冷启动 priority 先验、priority=50 中性和真实质量接管规则保持不变。候选 `codex/t139-quality-freshness` 已合并为根 `main@82f8c58a359c616021fce837c460f99a015fe39b`、tree `59810fbc921abf6cf6f638a752d64ddb85b07286`，并推送 `origin/main`。直接相关仓储测试、`go build ./internal/service`、`go build ./cmd/server`、gofmt 与 diff-check 通过；服务包定向测试仍受主线既有 `resolveCompositeModelOwnership`、`decodeCodexManifestModels` 缺失符号阻断，未修改无关代码。无 migration、配置或生产业务数据变化。按用户明确授权“合并到main，推送远端，部署主站，不同步测试站”完成主站预加载蓝绿发布，宿主记录 `/var/lib/sub2api/release-records/20260909T153457Z-production-2188216.json` 为 `succeeded`、`rolled_back=false`、活动槽 `blue`、`downtime_required=false`，主站 `/healthz`、`/readyz`、`/health` 均返回 200。测试站按授权未同步，主站与测试站版本可能不一致，不声称两站一致。恢复 bundle `/Users/gongtengxinwen/Documents/sub2api-archives/2026-09-09-t139-quality-freshness-production-only-fast.bundle` 已通过 `git bundle verify`，SHA-256 `510d57d22df45e26acff0120e4a8244d5527923d32d9218dc219b7f8ffda5dd6`；候选 worktree、本地和远端候选分支均已删除。

**T138 自购账号首选层与 API Key 冷启动调度（2026-09-09）：** 状态 `DONE`。用户已批准规格 `docs/superpowers/specs/2026-09-09-t138-self-owned-tier-and-api-key-cold-start-scheduling-design.md` 并要求开始实施。候选 `.worktrees/t138-self-owned-tier-cold-start` / `codex/t138-self-owned-tier-cold-start` 将从登记时已推送且干净的最新 `main` 创建。范围固定为：在现有统一普通文本 OpenAI HTTP 调度中，先经过原生硬门禁，再将自购账号置于首选资源层；自购账号不伪造质量评分，层内按原生 priority/负载/公平性排序；无可用自购账号时进入 API Key 层；API Key 质量证据不足时 priority 提供有界冷启动先验，真实质量积累后接管。默认 priority=50 保持中性；不新增账号归属事实源、质量系统、迁移或配置；不改变生图、Responses WebSocket、alpha-search、账务、重试、安全重放、原生槽位和部署授权边界。相邻 24h 质量窗口规格未获批准，不纳入本任务。候选已合入并推送根 `main@10a5dcb416cca6b42bc7dafe7169a1dc60b82f34`；主站按用户授权“快速部署主站，不同步验收站”发布成功，宿主记录 `/var/lib/sub2api/release-records/20260909T120040Z-production-1941807.json`，公网 `/healthz`、`/readyz`、`/health` 均 HTTP 200。验收站按授权未同步，当前版本仍为 `3f7d59df5aa6f511c64a8663a8f74b8c2b50b3ed` / tree `e049847dc3aba0296ea4e84a66f5caeda5df9da1`，两站明确不一致。测试证据 `/Users/gongtengxinwen/.codex/release-evidence/sub2api/2026-09-09-main-10a5dcb41-t138.json`。

**T137 账号卡片账号级统一有效观测修复（2026-09-08）：** 状态 `DONE`。规格与计划已获用户确认并直接实施。候选 `codex/t137-account-card-account-observations@dc3fb13de` 已合入；最终根 `main@fb966ff44696ab9811a0947daf4130967370bdaa`、tree `7b91f906f5070078a8d09ec7d6585f2dba436725` 已推送且与 `origin/main` 一致。账号卡片只消费账号级数据；有效观测为去重后的真实逻辑请求终态与去重后的主动探测终态直接相加，同桶不互斥；真实成功不再依赖 `actual_cost > 0`。直接相关后端仓储/分组真实请求测试、前端账号卡片 26/26、生产构建、Linux amd64 后端/model-detector 编译和差异检查通过；无 migration、配置或生产业务数据变化。用户已授权“快速部署主站，不同步测试站”，主站已完成 `succeeded/promoted` 蓝绿发布，`rolled_back=false`、`downtime_required=false`，活动槽 `blue`，公网三项健康探针通过。发布证据 `/Users/gongtengxinwen/.codex/release-evidence/sub2api/2026-09-08-main-fb966ff44-t137.json`，宿主记录名 `20260908T115516Z-production-281913.json`；独立测试站按授权未同步，记录基线 `3f7d59df5aa6f511c64a8663a8f74b8c2b50b3ed` / `e049847dc3aba0296ea4e84a66f5caeda5df9da1`，两站明确不一致。候选恢复 bundle `/Users/gongtengxinwen/Documents/sub2api-archives/2026-09-08-t137-account-card-production-only-fast/t137-account-card-account-observations.bundle` 已验证，SHA-256 `6e0ad44a1e25c36a7d408d1c9357c7b283d804937879d432a74ea5d19cd69e6d`。

**管理员手工充值确认按钮修复（2026-09-08）：** 状态 `DONE`。候选 `1d786c26c7bfeffdacdad297b94f694fe092daff` 已合入并推送根 `main@ab5d7ab47d6430c3e5432331276dbad81a6cf2c8`、tree `b6aaf9623a089042783191e9dfafa4a25c1565bc`；充值弹窗打开时自动生成符合后端审计/幂等合同的管理员交易号，仍允许人工覆盖，退款行为不变。直接相关 Vitest 7/7、typecheck、production build 与 diff-check 通过，无迁移、配置或生产业务数据写入。按用户明确授权“快速部署主站，不同步测试站”完成预加载蓝绿发布，宿主记录 `/var/lib/sub2api/release-records/20260908T040913Z-production-3924768.json` 为 `succeeded/promoted`、`rolled_back=false`、`downtime_required=false`，活动槽 `green`，公网三项健康探针通过；独立测试站按授权保持 `861a049af593724c0eb16041082be6bcecb5a649` / tree `a25999d47a2fefd4c07caa2fcd9e8543df34407d`，两站明确不一致。未执行真实充值；浏览器专项登录态检查因连接超时未形成页面证据。恢复 bundle `/Users/gongtengxinwen/Documents/sub2api-archives/2026-09-08-admin-recharge-production-only-fast/admin-recharge-autotrade.bundle` 已验证，SHA-256 `55fe390b5cf6b3017e2d5ffba2030f89880037f8f4423cc2b8aaf450ec8d115a`；候选 worktree、本地分支和远程分支均已删除。

**不支持模型错误诊断修复（2026-09-07）：** 状态 `DONE`。根 `main@ae352630845190612dda9e8e620fa7763024da45`、tree `e7da83be2e49637381d70c6e8a2905a98a0b3e8f` 已推送并按用户明确“快速部署主站，不同步测试站”完成主站发布；宿主记录 `/var/lib/sub2api/release-records/20260907T165249Z-production-3120823.json` 为 `succeeded/promoted`、`downtime_required=false`、活动槽 `blue`，公网健康通过。测试站按授权未同步，版本差异已保留；无迁移、配置或业务数据变化。恢复 bundle 已归档并验证：`/Users/gongtengxinwen/Documents/sub2api-archives/2026-09-07-unsupported-model-diagnostics-production-only-fast/unsupported-model-diagnostics.bundle`，SHA-256 `8fffb0e5190054f1b346d6b2aa9c332d2f049dcbc1d11bc6ddbf59f6f1ab408c`；候选 worktree/本地分支已删除。

**NewAPI 余额告警与原生临时不可调度修复（2026-09-08）：** 状态 `DONE`。候选 `5cc6a0ac236c164eabef14d3d9d05b6f6f6ba8e1` 已合入根 `main@28b9ed921fe23a8db6a45a3318ad16567e58be6d`，tree `8307eb6f3ae3b072804a931014e62fd6f71fa768` 并推送。NewAPI 正余额低于 5 不再发送低额提醒；`unlimited_quota=true`、HTTP 429、超时、字段缺失和解析失败不判定为零余额；成功有限配额 `total_available=0` 或明确额度耗尽错误才保留零余额告警，并接入 Sub 原生 `SetTempUnschedulable`，恢复后仅清除本功能设置的余额故障状态。按用户明确授权“快速部署主站，不同步测试站”从干净根 `main` 使用预加载蓝绿链发布，结果 `succeeded/promoted`、`downtime_required=false`、`rolled_back=false`、活动槽 `green`，宿主记录 `/var/lib/sub2api/release-records/20260907T161824Z-production-3074272.json`；公网 `/healthz`、`/readyz`、`/health` 均通过。验收站未同步，版本差异已记录；无迁移、配置或生产业务数据写入。`go build ./cmd/server`、gofmt、`git diff --check` 通过；直接 Go 测试被主线既有缺失符号阻断，未修改测试绕过。恢复 bundle `/Users/gongtengxinwen/Documents/sub2api-archives/2026-09-08-newapi-balance-native-temp-unschedulable/newapi-balance-native-temp-unschedulable.bundle`，SHA-256 `1ff6d84e7e8cc4b6c5219693ce8b32c99d084d97492f0d762b54e1c987610553`，已验证并完成 worktree/分支清理。

**官方 v0.2.1 增量同步（2026-09-06）：** 状态 `INTEGRATING`。官方最新稳定 tag 已确认为 `v0.2.1@adc26f68f687685e847bfb997559f48e79cac475`；当前定制根线为干净且已推送的 `main@376ef10da8d1bfb5ccb914c72dc8a84cc822d7b0`，无其他非 `main` worktree。按用户明确要求，将官方差异人工增量合入并保留本站定制内容，随后从同一已推送根 `main` 更新主站和独立测试站；本任务作为官方更新最高优先级插队并独占整合/部署/验证车道。官方更新不运行额外功能测试、回归、类型检查或独立构建验证，只保留发布链内置候选构建、迁移/停机保护、就绪检查及原子回滚。独立测试站现为 `957c2506e340dc883744a0b3180ce1672c9e6d72` / tree `8f4d374d761b017d9ee08e9774f1f2e547b4a9eb`；禁止复制两站数据、账号、凭据或运行卷。

**T133 账号监控连接测试模型入口与主动探测成功率修复（2026-09-05）：** 状态 `DONE`。候选 `45177506d` 已推送并无冲突合入根 `main@b4c36cfeb432de84ba5f60fdc177bdb7b7e2900e`（tree `34ff36e28bcae40012ca082578f420e396c0abe7`）；连接测试模型原生选择框已置于模型结果与自动化勾选项旁，懒加载选项、即时保存、失败回滚；统一聚合已让 `success/failed` 探测终态都进入分母，同桶真实请求优先、最新探测去重，分组质量不再被旧业务窗口零成功率覆盖。直接相关后端/前端测试、typecheck、server build、production build 和 diff-check 通过，无 migration、配置或生产数据写入。按用户明确“快速部署主站，不同步验收站”完成主站发布，宿主记录 `/var/lib/sub2api/release-records/20260905T080800Z-production-3262987.json` 返回 `succeeded/promoted`、`rolled_back=false`，公网 `/healthz`、`/readyz`、`/health` 均 HTTP 200；发布证据 `/Users/gongtengxinwen/.codex/release-evidence/sub2api/2026-09-05-main-b4c36cfeb-t133-production-only-fast.json`。验收站按授权未同步，当前 `957c2506e340dc883744a0b3180ce1672c9e6d72` / tree `8f4d374d761b017d9ee08e9774f1f2e547b4a9eb`，两站明确不一致；浏览器页面专项读取因连接超时未形成通过证据，待后续登录态复核。候选恢复 bundle 已归档并验证，随后删除 worktree/本地分支。
...
**T131 账号监控探测终态恢复（2026-09-04）：** 状态 `DONE`。候选 `codex/account-monitor-probe-terminus@bb0846151` 已合入根 `main@0d1f1e151bbe25b635e6d411e52eadd2ff61749e`，tree `fc54b9f36ffc316dd415271d019208b8374fe20e`，并推送 `origin/main`。修复主动探测终态投影、真实请求优先与探测兜底边界，避免“待确认/灰柱/证据不足”错误回流；直接相关测试、类型检查、后端构建和差异检查通过。按用户明确“快速部署主站，不同步验收站”完成主站蓝绿发布：`succeeded/promoted`、`downtime_required=false`、活动槽 `blue`，镜像 digest `sha256:21083dacbae3f57c0136b2ed31cd17192b99da5a7f1af40d49ebeeb62b502973`；公网 `/healthz`、`/readyz`、`/health` 均通过。验收站未同步，版本差异保留并已明确记录。恢复 bundle 已归档并通过 `git bundle verify`：`/Users/gongtengxinwen/Documents/sub2api-archives/2026-09-04-account-monitor-probe-terminus-production-only-fast/account-monitor-probe-terminus.bundle`，SHA-256 `3bf0f7a143478b53eb38bbe49ae49c105b8fca22602dfe9520d6fa624fe85bcf`；worktree/分支已删除。
...
**验收站指向纠偏（2026-09-03）：** 当前验收目标是独立服务器 `ubuntu@49.51.203.200`（SSH alias `sub2api-test-station`），入口 `http://49.51.203.200/`，Compose project `sub2api-test-station`，部署根目录 `/opt/sub2api-test-station/`，Compose 文件位于活动 release 的 `infra/independent-test-station/compose.yaml`。旧主站 `/admin/lab/`、`sub2api-acceptance`、`/opt/sub2api/acceptance-live`、`sub2api-prod` 和旧验收 env 仅为历史记录，不是本轮验收入口或发布目标。仓库旧 `ops/release-sub2api-acceptance.sh` 尚未证明支持新站，禁止据此发布新站；本次纠偏不部署、不清理远端数据、不修改业务代码。

**T128 性能指标与切号恢复纠偏（2026-09-03）：** 状态 `DESIGNING`。已完成只读生产根因核查和正式规格初稿 `docs/superpowers/specs/2026-09-03-t128-monitor-metrics-and-failover-correction-design.md`，范围拆为三个串行可验收任务包：Monitor V4 快照 `16/19` 列漂移修复、V4 标准 P95/缓存展示口径修正、OpenAI 跨故障域切号优化。已明确先修快照刷新根因；不增加未经验证的过期降级逻辑；不修改 Sub 原生控制面板、原生 Monitor V2、原生全站缓存汇总或原生账务统计。规格待用户审阅批准；当前不调用 writing-plans、不创建实现 worktree、不修改运行代码、不合并、不推送、不部署。
...
**T123/T124 Responses 语义切号与飞书余额范围隔离（2026-09-03）：** 状态 `DONE`。候选 `codex/t123-t124-pro-spec-and-pro-metric-audit@9272917df` 已合入、随根 `main@2fd63bb934f889c2529152b792b8ad09e7dcf06d` 推送，并以预加载蓝绿链快速部署主站。直接相关 service/handler 测试、`go build ./cmd/server`、`git diff --check` 通过；无迁移。宿主返回 `succeeded`、`downtime_required=false`，活动槽 `green`，API 与 worker 镜像为 `ghcr.io/leesssong/xingqiao-sub2api:release-2fd63bb934f889c2529152b792b8ad09e7dcf06d-828ce7055576911e169b0100fb687a2d619d093e4085f1d7211460a7a1a1949d`，公网 `/healthz`、`/readyz`、`/health` 均成功。按用户授权验收站未部署、版本可能不同。发布证据 `/Users/gongtengxinwen/.codex/release-evidence/sub2api/2026-09-03-main-2fd63bb9-t123-t124-production-only-fast.json`；完整历史 bundle 已归档并验证至 `/Users/gongtengxinwen/Documents/sub2api-archives/2026-09-03-t123-t124-production-only-fast/t123-t124-pro-spec-and-pro-metric-audit.bundle`，SHA-256 `11ab1013b32c3da281cef29461a99c8d4cf2d37092d8d23b9b69823cf42490e6`；worktree 与本地分支已清理。
...
**T119 OpenAI 调度决策日志（2026-09-03）：** 状态 `DONE`。候选 `codex/t119-openai-scheduler-decision-log@63706322b` 已合入并从干净、已推送的根 `main@78cd279fd8b985b515834a7e609cde754674522d`（tree `377fc03bd5ab8e2f52be2078a806cbb7eb1c61ae`）完成已授权维护发布。管理员列表按逻辑请求聚合/分页，详情展示服务端实际算法版本、原生运行时重试预算、真实切号次数、候选/排除/评分/冷却/失败链；有界尽力异步记录不阻塞请求，队列满/写入失败暴露不完整计数，且不再读取已失效 `extra_retry_count`。生产记录 `/var/lib/sub2api/release-records/20260903T032637Z-production-4108134.json` 为 `succeeded/promoted`、未回滚，活动槽 `green`；release state 的 commit/tree/migration hash 精确匹配，`234_openai_scheduler_logs.sql` 已生效且表 `openai_scheduler_logs` 存在。公网 `/healthz`、`/readyz`、`/health` 均 HTTP 200，green、worker、model-detector、PostgreSQL、Redis 均 healthy 且重启计数 0，匿名调度日志接口返回预期 401。按用户明确要求未同步测试站。候选完整历史已归档至 `/Users/gongtengxinwen/Documents/sub2api-archives/2026-09-03-t119-openai-scheduler-decision-log/t119-openai-scheduler-decision-log.bundle`，SHA-256 `e3b9500e467af700305a35f89dfcb16b016083c2ce4c8b56f9ff0733a439a9a6`；T119 worktree 与本地分支已删除。0600 发布证据为 `/Users/gongtengxinwen/.codex/release-evidence/sub2api/2026-09-03-main-78cd279fd-t119-maintenance.json`。
