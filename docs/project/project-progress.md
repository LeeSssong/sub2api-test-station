...
**T11-R1 Sub 原生计费聚合经营页纠偏登记与移交（2026-08-16）：** 状态：进行中（`INTEGRATING`，`P0/URGENT`）。候选 `/Users/gongtengxinwen/.codex/worktrees/ec64/sub2api搭建`（`codex/t11-r1-native-accounting-profitability@86d5c4cd41245e1adf98cb1dc52200044de38036`，implementation `c5df650cd4c1ab2f8dcd6982f31ffd842bf303a9`）完成 Tasks 1–4、逐任务复审、两轮前端修复复审、fresh whole-branch review 和 candidate handoff，随后由根总控按 `AUTHORIZE_MERGE_TO_MAIN` 合入 `main@b1cb220b1d8280aaace9a617152dc48e75020786`。merged-main 后端 focused 4 组、repository/service/admin/server neighboring 全包、Colima 真 PostgreSQL integration、前端 18/18、typecheck/build、diff/protected/forbidden/config/dependency/migration/workflow、发布脚本语法均通过，whole-branch reviewer Critical/Important 为 0，unexpected dirty 为 0，root 仅保留 handoff 指定的受保护内容。`downtime_required=false`；authenticated 390×844 与 31 天生产尺度 API latency/query-plan 被 fresh reviewer 归类为部署后的 `VERIFYING` 门禁，关闭前不得标记 DONE。当前等待根发布账本提交、clean release worktree final-tree evidence、推送与蓝绿部署。经营语义保持：`requests`、`tokens`、账号计费 `cost = SUM(COALESCE(account_cost, COALESCE(account_stats_cost, total_cost) * COALESCE(account_rate_multiplier, 1)))`、用户扣费 `user_cost = SUM(actual_cost)`，利润 `user_cost - cost`，美元单位；用户未消费余额保留为独立只读 CNY 卡。T03/T03-R1 历史 evidence/review/OAuth/override 能力保留但不再作为经营页取数源；仅移除已批准的页面控件/入口。T09 与 S1-R2 继续暂停。
