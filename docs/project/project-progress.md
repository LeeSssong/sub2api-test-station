...
**T08“暂不建议入组”轻提示（2026-08-15）：** 状态：进行中（`DESIGNING`）。已确认 T07 已完成生产发布并线上验收；已盘点非 `main` worktree：`codex/t07-global-score-weights` 为已部署候选的历史证据，`official-v0175-fast-merge` 与 `upstream-resilience-hardening` 均为未完成或含未提交内容的旧候选，不具备“已推送 + 已部署 + 已验证”完成证据，按冻结/保护规则保留，不合并、不修改、不清理。T08 目标仅为将“暂不建议入组”原因从常驻文本收敛为桌面悬浮/点击、移动端点击的 1–2 行轻提示；不改推荐算法、分组迁移或其他卡片布局。当前等待用户可见独立顶层任务完成完整 brainstorming、书面规格书与门禁审查；未写计划、未实施、未派实现代理、未合并、未部署。
