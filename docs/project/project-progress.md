...
**T13 NewAPI 上游倍率自动登记恢复（2026-08-16）：** P0 会话热修已生产收口，按持续目标恢复用户可见顶层任务 `01a00969-b2ea-7ff0-9f49-d7af64438e00`。实施前已盘点全部非 main worktree：历史 release 候选均已为当前 main 祖先，T12/T13/S1 独有提交都是未完成或冻结候选，不存在必须先合并的已完成领先分支。T13 把原 17 个 Task 2/3 文件收口为候选 `c8ec34498`，随后无冲突整合根 `main@0fc6bb9c7` 得到刷新候选 `8faf65547`；该候选已无冲突合入根 `main@9c5d30d40`。合并树的 3 个 focused service 用例、2 个 repository CAS 用例、相关包 compile-only、gofmt 与运行时代码 diff-check 均通过；无迁移、配置、依赖或工作流变化。T12 继续冻结，待 T13 推送、部署和线上验收后再恢复。**状态：进行中（INTEGRATING；待推送和发布预检）。**
...
**T13 NewAPI 上游倍率自动登记（2026-08-16）：** 状态：进行中（`INTEGRATING`）。用户已批准规格提交 `f2fc807d4fbca5f5917a00b3fadb890061cf3522`；范围仅限 NewAPI API-key 且没有原生 Sub 倍率声明的账号，以精确匹配日志 `other.group_ratio` 为权威倍率：首次真实成功请求后登记到 `accounts.rate_multiplier` 并在 `accounts.extra` 标记，已登记账号每个北京时间自然日仅首笔合格请求刷新一次；必须具备并发 CAS、失败不覆盖和管理员可见“已登记”。不做迁移、历史回填或生产修改。刷新候选 `8faf65547` 已合入根 `main@9c5d30d40` 并通过直接相关功能测试；待推送和发布预检。
