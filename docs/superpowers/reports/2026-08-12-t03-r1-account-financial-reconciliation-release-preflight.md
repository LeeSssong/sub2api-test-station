...
状态：`READY_FOR_ROOT_REVIEW`；`downtime_required=unverified until root preflight`。
...
- final review 已为 APPROVE；管理员 handler 的 read-time upstream fallback 已移除，缺失本地财务服务时 fail-closed。
