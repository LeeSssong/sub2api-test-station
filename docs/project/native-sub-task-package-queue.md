...
- 队列状态：T01、T02、T03、T04 已部署并完成各自既定线上验证。T03-R1 的用户可见独立顶层任务已完成书面规格、实施、独立复审、全分支终审、根授权合并、推送和合并后专项验证；它以独立逐笔证据、异常核对、账号财务首页、OAuth 每日成本和查询时本地汇总代替直接扩展 `usage_logs`。当前宿主蓝绿预检因新增迁移集合返回 `downtime_required=true` / `migration_set_changed`，已在任何候选启动、流量切换、停服务、迁移或重启前停止。T03-R1 的生产验收尚未完成，必须等待用户明确授权“允许停机部署”；在此之前不得启动 T05 或后续任务包。
...
- 当前状态：进行中（已合并、已推送、用户已授权停机部署；维护发布器缺少精确迁移转换，尚未部署）。候选 `2e28308a04495482fbe16af05344111bb4a0b0a7` 已合并为 `main@dbe1231e0243d7190996f21cd8b8e5ac17b8420a`；2026-08-14 核验时 `main`/`origin/main` 为 `fbd85f608fb6519a52e87b6a8d71011efcfa90a2`。活动迁移集 `f1b1…` 到 T03-R1 集 `6a0e…` 不在受审宿主执行器的 allowlist；即使用户已授权停机，发布器仍会在停服前以 `migration_set_changed` fail-closed。必须先以独立用户可见顶层维护任务增加、测试、复审唯一精确转换，再从其合并后的 `main` 重新验证和发布。生产未变更，旧候选 `1c3e87684`、`ce5691527` 保持冻结，不得恢复。
...
- T03-R1 已获得用户“允许停机部署”授权，但受审维护发布器尚无活动迁移集 `f1b1…` 到候选集 `6a0e…` 的精确 allowlist；必须先通过独立用户可见顶层维护任务补齐、测试和复审该唯一转换，再使用受审维护发布路径。旧计划和冻结候选不得恢复。
