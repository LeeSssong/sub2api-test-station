...
**全局无停机发布授权门禁（2026-08-17）：** 用户明确指令：发布预检返回 `downtime_required=false` 时不需要再次取得人工授权，唯一发布总控应直接继续既有本地/宿主蓝绿发布与线上验证；只有返回 `downtime_required=true` 时，才必须在任何停服、迁移、重启或切换前暂停并请求明确授权。此前“先不部署”的全局冻结现已解除；T15、T16 和历史冻结/受保护 worktree 的独立保护状态不因此解除。S2 已合入并验证根 `main@d1f9bc06c`，当前进入推送和发布预检，继续独占发布车道。**状态：进行中。**
