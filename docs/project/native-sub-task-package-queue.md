...
- **T38 可调度账号最近原生探测评分保留**：状态 `REFRESH_REQUIRED`，等待 T37 已部署后的最新根 main。候选 `codex/t38-retain-native-probe-score@8ccea77f8` 已完成规格、计划、TDD 实现、直接后端/前端合同测试和交接；刷新后继续保持当前 `unavailable|stale` 状态与所选窗口原生评分/排名分离，纯失败/无样本仍不评分。无迁移、配置或生产数据写入，候选预期 `downtime_required=false`。
