...
**本轮 T04 账号监控移除外部控制面状态（2026-08-12）：** 状态：进行中（正式规格书已获用户批准，实施计划已写入并获用户“写完后直接开始实施”授权；尚未完成实现、复审或发布）。独立工作区为 `/Users/gongtengxinwen/.codex/worktrees/72c7/sub2api搭建`，分支 `codex/t04-account-monitor-native-only`，基线 `eaa73610238f1effc253dc84a98eae4965390525`。已确认账号监控在首次加载、时间窗切换、刷新和写后重载中始终以 `/admin/accounts/monitor` 为唯一页面数据源且对 `/xingqiao/**` 零请求；删除页面级控制面投影、状态和状态条，不新增替代常驻状态，不改变现有卡片、布局与交互；共享控制面文件留给严格串行的 T05/T06。原生失败继续保留既有快照/首次错误空态和重试语义。验证按用户要求裁剪为账号监控专项测试、最低必要构建检查和一次线上 MVP 冒烟。正式设计见 `docs/superpowers/specs/2026-08-12-t04-account-monitor-native-only-design.md`，实施计划见 `docs/superpowers/plans/2026-08-12-t04-account-monitor-native-only.md`；最终交付需要在根任务发送包含目标 `main` SHA 的 `AUTHORIZE_MERGE_TO_MAIN` 后完成合并、推送、既有本地/宿主链部署和线上验证；不得使用 GitHub Actions。
