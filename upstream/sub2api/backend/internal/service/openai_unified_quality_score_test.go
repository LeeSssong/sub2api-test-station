...
func TestOpenAIUnifiedQualityScoreUsesVersionedFixedComponents(t *testing.T) {
	...
	require.Equal(t, "t139-v1", OpenAIUnifiedQualityScoreVersion)
	...
	require.InDelta(t, 1, got.Windows[OpenAIQualityWindow5M].Confidence, .001)
}

func TestOpenAIUnifiedQualityScoreUsesRecentFiveMinuteAndFiftyFiveMinuteWindows(t *testing.T) {
	quality := OpenAIAccountQuality{AccountID: 19, Windows: map[OpenAIQualityWindow]OpenAIQualityWindowMetrics{
		OpenAIQualityWindow5M: {
			AttemptCount: 5, SuccessCount: 5, SuccessRate: floatPtr(1),
			TTFTSampleCount: 5, TTFTP50MS: floatPtr(2000), TTFTP90MS: floatPtr(5000),
		},
		OpenAIQualityWindow55M: {
			AttemptCount: 50, SuccessCount: 25, SuccessRate: floatPtr(.5),
			TTFTSampleCount: 50, TTFTP50MS: floatPtr(20000), TTFTP90MS: floatPtr(40000),
		},
	}}
