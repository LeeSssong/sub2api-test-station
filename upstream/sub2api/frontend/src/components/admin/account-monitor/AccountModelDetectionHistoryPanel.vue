      ...
      <div class="flex min-h-0 flex-1 flex-col p-4 sm:p-5">
        ...
        <div class="grid grid-cols-1 gap-2 rounded-lg border border-slate-600 bg-slate-800/80 p-3 sm:grid-cols-3" data-test="detection-history-filters">
          <label class="grid gap-1 text-[10px] text-slate-300" for="detection-history-juice-filter">{{ t('admin.accounts.modelDetection.juiceFilter') }}
            <select id="detection-history-juice-filter" v-model="juiceFilter" class="h-8 rounded-md border border-slate-600 bg-slate-700 px-2.5 text-xs text-slate-100 outline-none transition focus:border-cyan-400 focus:ring-1 focus:ring-cyan-400/30" data-test="detection-history-juice-filter"><option value="">{{ t('common.all') }}</option><option value="verified">{{ juiceLabel('verified') }}</option><option value="mismatch">{{ juiceLabel('mismatch') }}</option><option value="insufficient">{{ juiceLabel('insufficient') }}</option><option value="non_gpt">{{ juiceLabel('non_gpt') }}</option></select>
          </label>
...
function juiceTranslationStatus(status?: string) { return status === 'verified' ? 'pass' : status }
function juiceLabel(status?: string) { const translated = juiceTranslationStatus(status); return translated ? t(`admin.accounts.modelDetection.juiceStatus.${translated}`) : t('admin.accounts.modelDetection.evidenceUnavailable') }
...
function reasonLabel(item: AccountModelDetectionSummary) {
  if (!item.trigger_reason) return t('admin.accounts.modelDetection.historical')
