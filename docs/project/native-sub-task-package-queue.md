...
- **T39 Responses 流式 413 二次错误映射修复（快速迭代-10）**：状态 `DESIGNING`。独立顶层任务 `01a031e3-1d55-7351-a6e3-fd0103ba4fbf`、worktree `/Users/gongtengxinwen/.codex/worktrees/98b0/sub2api搭建` 已从最新 `main` 启动。范围为修复应用内入站/上游 413 在 JSON 与 Responses SSE 路径被二次 `ProjectNativeUserError` 覆盖的问题，保持中文“请求内容过大”语义、错误类型、协议终止事件和脱敏；不处理 Cloudflare 已在边缘生成且未进入应用的 HTML 413，不并入 T40。候选不得自行修改全局总账、合并、推送或部署。
