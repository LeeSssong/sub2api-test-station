    ...
    settings: {
      ...
      features: {
        channelMonitor: {
          ...
          modeV2: 'V2 被动监控',
          modeV1: 'V1 主动探测',
          modeNativeProbe: '原生主动探测',
          modeHybrid: '混合性能监控',
          modeV2Hint: '需主动选择：基于真实网关流量聚合健康指标，不向上游发送探活请求；启用期间 V1 探测停止。',
          modeV1Hint: '默认模式：按配置的渠道监控项定时发起上游健康检查（产生探测流量）。',
          modeNativeProbeHint: '原生页面：使用原生主动探测监控，不改变探测口径。',
          modeHybridHint: '混合模式：主动探测与真实请求统一聚合，按五分钟计算可用性和性能 P95。',
      ...
      openaiExperimentalScheduler: {
        ...
        topKLabel: '候选数',
        topKHint: '每次先保留得分最高的候选数；数值越小越省探测开销，但可能长期忽略低分账号。公平模式仍可把长期未用账号加入候选。范围：正整数（>= 1）；留空继承配置/默认值。',
        priorityWeight: '优先级',
        priorityWeightHint: '账号 priority 的权重；越大越偏向优先级数字更小的账号，0 表示不参与排序。范围：非负有限数（>= 0，支持小数）；留空继承配置/默认值。',
        loadWeight: '负载',
        loadWeightHint: '当前并发负载权重；越大越避开高负载账号，降低单账号拥塞。范围：非负有限数（>= 0，支持小数）；留空继承配置/默认值。',
        queueWeight: '排队',
        queueWeightHint: '等待队列权重；越大越优先选择排队更短的账号。范围：非负有限数（>= 0，支持小数）；留空继承配置/默认值。',
        errorRateWeight: '错误率',
        errorRateWeightHint: '近期错误率权重；越大越避开错误率高的账号。范围：非负有限数（>= 0，支持小数）；留空继承配置/默认值。',
        ttftWeight: '首字',
        ttftWeightHint: '近期首 token 延迟权重；越大越偏向响应更快的账号。范围：非负有限数（>= 0，支持小数）；留空继承配置/默认值。',
        resetWeight: '重置',
        resetWeightHint: '会话窗口即将重置的权重；越大越优先消耗临近重置的额度，0 表示关闭。范围：非负有限数（>= 0，支持小数）；留空继承配置/默认值。',
        quotaHeadroomWeight: '额度余量',
        quotaHeadroomWeightHint: '额度余量权重；越大越偏向剩余额度更充足的账号，0 表示关闭。范围：非负有限数（>= 0，支持小数）；留空继承配置/默认值。',
        upstreamCostWeight: '上游成本',
        upstreamCostWeightHint: '上游计费倍率权重；越大越偏向低倍率账号，0 表示不考虑倍率。范围：非负有限数（>= 0，支持小数）；留空继承配置/默认值。',
        previousResponseWeight: '响应连续',
        previousResponseWeightHint: 'previous_response_id 粘性权重；越大越优先复用原账号，减少上下文迁移。范围：非负有限数（>= 0，支持小数）；留空继承配置/默认值。',
        sessionStickyWeight: '会话粘性',
        sessionStickyWeightHint: 'session_hash 粘性权重；越大越优先复用同一会话账号。范围：非负有限数（>= 0，支持小数）；留空继承配置/默认值。',
        fairnessTitle: '公平调度',
        fairnessDescription: '控制候选池覆盖和长期未使用账号的探索，减少请求集中在少数上游账号。粘性请求仍遵循原有绑定语义。',
        poolMode: '候选池',
        poolModeHint: 'top_k 只使用 Top-K；all_eligible 使用全部可用账号；hybrid 保留 Top-K，并在需要时加入最久未使用账号。',
        poolModeTopK: '仅优选',
        poolModeAll: '全部可用',
        poolModeHybrid: '混合公平',
        explorationRatio: '探索比例',
        explorationRatioHint: '仅 hybrid 生效；没有达到饥饿阈值时，按该百分比抽取最久未用账号。0=关闭，100=每次都探索。',
        starvationThreshold: '冷落阈值',
        starvationThresholdHint: '账号超过该时长未使用就进入优先探索；0=关闭阈值，仅按探索比例运行。默认 21600=6 小时。',
        fairnessWeight: '公平权重',
        ...
        fairMode: '公平调度',
        presetSelect: '公平预设',
        presetSpecialOffer: '体验优先',
        presetBalanced: '体验均衡',
        presetPro: '利润优先',
        weightRange: '范围：0–10',
        topKRange: '范围：1–32',
        topKShortHint: '每次参与评分的候选账号数。',
        poolModeRange: '范围：三种固定选项',
        poolModeShortHint: '选择参与评分的账号池。',
        explorationRange: '范围：0–100%',
        explorationShortHint: '控制低频账号的探索比例。',
        starvationRange: '范围：0 或 300–86400 秒',
        starvationShortHint: '保护长期未被选中的账号。',
        fairnessRange: '范围：0–10',
        fairnessShortHint: '控制公平保护对排序的影响。',
        priorityShortHint: '账号基础优先级影响。',
        loadShortHint: '当前负载惩罚强度。',
        queueShortHint: '排队等待惩罚强度。',
        errorRateShortHint: '历史错误惩罚强度。',
        ttftShortHint: '首字延迟偏好。',
        resetShortHint: '额度重置接近度偏好。',
        quotaHeadroomShortHint: '可用额度余量偏好。',
        upstreamCostShortHint: '上游成本排序影响。',
        previousResponseShortHint: '延续上一响应账号的偏好。',
        sessionStickyShortHint: '保持会话账号的偏好。',
        presetValuesHint: '已选择 {mode}；公平预设的底层权值和公平参数自动回填并锁定。',
        clearGroupPolicy: '清除该分组覆盖，继承全局值',
        stepGroup: '第一步：选择分组',
        stepMode: '第二步：选择策略模式',
        stepValues: '第三步：配置参数或选择预设',
        customMode: '自定义参数',
        presetMode: '预设模式',
        customPresetName: '预设名称',
        customPresetNamePlaceholder: '例如：晚高峰稳态',
        saveAsPreset: '保存为预设',
        presetNameRequired: '请输入 1-40 个字符的预设名称。',
        presetNameDuplicate: '预设名称已存在，请换一个名称。',
        presetReadOnlyHint: '预设参数由服务端快照决定，仅可查看。',
        customPresetTitle: '管理员预设',
        renamePreset: '重命名',
        deletePreset: '删除',
        deletePresetConfirm: '确认删除预设“{name}”？',
        presetReferenced: '该预设正在被分组使用，不能删除。',
        builtinPreset: '内置',
        customPreset: '管理员',
        selectGroupBeforePolicy: '请先选择分组，再配置策略。',
        serviceGuard: '服务不中断是固定护栏；健康、容量、冷却和并发检查始终生效。',
        prioritySummary: '当前优先级：',
        resetRecommended: '恢复推荐设置',
        metricProfit: '利润',
        metricTTFT: '首字速度',
        metricLatency: '完整耗时',
        balanceTitle: '账号均衡',
        balanceLow: '仅分散高负载',
        balanceStandard: '标准覆盖',
        balanceHigh: '优先补齐覆盖',
        peakTitle: '高峰期保护',
        peakStrict: '严格保护',
        peakStandard: '标准保护',
        ...
        sessionStandard: '标准',
        sessionSwitch: '更快切换',
        previewNormalTitle: '常态请求',
        previewNormalBody: '按当前优先级选择健康且有容量的账号。',
        previewPeakTitle: '高峰期',
        previewPeakStrict: '优先避开紧张账号，暂停纯覆盖探索。',
        previewPeakOpen: '在安全边界内保留必要的账号覆盖。',
        previewSessionTitle: '会话切换',
        previewSessionKeep: '尽量沿用原账号，减少上下文迁移。',
        previewSessionSwitch: '遇到负载或健康问题时更快切换。',
        previewSessionStandard: '按健康、容量和连续性综合判断。',
      },
      usageRecords: {
        title: '使用记录',
      ...
      monitorPageRefresh: {
        title: '监控页面自动刷新',
        description: '仅管理员可设置刷新间隔，用户无法自行调整。',
        off: '关闭',
        seconds: '{count} 秒',
        minute: '1 分钟',
        minutes: '{count} 分钟',
