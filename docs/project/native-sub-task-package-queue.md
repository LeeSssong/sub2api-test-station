...
- **T37 渠道状态用户可见专属分组裁剪**：状态 `DESIGNING`。修复 `/api/v1/monitor-v2` 把管理员角色等同于“可查看全部专属分组”的越权投影；复用 Sub 原生当前用户可访问分组事实，公开分组可见，专属分组仅在当前用户获授权时可见。不得仅做前端隐藏，不改授权数据、分组配置、主动探测、调度、账务、迁移或生产数据。独立用户可见任务使用用户指定的 `GPT-5.6 Sol / high`，从本登记后的最新干净 `main` 创建独立 worktree；规格和计划由根总控按既有代审授权审查。
- **T38 可调度账号最近原生探测评分保留**：状态 `DESIGNING`。账号当前仍可调度且已有可评分的原生主动探测证据时，即使最新探测失败、当前状态为不可用或证据超过当前状态新鲜度，也保留最近原生探测聚合形成的评分和排名；状态继续如实展示，不以真实业务请求补分；从未获得有效原生探测证据时仍为暂无评分。不得修改评分公式、评分权重、调度、账号配置、迁移或生产数据。独立用户可见任务使用用户指定的 `GPT-5.6 Sol / high`，从本登记后的最新干净 `main` 创建独立 worktree；规格和计划由根总控按既有代审授权审查。
