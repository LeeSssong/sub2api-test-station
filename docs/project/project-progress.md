...
**本轮非 main 工作区整合与发布准备（2026-08-09）：** 状态：进行中。所有候选已并入并推送至 `main@d73e82254dd0074557712f1903a1adc9d0901928`。获得停机授权后的发布多次在停机前因 SSH 连接重置或远端目录校验连接失败而中止；生产 API/worker/PostgreSQL/Redis/Caddy 均保持运行。已用同一 SSH 身份独立确认 `/usr/local/libexec` 全链路为 `root:root 755` 且无符号链接；`bash -s -- name path` 的正确参数位为 `$2`，一次误判修改已在未部署前识别并撤销。正在重新冻结发布证据并重试；未完成生产部署与线上验证前保持“进行中”。
