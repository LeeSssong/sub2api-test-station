...
**本轮 T03-R1 账号财务、逐笔上游成本与异常核对 MVP（2026-08-12）：** 状态：进行中（新正式规格已写成并完成自审，等待用户最终书面批准；禁止实施）。任务从干净 `main@747c7fb14d1ded243794a77984778babece7c799` 创建用户可见独立顶层任务、独立 worktree 与分支 `codex/t03-r1-upstream-cost-persistence`。生产最近 500 条自然流水精确 `upstream_request_id` 覆盖 500/500（Sub 244、New 256），但本地持久化上游成本/利润为 0/500，根因是管理员详情仍即时查询上游。旧规格/计划及直接扩展 `usage_logs` 的候选 `1c3e87684`、`ce5691527` 已被用户后续决策冻结，不合并、不推送、不部署。新批准方向：官方 `usage_logs` 保持不变；独立一对一证据模型登记功能启用后的 Sub/New 原生账单；正常非零证据立即纳入人民币营收/支出/利润，零成本或不可用证据进入使用记录的异常 Tab，人工核对后才纳入；字面 `oauth` 类型不查询上游，由管理员填写北京自然日人民币成本；现有账号盈利页升级为管理员财务首页，显示全站与账号营收、本站支出、利润、利润率、异常和未删除用户余额快照，查询时实时汇总并每 60 秒刷新；今日可直接覆盖营收/成本且以截止点保护后续增量；无历史回填、无延迟补查、无上游重试、无估算、无汇率或采购分摊。新有效规格为 `docs/superpowers/specs/2026-08-12-t03-r1-account-financial-reconciliation-design.md`；旧规格和旧计划只作历史追溯。T05 继续暂停。

**本轮官方 `v0.1.175` Task 3 资格、合并与生产发布（2026-08-12）：** 状态：进行中（外部发布 profile 阻塞）。工作区为根目录 `/Users/gongtengxinwen/Documents/sub2api搭建`，合并基线 `main@350e050575377d8e31ed153624bb19da3591517f` 与起始 `origin/main` 一致；发布登记提交为 `f1ac505e60a22bb7a169d99c8551ca1f6b64415e`。已在最终树完成 `internal/handler`/`internal/service` Go 测试、`go vet ./internal/handler`、UsageView/UsageTable 39/39 Vitest、前端 typecheck/build、版本/官方来源与 `git diff --check`；现有最终全分支审查 `011bf2b0c48f775e624c82bff3bceddf8e12c69a` 为 `APPROVE`，不重复候选审查。官方合并相对第一父提交无 migration 路径，当前迁移哈希 `f1b1f3537d518c30dc2fe99d75e9f2d7a5a27452f59ce4a50a1e81277c8cfbcc`；绑定 `f1ac505e…`/tree `0186d1ea…` 的 `0600` 资格证据为 `/private/tmp/official-v0175-release.7SHfXv/test-evidence.json`。以已知非秘密 `SUB2API_IMAGE_REPOSITORY=ghcr.io/leesssong/xingqiao-sub2api`、`RELEASE_TRANSPORT=preloaded` 调用审查过的生产 controller 时，控制器在 SSH、构建和宿主变更前 fail-closed：`RELEASE_SSH_TARGET is invalid`（环境中不存在该变量）。只读检查 runbook、`config/releases/sub2api.env`、本地 known_hosts 和既有 profile 位置，未发现可用 production profile；尚缺 `RELEASE_SSH_TARGET`、`RELEASE_SSH_PORT`、`RELEASE_SSH_KEY`、`RELEASE_SSH_KNOWN_HOSTS`、`RELEASE_NETWORK_CURL_IMAGE`、`RELEASE_NETWORK_CURL_IMAGE_ALLOWLIST`。不猜测凭据、不推送、不构建、不部署、不回滚且未做线上验证；T03-R1 与 `upstream-resilience-hardening` 保护 worktree 均未修改。恢复后必须先重跑 controller 预检，只有真实输出 `downtime_required=false` 才可推送并按预加载蓝绿链继续；此之前保持进行中，不能交付 T03-R1 已验证生产基线。
