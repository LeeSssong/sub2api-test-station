...
**T20 生产收口与 T21 根整合（2026-08-18）：** T20 候选 `3b120046e328535ce587db60a5ef750586d652d0` 已合入并推送根 `main@c2a1429623b22d0e5c3d4746a508d0f34e0a93e9`，tree `e41c1460b05ad1b040c2700fb54227b2dad0f947`。既有预加载蓝绿链返回 `succeeded/promoted`、`rolled_back=false`、`downtime_required=false`，活动槽 `blue`，宿主记录 `/var/lib/sub2api/release-records/20260818T051214Z-production-2858199.json`，迁移哈希保持 `bb6ebff31f0ffe9be5ad204ba79ef896d98522ccdd7b3933843c94d6c9ad5951`，公网 `/healthz`、`/readyz`、`/health` 均 200。生产用量样本 125444/125509/125512 的有效账号成本与 T17 口径一致，evidence `unavailable` 保留为辅助状态；24h 财务 API 已返回多个零流水有效绑定账号，请求、成本、用户扣费和利润均为零。T20 状态为 `DONE`。T21 候选 `7120593f2` 已完成规格/计划、实现与直接相关门禁，现进入 `INTEGRATING`；T22 保持 `BACKLOG`。T21 生产宿主尚未提供合规 detector sidecar URL/token，线上先验收离线状态语义，真实模型检测待制品与配置满足 T15 门禁后补验收。**状态：T20 已完成；T21 根整合中。**
