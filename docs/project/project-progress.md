...
**T03-R1 维护发布迁移门禁补全（2026-08-14）：** 状态：进行中。用户在已获知宿主预检 `downtime_required=true`、`reason_code=migration_set_changed`、预计不可用最多 300 秒及保留独立表的应用回滚方式后，明确授权“允许停机部署”。当前独立工作区 `/Users/gongtengxinwen/Documents/sub2api搭建/.worktrees/t03-r1-maintenance-transition`、分支 `codex/t03-r1-maintenance-transition`、基线 `main@fbd85f608fb6519a52e87b6a8d71011efcfa90a2`。发布前核对发现现有宿主执行器仅允许六组历史迁移转换，尚未允许生产活动迁移集合 `f1b1f3537d518c30dc2fe99d75e9f2d7a5a27452f59ce4a50a1e81277c8cfbcc` 到 T03-R1 集合 `6a0e141eb4788460a99fc3e108ce5b46c866fd2c45b9a7265ea66b0ef8faaf71` 的精确转换；直接重试仍会在停服前 fail-closed。当前仅按 TDD 补充这一对完整哈希、migration 222 的受控维护测试和对应运行手册说明，禁止通配、跳过哈希、修改业务功能、迁移内容、生产数据或其他工作区。完成独立复审、合并、推送和新证据后，才可执行受审维护发布；T05 继续暂停。
