...
**T16 经营页真实结果与视觉层级重设计登记（2026-08-17）：** 用户已明确批准视觉方向，本项作为 T12 已生产收口后的新独立任务包登记，不回写或重开 T12。产品合同为：默认打开“全部真实结果”；账号明细只保留运营消耗、业务消耗、业务营收、总消耗、净利润五项；管理员免费使用的站内扣费虽为 0，真实上游成本仍计入内部运营消耗和总消耗，净利润可为负但必须表述为内部补贴/内部运营而非客户亏损。事实源继续唯一复用 Sub 原生 `usage_logs`，不改变 `cost`/`user_cost` 基础公式或建立第二账务源；禁止以 `user_cost=0` 猜管理员身份，正式规格必须核查 `user_id/role` 的历史角色变化风险，若需不可变 actor type 则单独论证最小数据契约且不回填猜测。视觉语义为业务营收蓝、真实上游消耗琥珀、内部运营紫、净利润绿、真实亏损/内部补贴警示红；摘要突出业务营收/总消耗/净利润/对外毛利率并单列已包含于总消耗的内部运营消耗，明细使用紧凑表格，390px 摘要两列且整页无横向溢出。当前仅登记 `BACKLOG`，不创建 worktree、不 planning、不实现、不合并或部署；轮到时从届时最新干净 `main` 启动独立用户可见任务。**状态：待办（BACKLOG）。**
