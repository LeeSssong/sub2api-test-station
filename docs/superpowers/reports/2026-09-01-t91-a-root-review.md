...
**复核对象：** 独立候选 `codex/t91-a-task0-source-baseline@b112e84a1`。

## 结论

本任务**不能直接进入最终业务开发阶段**，当前状态为 **`REFRESH_REQUIRED`**；可以继续进入 **T91-A 的 acceptance 只读基线补证阶段**。

源码映射报告已达到“真实路径与入口已核验”的要求；但规格书第 10.0 和第 12 节要求的 acceptance 运行态基线仍有未执行项，且报告已明确候选与根 `main` 分离。因此当前结论为：

> **条件通过但需刷新：允许补齐 Task 0 acceptance 基线；不允许直接修改 schema、Ent、迁移或运行时代码。**
...
3. 刷新 Task 0 报告中的实际 `main`/`origin/main` SHA、工作区状态和当前 worktree 盘点；当前候选 SHA 已核验为 `b112e84a1`。
