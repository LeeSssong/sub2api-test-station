...
**T12 经营页本站探测花费与排序/美元字段优化（2026-08-17）：** 原候选 `c7587599a` 曾合并后因 P0 插队在发布前被可追溯撤回，生产未构建、未迁移、未切换。P0 收口后，唯一发布总控解除冻结；T12 worktree 快进 `main@b16d45203`，仅反向撤销移除 T12 运行时/任务文档的 `9d9e2b758`，保留当前根全局文档和全部 P0 行为，生成运行时候选 `35baf14ae`、最终 handoff 候选 `4029240f4`。候选无冲突快进合入后，首次根发布树 `59316b4a9` 的预检因迁移集合变化返回 `downtime_required=true`、`reason_code=migration_set_changed`、预计不可用约 300 秒，并明确在候选启动前停止；没有执行迁移、没有切换活动槽，生产仍运行 P0 修复版本 `e554b7d2e` 的 `green` 槽。就绪审计进一步发现宿主精确维护白名单只到当前生产哈希 `ef121384…`，因此根总控以 `04d171e35` 仅补齐 T12 `ef121384… -> aaebed88…` 的 maintenance-10 哈希对、fail-closed 测试和当前 runbook 并推送。最新干净发布 worktree `/private/tmp/sub2api-release-t12-04d171e35` 上，四组 focused Go、前端 19/19、typecheck/build、维护白名单精确允许/错误 old/new/未授权拒绝、bash 语法和 diff-check 均通过；新的 0600 证据 `/Users/gongtengxinwen/.codex/release-evidence/sub2api/2026-08-17-main-04d171e35-t12-probe-cost-maintenance-ready-v1.json` 绑定 `main@04d171e357b2e30fe7408f855a48999c07647250`、tree `f704a242e041061deadccec06d966e16158e8741` 和迁移哈希 `aaebed88f7fb712e1f518e73cc89bd44eb214f365f3b49f003598c93883a4604`；旧门禁证据继续保留。不得复用历史停机授权或自行带维护参数重试；worktree/分支继续保留，旧 docs-only T12、v0.1.175 和 S1 分支继续冻结。**状态：受阻（BLOCKED；维护发布已技术就绪，等待用户明确授权本次约 300 秒维护窗口，随后才能部署和线上验收）。**

**T15 账号监控原生探测模型与异步模型检测（2026-08-17）：** brainstorming 与产品设计已完成并获用户批准，用户明确豁免再次确认正式规格；当前只登记后续任务，不调用 writing-plans、不创建实现 worktree、不实现、不合并、不部署，不占用 T12 当前车道。批准合同为：原生连接测试复用 `AccountTestService.ProbeAccountConnection` 并按账号保存 `connection_probe_model`；独立 `model_detection_model` 只从 Sub 原生账号模型登记与检测器基线目录交集选择；仅执行 sidecar 由 Sub worker 在北京时间 `00:00/10:00/12:00/15:00/18:00/21:00` 调度 API Key 账号并持久化唯一事实，OAuth 不执行，固定时隙持久化去重且只在 30 分钟内补触发；单账号立即检测异步且复用在途任务。账号卡片只增加默认收缩状态行与详情弹窗，无全局摘要且不改主样式；八种状态及结果不影响评分、调度、可调度状态或分组建议，异常仅称“检测器观察到异常”。凭据只经私网内存传递，不保存 API Key、完整提示词/输出或上游地址，sidecar 失败不影响原生连接测试/账号状态。轮到时由独立顶层任务/worktree产出正式规格和自审后再进入计划门禁；继续本地/宿主发布链，`downtime_required=true` 必须等待用户授权。**状态：BACKLOG。**
