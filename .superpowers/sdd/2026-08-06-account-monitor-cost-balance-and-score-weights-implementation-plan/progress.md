...
- Task 6: completed — final runtime fix `c12f930b1` handles accounts with no probe snapshot without panic. Candidate was pushed, authorized maintenance migration 197/198 was deployed, then the same-hash zero-downtime fix was deployed. Production is active on blue with healthy API/worker and preserved shared-service identities.
...
- Native convergence migration set: production now runs the complete set including migrations 197 and 198; canonical hash `0204f39423f3218ffa0c8d4e3d665f7113c4990610e0dd22e9f5910c4d578c6d`. Maintenance release preserved the PostgreSQL, Redis, and Caddy container identities.
...
- Production verification (2026-08-07): database backup `/var/lib/sub2api/backups/pre-native-multiplier-20260807T011920Z.dump` created (29,520,745 bytes, mode 0600); legacy policy rows 0, legacy measurement rows 0, native sync rows 40. Public `/health` returned 200. Authenticated monitor API returned 8 groups and 94 accounts with score weights, native multiplier status/observed_at, and balance snapshots. Final release source `c12f930b1`, active slot `blue`.
