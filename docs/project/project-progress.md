...
**本轮 T04 账号监控移除外部控制面状态（2026-08-12）：** 状态：进行中（候选已达到 `READY_FOR_ROOT_REVIEW`，尚未合并、推送、部署或线上验收，不得标记已完成）。独立工作区为 `/Users/gongtengxinwen/.codex/worktrees/72c7/sub2api搭建`，分支 `codex/t04-account-monitor-native-only`，基线 `eaa73610238f1effc253dc84a98eae4965390525`；设计提交 `5b58197e90063e92793793b1326770068843c3c2`，计划提交 `e123fc472`，实现候选 `8f14556dfc327ab56d7004fbe33c0fb8cfb965bd`。账号监控现已在首次加载、时间窗切换、刷新和代表性单账号探测后重载中只调用 `/admin/accounts/monitor`，且页面不再请求 `/xingqiao/**`；页面控制面投影、状态及状态条已删除，未引入替代常驻状态，也未改动卡片、布局或写操作。原生首次加载失败显示独立的可重试空态；保留已有快照时继续显示范围错误和旧卡片。独立任务复审结论为规格符合性 PASS、任务质量 PASS；最终全分支终审 Critical/Important/Minor 均无发现并确认可交根任务审查。最终候选上新鲜专项 `pnpm exec vitest run src/views/admin/__tests__/AccountMonitorView.spec.ts` 通过（28/28），`pnpm typecheck`、`pnpm build`、`git diff --check` 与范围检查通过，页面的控制面符号与 `/xingqiao/` 扫描为零匹配；仅有既有非致命 pnpm、Browserslist、Vite 动态导入/分块警告。共享控制面文件未改动；无迁移、无配置变更，`downtime_required=false`。正式设计见 `docs/superpowers/specs/2026-08-12-t04-account-monitor-native-only-design.md`，实施计划见 `docs/superpowers/plans/2026-08-12-t04-account-monitor-native-only.md`。当前本地 `main` 已前进至终审观察到的 `e8f93a57a1e59aedff694bf3591bdf3dcbd4a03a` 且与候选均修改本总账；最终交付仍须根任务发送包含届时精确目标 `main` SHA 的 `AUTHORIZE_MERGE_TO_MAIN`，合并时保留双方总账历史，再在合并后的 `main` 上完成专项回归、发布预检、推送、既有本地/宿主链部署和登录态浏览器零 `/xingqiao/**` 线上 MVP 验证；不得使用 GitHub Actions。
