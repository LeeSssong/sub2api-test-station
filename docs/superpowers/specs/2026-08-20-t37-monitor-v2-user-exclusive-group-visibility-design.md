...
1. 仅展示在“渠道管理 → 渠道监控”中配置且启用监控项所关联的 active 分组。
2. active 专属分组还必须出现在当前登录用户的 Sub 原生可访问分组结果中；公开分组同样必须先满足渠道监控配置。
...
- 前端仅做展示层修正：移除标题下的冗余说明，时间线 tooltip 固定在柱体下方；现有 v7 客户端只会看到更窄且正确的 `groups` 数组。
...
无产品待决事项。2026-08-20 用户补充确认：Monitor V2 只展示渠道管理 → 渠道监控中启用监控项已关联的分组；未配置分组不展示；tooltip 位于柱体下方；标题下说明删除。
