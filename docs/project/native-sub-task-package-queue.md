...
- 当前状态：进行中（已合并、已推送、用户已授权停机部署；维护发布器缺少精确迁移转换，尚未部署）。候选 `2e28308a04495482fbe16af05344111bb4a0b0a7` 已合并为 `main@dbe1231e0243d7190996f21cd8b8e5ac17b8420a`；当前 `main`/`origin/main` 为 `fbd85f608fb6519a52e87b6a8d71011efcfa90a2`。活动迁移集 `f1b1…` 到 T03-R1 集 `6a0e…` 不在受审宿主执行器的 allowlist；即使用户已授权停机，发布器仍会在停服前以 `migration_set_changed` fail-closed。必须先以独立用户可见顶层维护任务增加、测试、复审唯一精确转换，再从其合并后的 `main` 重新验证和发布。生产未变更，旧候选 `1c3e87684`、`ce5691527` 保持冻结，不得恢复。
