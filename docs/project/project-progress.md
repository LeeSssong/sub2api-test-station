...
**官方 Sub2API v0.1.177 最高优先级更新（2026-08-16）：** 状态：进行中（`DEPLOYING`）。用户明确要求官方更新插队为最高优先级，并建立全局例外：只解决上游合并冲突后直接通过既有本地/宿主发布链使更新生效，不运行功能测试、回归测试、类型检查、独立构建验证或上线专项验收；发布链内置的候选制品生成、迁移/停机保护、原子切换和失败回滚不得绕过。T09 可见任务 `01a008e1-55d1-74f1-a659-fac363dcfd28` 已保全旧候选并交付官方 `v0.1.177@073e92d17178a1ccdb0a27017f572f10c9c7ab62` 刷新候选 `458c27c1cc4511a029884fbe3e34d907cd4ab47b`、tree `72b01b12a01c96c2147e2791ec52a7217d26a8cf`；5 个冲突已人工解决。候选已 fast-forward 合入并推送为根 `main@458c27c1cc4511a029884fbe3e34d907cd4ab47b`，现由唯一发布总控执行既有本地/宿主发布链。候选新增迁移 221/222/223，`downtime_required` 必须以发布链门禁为准；若为 `true` 或运行就绪失败，必须在受保护动作前停止。T12 与其他功能任务继续暂停在设计/排队阶段。
