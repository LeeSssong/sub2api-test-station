...
func newCodexCatalogMappedAccount(id int64, target, displayName string, levels, modalities []string, contextWindow int64, schedulable bool, extraMapping map[string]any) Account {
	reasoning := true
	...
	account := Account{
		...
		Credentials: map[string]any{
			"base_url": fmt.Sprintf("https://provider-%d.example/v1", id), "model_mapping": mapping,
		},
	}
	models := map[string]UpstreamModelMetadata{target: {
		ID: target, DisplayName: displayName, Description: displayName + " upstream", Reasoning: &reasoning,
		SupportedReasoningLevels: levels, InputModalities: modalities, ContextWindow: contextWindow,
	}}
	for _, value := range extraMapping {
		...
		models[exclusive] = UpstreamModelMetadata{
			ID: exclusive, DisplayName: "Exclusive Model", Description: "Only mapped on the unschedulable account",
			Reasoning: &reasoning, SupportedReasoningLevels: []string{"high"}, InputModalities: []string{"text", "image"}, ContextWindow: 1_000_000,
		}
	}
