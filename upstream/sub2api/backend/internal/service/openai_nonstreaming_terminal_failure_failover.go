...
// verdict when a stream=false request receives SSE from an upstream.
func (s *OpenAIGatewayService) nonStreamingTerminalFailureFailover(
	...
	return s.newOpenAIStreamFailoverErrorWithModel(
		c, account, passthrough, upstreamRequestID, payload, message,
		firstNonEmpty(canonicalModel...), headers,
	)
}
