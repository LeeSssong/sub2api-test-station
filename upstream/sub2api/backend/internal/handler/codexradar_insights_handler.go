...
func (h *CodexRadarInsightsHandler) Get(c *gin.Context) {
	...
	if h == nil || h.service == nil {
		c.JSON(http.StatusServiceUnavailable, gin.H{"error": gin.H{"code": "CODEXRADAR_INSIGHTS_UNAVAILABLE", "message": "站长推荐暂时不可用"}})
		return
	}
	...
	c.JSON(http.StatusOK, gin.H{
		...
		"recommendations": value.Recommendations, "source_status": sourceStatus, "stale": stale,
	})
}
