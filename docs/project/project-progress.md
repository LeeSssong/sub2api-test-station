...
0.1. **账号监控 V3：账号质量、评分、排名与调度信息**：2026-08-04 最新确认已覆盖旧的“账号监控同时承接经营/账务”方案。当前唯一目标是按最终截图 1:1 实现原生分组 Tab、七项分组汇总、真实请求时间窗、完整六段式账号卡片、卡内全局优先级、一次性采购成本、原生并发与桌面双列/移动单列；营收、利润、账务和对账迁往后续独立“营收”页面。**状态：进行中（隔离工作树本地实现、最终同视口视觉验收和受影响测试矩阵已完成，当前为可部署候选；仍未推送、部署并在线验证，禁止标记完成）**。
