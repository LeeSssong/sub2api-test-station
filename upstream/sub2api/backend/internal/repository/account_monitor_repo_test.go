...
func TestAccountMonitorRepositoryProjectMonitorV4UsesLogicalRequestProjection(t *testing.T) {
	...
	mock.ExpectQuery(`(?s)WITH scopes AS.*groups AS.*buckets AS.*raw_usage_candidates AS.*unknown_usage_keys AS.*SELECT DISTINCT u\.group_id, NULLIF\(u\.request_id, ''\).*SELECT DISTINCT u\.group_id, NULLIF\(u\.logical_request_id, ''\).*error_candidates AS.*NOT EXISTS.*unknown_usage_keys.*real_events AS.*PARTITION BY rc\.group_id, rc\.request_key.*selected_events AS.*missing_probe_counts AS.*has_real IS NOT TRUE AND probe_missing.*SELECT group_id, success_rate, request_count, success_count, real_request_count, real_success_count,.*ttft_p95_ms, ttft_sample_count,.*latency_p95_ms, latency_sample_count, cache_read_tokens, cache_creation_tokens,.*cache_hit_denominator, cache_hit_rate, source_updated_at, current_operational`).
		...
		WillReturnRows(sqlmock.NewRows([]string{
			...
			"latency_p95_ms", "latency_sample_count", "cache_read_tokens", "cache_creation_tokens", "cache_hit_denominator", "cache_hit_rate", "source_updated_at", "current_operational",
		}).AddRow(7, 75.0, 4, 3, 2, 1, 2, 2, 0, 120.0, 2, 800.0, 3, 30, 10, 40, 0.75, updatedAt, true))
	...
	if row.CacheHitRate == nil || *row.CacheHitRate != 0.75 || row.CacheHitDenominator != 40 || row.CacheReadTokens != 30 || row.CacheCreationTokens != 10 {
...
func TestAccountMonitorRepositoryProjectMonitorV4ReportsMissingProbeWithoutServiceFailure(t *testing.T) {
	...
	mock.ExpectQuery(`(?s)WITH scopes AS.*groups AS.*buckets AS.*bucket_matrix AS.*selected_events AS.*WHERE bm\.has_real IS NOT TRUE\s+AND bm\.probe_missing IS NOT TRUE\s*\), latest_selected AS`).
		...
		WillReturnRows(sqlmock.NewRows([]string{
			...
			"probe_fallback_bucket_count", "probe_fallback_request_count", "missing_probe_terminal_count", "ttft_p95_ms", "ttft_sample_count",
			"latency_p95_ms", "latency_sample_count", "cache_read_tokens", "cache_creation_tokens", "cache_hit_denominator", "cache_hit_rate", "source_updated_at", "current_operational",
		}).AddRow(7, nil, 0, 0, 0, 0, 0, 0, 1, nil, 0, nil, 0, 0, 0, 0, nil, nil, false))
...
func TestAccountMonitorRepositoryProjectMonitorV4ConstructsGroupMatrixWithoutAccountScopes(t *testing.T) {
	...
	mock.ExpectQuery(`(?s)WITH scopes AS.*SELECT unnest\(\$6::bigint\[\]\).*CROSS JOIN buckets.*LEFT JOIN real_buckets`).
		...
		WillReturnRows(sqlmock.NewRows([]string{
			...
			"probe_fallback_bucket_count", "probe_fallback_request_count", "missing_probe_terminal_count", "ttft_p95_ms", "ttft_sample_count",
			"latency_p95_ms", "latency_sample_count", "cache_read_tokens", "cache_creation_tokens", "cache_hit_denominator", "cache_hit_rate", "source_updated_at", "current_operational",
		}).AddRow(99, nil, 0, 0, 0, 0, 0, 0, 1, nil, 0, nil, 0, 0, 0, 0, nil, nil, false))
