...
images=(
  ...
  'xingqiao-caddy:homepage-20260725-v8-detailed-guide'
)
...
if rg -n -F 'reverse_proxy @relay_ops_admin relay-ops:8100' infra/Caddyfile; then
  fail 'retired Relay admin proxy must not be publicly routed'
fi
...
[[ "$path_lines" == $'\t\tpath /api/v1/admin/system/rollback' ]] || \
  fail 'binary mutation matcher must contain exactly the rollback path'
