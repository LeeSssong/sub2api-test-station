...
**T132 账号监控检测判定与证据可见性修复（2026-09-05）：** 状态 `INTEGRATING`。候选提交 `0b53b67b8`、`f6ad3d34f`、`9ea86eb0d` 已无冲突合入根 `main@822e85297`，追加修复 `933986be4` 已推送。已恢复 v4.1.1 官方预设及正式模型判定；账号卡片拆分 Juice/模型两行结果并使用固定性能柱宽；历史详情展示 Juice 申报冲突证据，并为升级检测保存上一轮具体可疑证据白名单快照。追加审查修复将证据不足作为合法 `insufficient` 终态而非请求失败，并将主动兜底探测最大周期钳制为 5 分钟；失败探测仍不进入成功率分子、分母或延迟指标。旧记录不回填，无凭据、提示词或完整响应落库。用户再次明确授权主站快速部署、不同步独立验收站并允许停机；已按 TDD 补齐 migration 236 唯一精确维护转换 `30adae913... -> a01097cc0...`，未知或错误哈希继续 fail-closed。adapter 9/9、service/repository/migration、前端 69/69、typecheck、server build、T132 RED/GREEN、完整维护合同、脚本语法与 diff-check 已通过；准备提交推送并执行受控维护发布。独立验收站按授权保持未同步。
