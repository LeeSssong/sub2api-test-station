...
**本轮 T02 原生错误转译与管理员诊断 MVP（2026-08-12）：** 状态：进行中。候选 `codex/t02-native-error-diagnostics` 已在独立 worktree 完成本地实现、三项 P1 复审修复与验证，待根线程最终独立复审；尚未合并、推送、部署或线上验证，因此不标记完成。T01 已完成推送、无停机部署、线上验证、归档和候选清理后，从最新且已推送的 `main@a4b7d8c6e` 启动队列唯一下一任务包，并以 `74f139243a0d402674713f218c8079232ad6465d` 为候选基线。已实现四类稳定诊断：本站 RPM/并发限制、上游过载、通用上游失败、请求上传中断；已选账号后发生的本站账号并发/等待队列限制依据 `phase/owner/is_business_limited` 与明确本地错误文本优先归为 `local_limit`，不再被 429 覆盖为上游过载；`local_limit` 用户文案按频率/并发、额度/订阅、策略/模型规则三类证据精确投影。列表 SQL 只额外读取不序列化的最小上游分类证据，保证同一条 `503 + overloaded/capacity` 在列表与详情中一致，且用户 DTO 不泄露该证据。用户列表/详情仅投影脱敏中文含义与可执行建议，不返回上游状态、原始错误体、平台、分组或账号内部信息；现有共用管理员详情弹窗新增诊断段，并对 `X-Goog-Api-Key`/`apikey`/通配 API key、token、secret、Cookie、Bearer 和 query 变体统一二次脱敏；旧响应正文与关联上游预览不再渲染未脱敏原字段，仍保留管理员可用的安全证据。HTTP/SSE 机器客户端状态、type、code 和 body 未修改，二者仅在持久化记录投影层统一口径。无迁移、无配置变更、无 GitHub Actions，`downtime_required=false`。新增回归均保留 RED/GREEN 证据；定向 Go 测试、service/handler/repository 全包测试、`go vet`、server build、前端 26 个聚焦用例、typecheck、production build 及 `git diff --check` 均已通过；交接证据见 `docs/superpowers/reports/2026-08-12-t02-native-error-diagnostics.md`。未完成线上验证前不得启动 T03。
