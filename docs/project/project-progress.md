...
0.2. **本地提链支付工作台**：按 miaodongAI 公共 API 搭建独立本地单用户工具，支持三列 1:1:1 批量录入、异步提链与支付、SQLite 持久化、重启恢复及批次任务明细。**状态：进行中（全部实现已完成本地 TDD、独立复审和浏览器验收；`npm test` 为 57/57；本地服务可通过 `npm start` 启动。该工具仅本地运行，尚未按工作区生命周期推送、合并和清理）**。
