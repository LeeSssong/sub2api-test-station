...
func (h *AccountMonitorHandler) AccountModelDetectionHistory(c *gin.Context) {
	...
	for _, item := range page.Items {
		...
		result = append(result, service.AccountModelDetectionSummary{Status: item.Status, ModelID: item.ModelID, ClaimedModel: item.ClaimedModel, JuiceStatus: item.JuiceStatus, JuiceSummary: item.JuiceSummary, FingerprintCandidate: item.FingerprintCandidate, FingerprintSimilarity: item.FingerprintSimilarity, DetectorVersion: item.DetectorVersion, ErrorCode: item.ErrorCode, ErrorMessage: item.ErrorMessage, QueuedAt: &queued, StartedAt: item.StartedAt, FinishedAt: item.FinishedAt, RunID: item.ID, Source: detectionHistorySource(item), Profile: item.Profile, Mode: item.Mode, TriggerReason: item.TriggerReason, TriggerEvidence: item.TriggerEvidence, PlannedRequests: item.PlannedRequests, ValidSamples: item.ValidSamples, EvidenceState: item.EvidenceState, FingerprintStatus: item.FingerprintStatus})
	}
