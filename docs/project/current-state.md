...
- 当前工程差异：`main@6f0609799` 已推送；其应用代码已通过 `main@48244833b` 的 immutable Sub2API 镜像和 `relay-ops@release-48244833b` 在生产 blue/green 双 API、唯一 worker 拓扑生效，活动槽为 green。账号监控对账汇总、异常列表、管理员刷新/补登记 API 已通过公网入口验证，新增对账迁移已在生产数据库生效；TLS 1.2/1.3 与 RSA 2048 证书已验收。Native P0/P1 Bridge 未接线通道保持策略关闭。
