...
**本轮 T03 上游扣费与利润始终有值（2026-08-12）：** 状态：进行中。T02 已完成无停机生产发布和线上验证；T03 已从根线程指定且干净的 `main@0b6ef4ef0faec1b84de6f7133b5c99c4ae6e405d` 创建独立工作区 `/Users/gongtengxinwen/Documents/sub2api搭建/.worktrees/t03-upstream-cost-profit-values` 和分支 `codex/t03-upstream-cost-profit-values`，是当前队列唯一活动实现包。范围仅限管理员流水中 Sub/New 原生精确请求 ID 账单投影：上游原生实际扣费有值时返回该数值，原生字段空白时按 `0` 返回，利润按本站实际扣费减上游实际扣费返回数值；非管理员继续不可见。不做估算、对账状态、异常标记、模糊匹配、历史回填、外部控制面或其他任务包功能。派生任务需先写本包规格书与实施计划，再按 TDD 实施、独立复审并以 `READY_FOR_ROOT_REVIEW` 等待根线程授权；不得自行合并、推送或部署。若后续发布预检返回 `downtime_required=true`，必须立即暂停等待用户人工确认。
