...
- 状态：`READY_FOR_ROOT_REVIEW`（刷新后实现和直接相关验证通过；未合并根 `main`、未推送、未发布、未触碰生产）
- 基线：`main@b59baac1434f54024c5d5ea15e1d6f804d511aae`
...
- 候选提交：已将根 `main@b59baac1434f54024c5d5ea15e1d6f804d511aae` 整合到 T15，并按当前生成依赖重生成 `wire_gen.go`；包含本交接更新和 RoundTripper 测试替换的最终提交由根总控以精确 SHA 导入指定分支后授权合并。
...
- 北京时间 `00:00/10:00/12:00/15:00/18:00/21:00` 固定时隙，30 分钟内补触发；`(account_id, slot_key)` 持久化去重，同账号 queued/running 复用。
...
- `vitest run src/components/admin/account-monitor/AccountMonitorCard.spec.ts src/views/admin/__tests__/AccountMonitorView.spec.ts`：2 files / 93 tests 通过（使用现有 frontend node_modules；候选新增内存 RoundTripper sidecar 测试，避免沙箱 loopback bind 限制）。
- `vue-tsc --noEmit`、`vite build`、后端 `gofmt`、`git diff --check` 通过。
...
- 刷新冲突仅为 `backend/cmd/server/wire_gen.go`；已按合并后的 provider 图重新运行 `go generate ./cmd/server`，同时保留 T15 detector 与主线 shared-health/ops provider。
