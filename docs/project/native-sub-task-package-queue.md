...
**T105 OpenAI OAuth 429 账号级原生限流恢复（2026-08-31）：** 状态 `IMPLEMENTING`。用户已批准仅账号级、不做模型级；保留当前有限同账号重试，重试窗口耗尽或进入跨账号 failover 时复用 Sub 原生 `SetRateLimited`/账号限流状态；上游提供可靠 reset 时尊重该时间，无可靠 reset 时固定 5 分钟。请求级 `failedAccountIDs` 只用于当前请求防重复，不形成持久化的第二套控制；管理员现有 clear-rate-limit/recover-state 继续作为手动恢复入口。候选工作区 `/Users/gongtengxinwen/Documents/sub2api搭建/.worktrees/t105-openai-429-account-cooldown`，分支 `codex/t105-openai-429-account-cooldown`，基线 `main@d8f75dbfcc201a803ac09c781efc34158006221b`（tree `3a49741a4850df829116a26396beac2652689ff1`）。当前只在候选中完成规格、实现和直接相关测试；不得修改根 `main`/全局总账、推送、部署或触碰生产。
