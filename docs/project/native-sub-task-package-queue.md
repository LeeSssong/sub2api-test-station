...
- **T34 渠道状态原生探测重构**：状态 `DESIGNING`。从已包含 T32/T33 的 `main@a108ba2bdfd4f2a0b3af8f5cc451e4e86c0938c3` 创建独立用户可见任务与 worktree，任务 ID 待创建后回填。Monitor V2 完全复用 Sub 原生账号主动探测；可用性、首字速度、平均耗时、当前状态和时间线均来自主动探测，不使用真实请求；分组内至少一个可调度账号探测成功即为时间桶可用。分组倍率紧邻分组名称，删除“旗舰”，保持当前页面宽度。T34 可并行完成设计、实现和直接测试；T33 用户真机验收完成前，T34 不进入合并、部署或验收单车道。
