  ...
  <AppLayout>
    <section data-test="hybrid-performance-view" class="mx-auto w-full max-w-[1500px] rounded-2xl text-slate-900 dark:bg-[#07101f] dark:px-3 dark:py-3 dark:text-slate-100 sm:dark:px-4 sm:dark:py-4" aria-labelledby="hybrid-performance-title">
      ...
      <div v-if="loadError" data-test="hybrid-load-error" role="alert" class="mt-4 flex items-center justify-between gap-3 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 dark:border-red-900/80 dark:bg-red-950/30 dark:text-red-300">
        <span>{{ loadError }}</span>
        <button type="button" data-test="hybrid-retry" class="shrink-0 font-semibold underline underline-offset-2" @click="reload(currentWindow)">{{ t('channelMonitorV2.hybrid.retry') }}</button>
      </div>
...
const windowOptions = computed(() => [{ value:'1h' as const, label:t('monitorV2.window.1h') }, { value:'24h' as const, label:t('monitorV2.window.24h') }, { value:'7d' as const, label:t('monitorV2.window.7d') }]); const updatedAt = computed(() => new Intl.DateTimeFormat(undefined,{month:'short',day:'numeric',hour:'2-digit',minute:'2-digit'}).format(new Date(snapshot.value.generated_at)))
