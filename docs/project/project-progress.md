...
**T13 NewAPI 上游倍率自动登记恢复（2026-08-16）：** P0 会话热修已生产收口，按持续目标恢复用户可见顶层任务 `01a00969-b2ea-7ff0-9f49-d7af64438e00`。实施前已盘点全部非 main worktree：历史 release 候选均已为当前 main 祖先，T12/T13/S1 独有提交都是未完成或冻结候选，不存在必须先合并的已完成领先分支。T13 当前保留 `codex/newapi-rate-multiplier-registration@0730603c8` 与 17 个未提交 Task 2/3 文件；仅完成 P0 中断前的窄身份修复、直接相关 focused tests、候选提交和最终 handoff，不再增加 reviewer 或扩大验证。T12 继续冻结，待 T13 推送、部署和线上验收后再恢复。**状态：进行中（IMPLEMENTING；尚未提交新候选、合并、推送或部署）。**
