...
**T06 利润页移除外部控制面状态（2026-08-14）：** 状态：`VERIFYING`。候选已通过顶层任务规格/计划、实施、独立复审、全分支终审和根授权合并；发布基线为已推送的 `main@04349dda8f461f51aabb5f9fc0a3e2f6af251517`，tree `1c30f936e4a4d8c6338d18ace36d71860b58fc1c`，迁移哈希 `6a0e141eb4788460a99fc3e108ce5b46c866fd2c45b9a7265ea66b0ef8faaf71`。网络恢复后已完成 `downtime_required=false` 蓝绿生产发布；宿主记录 `/var/lib/sub2api/release-records/20260814T154749Z-production-3329818.json` 为 `succeeded/promoted`、`rolled_back=false`，活动槽 `blue`，API/worker 运行镜像 `ghcr.io/leesssong/xingqiao-sub2api:release-04349dda8f461f51aabb5f9fc0a3e2f6af251517-89acc95334957dc0e7aab3c346e1db7d2be111cdfe65ac44dbc55ecc47950d3e` 且健康，PostgreSQL、Redis、Caddy 和保留的 green 槽均健康；公网 `/healthz`、`/readyz`、`/health` 均返回 200。当前只缺管理员登录态 `/admin/operations/account-profitability` 专项验收；Chrome 与应用内浏览器均无登录态，已停在登录页且未读取或代填凭据。T07 仍未启动，待完成页面初载、reload、手动刷新、范围切换、无 `/api/v1/xingqiao/**` 请求及无控制面/完整性/unknown 状态验收后再转为已完成、归档清理并创建 T07 顶层任务。
