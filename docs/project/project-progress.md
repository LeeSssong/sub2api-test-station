...
**S3 自适应选择、粘性逃逸与调度体验观测（2026-08-17）：** S2 已完成生产验收，依赖门禁已满足。发布总控已盘点全部注册 worktree；领先 `main` 的提交均属于已明确受保护、冻结、已被主线替代或只读历史证据候选，不构成 S3 前置整合项。S3 现以根 `main@7c8ffe70d0217fcac90c35e2de3d7bdd3376c87f` 为启动基线进入设计，规划在 `/Users/gongtengxinwen/Documents/sub2api搭建/.worktrees/s3-adaptive-scheduling-experience` 使用分支 `codex/s3-adaptive-scheduling-experience`；必须先重新核对当前 S1/S2 代码事实并刷新正式规格，再进入实施。T15 保持受保护 `READY_FOR_ROOT_REVIEW`，T16 保持 `FROZEN`，历史 `upstream-resilience-hardening` 保持只读。**状态：进行中（DESIGNING）。**
