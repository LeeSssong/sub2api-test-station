    ...
    <div class="card p-4 flex items-center gap-3">
      ...
      <div class="min-w-0 flex-1">
        ...
        <p class="text-xl font-bold text-green-600">
          ${{ formatMoneyFixed(stats?.total_actual_cost) }}
        </p>
        <p class="text-xs text-gray-400">
          <template v-if="showAccountCost && totalAccountCost != null">
            <span class="text-orange-500">{{ t('usage.accountCost') }} ${{ formatMoneyFixed(totalAccountCost) }}</span>
            <span> · </span>
          </template>
          <span>
            {{ t('usage.standardCost') }}
            <span :class="{ 'line-through': strikeStandardCost }">${{ formatMoneyFixed(stats?.total_cost) }}</span>
          </span>
        </p>
      </div>
    </div>
