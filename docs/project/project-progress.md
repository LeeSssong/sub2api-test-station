...
**本轮官方 `v0.1.175` Task 3 资格、合并与生产发布（2026-08-12）：** 状态：进行中。工作区为根目录 `/Users/gongtengxinwen/Documents/sub2api搭建`，基线 `main@350e050575377d8e31ed153624bb19da3591517f`（与 `origin/main` 一致）。范围仅限合并后最小资格矩阵、迁移/发布预检、官方 `v0.1.175` 预加载蓝绿发布和线上验收；保护 T03-R1 与 `upstream-resilience-hardening` worktree，不使用 GitHub Actions。生产尚未变更，待完成本地门禁后按 `downtime_required` 决定是否继续。
