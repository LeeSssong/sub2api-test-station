...
**本轮 T04 账号监控移除外部控制面状态（2026-08-12）：** 状态：阻塞（等待用户明确批准第二节设计，尚未写规格书或实施）。T03 已完成推送、生产验收、可恢复归档和候选清理；T04 已从当时最新干净 `main@eaa73610238f1effc253dc84a98eae4965390525` 创建用户可见的独立顶层 Codex 任务 `019ff559-b4d6-7411-81c7-d0e631584d70`，独立 worktree 为 `/Users/gongtengxinwen/.codex/worktrees/72c7/sub2api搭建`，分支为 `codex/t04-account-monitor-native-only`。只读探索确认账号监控当前先请求原生 `/admin/accounts/monitor`，再请求外部页面决策与监控投影，外部结果可替换原生数据，失败时显示“控制面暂时不可用 / 完整性 unknown”并延长加载链。用户已确认账号监控所有场景对 `/xingqiao/**` 零请求、原生 `/admin/accounts/monitor` 为唯一数据源、共享控制面文件留给 T05/T06、原生失败不回退外部数据、正常态不增加替代状态条；已批准推荐方案及第一节“架构与范围边界”，并新增不可漂移约束：不修改现有账号卡片、页面布局、视觉样式或交互行为，必须无感优化。第二节“端到端数据流与失败语义”已展示，但连续多轮未收到用户明确批准；根据新增硬门禁，根任务不得代签或继续到下一节，T04 worktree/分支保持干净并暂停。恢复条件：用户在 T04 顶层任务中明确批准第二节或提出修改意见。未经全部分段设计批准和书面规格书明确批准，不得调用 writing-plans、实施、派生实现代理、合并、推送或部署；T05 不得启动。
