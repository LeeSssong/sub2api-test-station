...
**本轮登记（2026-08-07）：** Sub2API `0.1.171` 生产候选登记，状态：已完成（本地标签 `xingqiao-sub2api:upstream-0.1.171` 已指向合格镜像 `sha256:57436c65…`；更新器同一套生产解析器已返回 `ready=true`、`target_version=0.1.171`；官方版本/提交、完整资格标签、RepoDigest、二进制版本及 `linux/amd64` 架构均已核对；Sub2API 蓝/绿/worker、PostgreSQL、Redis、Caddy 容器身份、启动时间和重启次数均未变化；本次仅完成候选登记，尚未执行实际版本切换）。
