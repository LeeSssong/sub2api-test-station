...
**本轮登记（2026-08-13）：** 监控卡片 TTFT/总延迟 P95 数据异常排查与最快投产修复方案，状态：进行中。生产只读 SQL 已证明截图中的 GPT-Pro `121.08s/131.11s`、GPT-Plus `88.85s/447.49s`、GPT-特惠分组 `18.09s/96.58s` 分别等于过去 7 天“小时 P95 的最大值”，而相同窗口全量原始流水的真实 P95 分别约为 TTFT `20.60s/19.40s/10.79s`、总延迟 `38.63s/40.59s/19.35s`。根因是监控卡片使用 Ops `auto` 预聚合路径：小时汇总跨窗口时把 P95/P99 取 `MAX`，P50/P90 则做加权平均，因此展示的是保守近似而非窗口真实分位数；前端仅做毫秒转秒，不是单位错误，原始 SQL也未混入错误请求。最快修复建议仅对监控卡片切换到已有 raw 精确分位数路径并增加合同回归，零迁移、零数据回填，走现有蓝绿无停机发布；实施已合并到 `main`，尚未推送、部署和线上验证。

**监控 P95 紧急热修（2026-08-13）：** 状态：进行中。用户明确授权在不暂停 T03-R1 的前提下插队实施并立即上线；T03-R1 独立工作区继续保留，两个候选仅在 `main` 合并与生产发布阶段串行。生产只读证据确认监控卡片 7 天 TTFT/总延迟 P95 错误取成“各小时 P95 最大值”，目标仅为将监控卡片切换到已有 raw 精确窗口分位数路径并增加最小合同测试；不修改通用 Ops 大盘预聚合、不迁移、不回填、不使用 GitHub Actions。候选 `3957ca4b7` 经任务审查和最终全分支审查均 `APPROVE`，已合并为 `main@73f36df44`。Task 1 已完成 RED→GREEN：7 天快照合同测试记录的 group overview filter 已从 `auto` 切换为 `raw`，聚焦服务测试、完整 `internal/service`、MonitorV2 race 测试与 `git diff --check` 通过；尚未推送、部署或线上验证。
