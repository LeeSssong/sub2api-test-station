...
**T91-A 额度账务 schema、Ent、迁移与只读对账基础（2026-09-01）：** 状态 `REFRESH_REQUIRED`（Task 0 根审查补证）。候选 `codex/t91-a-task0-source-baseline@b112e84a1` 已提交源码映射与验收站只读盘点；因验收站无 usage 样本，paid/gift 不足矩阵、失败请求、零费用、重复回放等运行态场景仍未完成，必须在独立任务中补证并刷新报告后再放行 schema/Ent/迁移。固定 `attempted_quota_usd` 为应扣额度、`delta_usd` 为实际扣费，未扣差额不进余额公式、不形成欠款；旧 Ent `float64` 只能经统一适配器进入 `decimal.Decimal` 新账务域。退款 Saga、`quota_refund.requested` worker 和 dead-letter 合同属于 T91-D；不确定结果不得落终态 `failed`。在 Task 0 报告获根总控接受前不得修改 schema、迁移或运行代码。

**T98-R2 飞书余额新鲜度与排名投影修复（2026-09-01）：** 状态 `DESIGNING`。用户确认同一规范化 BaseURL 继续取 `observed_at` 最新的有效 API Key 可用余额。生产只读证据显示 CX/SHUAI 告警使用约一小时前旧快照；代码根因是有真实流量时主动探测提前返回，并连带跳过余额刷新。排名全部“未排名”的确定性根因是通知读取默认 `List()` 投影中的 `SchedulerRank`，而该路径只生成 `GroupRank`。批准方案：余额刷新与主动探测解耦；通知改读生成原生 `SchedulerRank` 的 24 小时投影；TDD 覆盖流量跳探测仍刷新、最新快照选择和分组调度排名。独立 worktree/分支为 `.worktrees/t98-r2-feishu-balance-rank` / `codex/t98-r2-feishu-balance-rank`；无迁移、配置、余额事实源、生产写入或真实飞书发送。T106 推送阻塞期间只允许设计、实现和直接测试，不进入整合/发布车道。
