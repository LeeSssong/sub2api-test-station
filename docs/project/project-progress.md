...
**本轮 T01 大上下文入站上传稳定性（2026-08-11，2026-08-12 审查修复）：** 状态：READY_FOR_ROOT_REVIEW。候选工作区 `.worktrees/t01-upload-stability`，分支 `codex/t01-upload-stability`，基线 `main@54f35713323116ce44646041347bde9ce548295e`。范围仅限 Caddy 入站长窗口、现有请求体大小保护兼容、配置合同测试及受控慢速/不完整上传验证；不修改错误转译、重试、调度、CDN、生产或其他任务包。已登记规格 `docs/superpowers/specs/2026-08-11-t01-large-context-upload-stability-design.md` 与计划 `docs/superpowers/plans/2026-08-11-t01-large-context-upload-stability.md`。实现：Caddy `read_body 15m` 覆盖慢速请求体读取；公共 fallback `response_header_timeout 15m` 仅覆盖完整请求写入 upstream 后的响应头等待；原生 128/128/16 MiB body limits 保持不变。审查修复将上传测试改为加载真实 `infra/Caddyfile`，只在测试派生配置中关闭证书签发、注入本地监听地址和 upstream；以 `/health` 成功反代作为 readiness，不依赖异步访问日志。验证：生产 `15m` 策略下 301 秒持续上传成功；短模式连续 3 次稳定通过；从同一配置仅把 `read_body` 改为 `2s` 的派生测试证明不完整上传释放且未到达 upstream。Caddy validate/adapt、Caddy/NGINX 合同、后端 ingress/body-limit 定向测试、`go vet`、构建、`git diff --check` 通过。完整 `tests/infra/validate-baseline.sh` 在既有无关 Dockerfile `ARG FRONTEND_NODE_MAX_OLD_SPACE_SIZE=1536` 合同不匹配处失败，未修改该无关问题；无迁移、无生产动作，尚未合并、推送、部署或线上验证。
