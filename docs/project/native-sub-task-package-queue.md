...
- 队列状态：官方 Sub2API `v0.1.177` 更新是当前最高优先级插队任务；5 个冲突已人工解决，候选已合入并推送为 `main@716f6876f19c7781afe1df5e43bf0ab889e6e325`，发布链在候选制品生成后以 `migration_set_changed` 返回 `downtime_required=true`（预计不可用约 300 秒），现处于 `BLOCKED`，等待用户人工确认；未启动候选、未切换槽位、未部署。官方更新完成并解除门禁后，先安排 T14 用量详情字段兼容最小热修；T12 与 T13 暂停在设计阶段，S1-R2 保持原队列位置。
- 唯一发布总控：根目录 `/Users/gongtengxinwen/Documents/sub2api搭建` 的 `main`。只有发布总控可以修改全局队列/总账、根 `main`、发布证据和生产状态记录。
- 当前发布状态：官方 `v0.1.177` 已完成候选交接、根合并和 push；既有本地/宿主发布链已完成候选制品生成，但停机预检返回 `downtime_required=true` / `migration_set_changed`，已在候选启动前停止。T12、T13、T14 不得进入实现、合并或发布。
...
- 当前状态：`BLOCKED`（`downtime_required=true`）。用户指定可见任务 `01a008e1-55d1-74f1-a659-fac363dcfd28` 继续作为执行窗口，并由可见任务 `01a006a8-ef15-7960-b72a-de2fddab0339` 统筹；旧方向的提交和未提交内容已保全，新方向已刷新并完成官方合并。发布链预检返回 `reason_code=migration_set_changed`、预计不可用约 300 秒；按门禁不得启动候选或绕过停机保护，证据见 `/Users/gongtengxinwen/.codex/release-evidence/sub2api/2026-08-16-main-716f6876f-official-v0177-downtime-gate.json`。
...
- 完成条件：取得用户对该次停机门禁的明确确认后，按既有发布链完成切换；不另做功能验收。发布链若继续因停机/迁移/运行就绪保护拒绝，保留候选和证据，不绕过保护。
...
- T07、T08、T10、T11、T11-R1 与 OAuth MIME 热修已完成生产收口；T09 官方 v0.1.177 更新当前因 `downtime_required=true` 处于 `BLOCKED`，发布车道暂停等待人工确认。
- T09 已完成官方候选刷新、根合并、push 和候选制品生成；当前停在迁移集合变化的停机门禁前。T14 排在官方更新完成之后；S1-R2 继续原队列位置，S1 旧候选继续冻结。
