...
- 当前状态：进行中（已合并、已推送、待明确停机授权）。候选 `2e28308a04495482fbe16af05344111bb4a0b0a7` 已合并为 `main@dbe1231e0243d7190996f21cd8b8e5ac17b8420a`；当前已推送发布基线为 `main@f0746826c10a83688ad4363ac860a74f0f041e32`。合并后专项测试、构建、负向守门与 `0600` 证据均通过；新增迁移 222 使宿主预检返回 `downtime_required=true`、`reason_code=migration_set_changed`、预计不可用 300 秒。生产未变更，旧候选 `1c3e87684`、`ce5691527` 保持冻结，不得恢复。
...
- T03-R1 只能等待用户明确授权“允许停机部署”；若授权，必须使用受审维护发布路径、保留新增独立表并完成线上验收。旧计划和冻结候选不得恢复。
